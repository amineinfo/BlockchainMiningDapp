...
/**
 * This is the implementation of the HUSRM algorithm that we have submited at
 * MLDM 2015. <br/>
 * <br/>
 * Zida, S., Fournier-Viger, P., Wu, C.-W., Lin, J. C. W., Tseng, V.S., (2015).
 * Efficient Mining of High Utility Sequential Rules. Proc. 11th International
 * Conference on Machine Learning and Data Mining (MLDM 2015). Springer, LNAI,
 * 15 pages (to appear). <br/>
 * 
...
public class AlgoHUSRM {
	 ...
	 * /* some additional information about the algorithm will be shown in the
	 * console for /* debugging
	 **/
	final boolean DEBUG = false;

	/**
	 * this is a contrainst on the maximum number of item that the left side of a
	 * rule should /* contain
	 */
	private int maxSizeAntecedent;

	/**
	 * this is a contrainst on the maximum number of item that the right side of a
	 * rule should /* contain
	 */
	...
	/**
	 * Strategy 2: remove rules contains two items a--> b with a sequence estimated
	 * utility < minutil
	 */
	private boolean deactivateStrategy2 = false;

	/**
	 * Strategy 3 use bitvectors instead of array list for quickly calculating the
	 * support of /* rule antecedent
	 */
	private boolean deactivateStrategy3 = false;

	/**
	 * Strategy 4 : utilize the sum of the utility of lutil, lrutil and rutil /* If
	 * deactivated, we use the same utility tables, but the upper bound will be
	 * calculated as /* lutil + lrutil + rutil instead of the better upper bounds
	...
	/**
	 * This is a structure to store some estimated utility and a list of sequence
	 * ids. It will be use in the code for storing the estimated utility of a rule
	 * and the list of sequence ids where the rule appears.
	 */
	 ...
	 * @param maxConsequentSize        a constraint on the maximum number of items
	 *                                 that the right side of a rule should contain
	 * @param maxAntecedentSize        a constraint on the maximum number of items
	 *                                 that the left side of a rule should contain
	...
	public void runAlgorithm(String input, String output, double minConfidence, double minutil, int maxAntecedentSize,
			int maxConsequentSize, int maximumNumberOfSequences) throws IOException {
		...
		if (deactivateStrategy1 == false) {
			...
			if (DEBUG) {
				...
				for (Entry<Integer, Double> entreeMap : mapItemEstimatedUtility.entrySet()) {
					...
					System.out.println("item : " + entreeMap.getKey() + " profit estime: " + entreeMap.getValue());
				}

				// NEXT STEP: WE WILL REMOVE THE UNPROMISING ITEMS

				System.out.println("==============================================================================");
				System.out.println("-------------------ESTIMATED UTILITY OF PROMISING ITEMS      ----------------");
				System.out.println("==============================================================================");
			}
			...
			while (iterator.hasNext()) {
				...
				Map.Entry<java.lang.Integer, java.lang.Double> entryMapItemEstimatedUtility = (Map.Entry<java.lang.Integer, java.lang.Double>) iterator
						.next();
			...
			if (DEBUG) {
				...
				System.out.println("==============================================================================");
				System.out.println(
						"-------------- DATABASE WITH ONLY ITEMS HAVING ESTIMATED UTILITY >= miinutil-------------");
				System.out.println("==============================================================================");
		...
		if (DEBUG) {
			...
			System.out.println("==============================================================================");
			System.out.println("----- FOR EACH ITEM, REMEMBER THE IDS OF SEQUENCES CONTAINING THE ITEM  -------");
			System.out.println("==============================================================================");

		}

		// We create a map to store for each item, the list of sequences containing the
		// item
		...
		for (int i = 0; i < database.getSequences().size(); i++) {
			...
			for (List<Integer> itemset : sequence.getItemsets()) {
				...
				for (Integer item : itemset) {
					...
					// finally we add the current sequence ids to the list of sequences ids of the
					// current
					// item
		...
		if (DEBUG) {
			...
			System.out.println("==============================================================================");
			System.out.println("----- CALCULATE SEQUENCE ESTIMATED UTILITY OF EACH RULE OF SIZE 2 -------------");
			System.out.println("==============================================================================");
		}

		// We create a map of map to store the estimated utility and list of sequences
		// ids for
		...
		// The value in the second map: the estimated utility of the rule and sequence
		// ids for that rule
				...
				for (int j = 0; j < itemset.size(); j++) {
					...
					for (int k = i + 1; k < sequence.getItemsets().size(); k++) {
						...
						for (Integer itemY : itemsetK) {
							...
							Map<Integer, EstimatedUtilityAndSequences> mapXItemUtility = mapItemItemEstimatedUtility
									.get(itemX);
		...
		// if in debuging mode
		if (DEBUG) {
			// we will print the estimated utility and list of sequences ids of all rules
			// containing two items
			// e.g. "a" -> "b"

			// for each item X
			for (Entry<Integer, Map<Integer, EstimatedUtilityAndSequences>> entreeX : mapItemItemEstimatedUtility
					.entrySet()) {
				...
				for (Entry<Integer, EstimatedUtilityAndSequences> entreeYProfit : entreeX.getValue().entrySet()) {
					...
					System.out.println("  RULE: " + itemX + " --> " + itemY + "   estimated utility "
							+ structureXY.utility + "   sequences " + structureXY.sequenceIds);
				}
			}

			System.out.println("==============================================================================");
			System.out.println("-------------- RULES OF SIZE 2 WITH ESTIMATED UTILITY >= minutil -------------");
			System.out.println("==============================================================================");
		}
		...
		if (DEBUG) {
			...
			System.out.println("==============================================================================");
			System.out.println("-------------- RULES OF SIZE 2 WITH UTILITY >= minutil -------------");
			System.out.println("==============================================================================");
		}

		// For each rule X --> Y
		for (Entry<Integer, Map<Integer, EstimatedUtilityAndSequences>> entryX : mapItemItemEstimatedUtility
				.entrySet()) {
			// Get the item X
			...
			for (Entry<Integer, EstimatedUtilityAndSequences> entryYUtility : entryX.getValue().entrySet()) {
				...
				for (Integer numeroSequence : sequencesIDsXY) {
					...
					// At the same time as we scan the sequence, we will add the utility of items in
					// Y

					// for each itemset starting from the last one until itemset alpha+1
					loop2: for (int i = sequence.getItemsets().size() - 1; i > element.positionAlphaItemset; i--) {
					...
					// (B) Scan the other itemsets after the alpha itemset but before the beta
					// itemset
				...
				// if in debug mode
				if (DEBUG) {
					// We will print the rule and its profit and whether it is a high utility rule
					// or not
					String isInteresting = (table.totalUtility >= minutil) ? " *** HIGH UTILITY RULE! ***" : " ";
					System.out.println("\n  RULE: " + itemX + " --> " + itemY + "   utility " + table.totalUtility
							+ " frequence : " + supportXY + " confiance : " + confidence + isInteresting);
					...
					for (ElementOfTable element : table.elements) {
						System.out.println("      SEQ:" + element.numeroSequence + " \t utility: " + element.utility
								+ " \t lutil: " + element.utilityLeft + " \t lrutil: " + element.utilityLeftRight
								+ " \t rutil: " + element.utilityRight + " alpha : " + element.positionAlphaItemset
								+ " beta : " + element.positionBetaItemset);
					}

					System.out.println("      TOTAL: " + " \t utility: " + table.totalUtility + " \t lutil: "
							+ table.totalUtilityLeft + " \t lrutil: " + table.totalUtilityLeftRight + " \t rutil: "
							+ table.totalUtilityRight);
					System.out.println("      Should we explore larger rules by left expansions ? "
							+ (conditionExpandLeft >= minutil) + " (" + conditionExpandLeft + " )");
					System.out.println("       Should we explore larger rules by right expansions ? "
							+ (conditionExpandRight >= minutil) + " (" + conditionExpandRight + " )");
				}
				...
				// if the right side size is less than the maximum size, we will try to expand
				// the rule
				if (conditionExpandRight >= minutil && maxConsequentSize > 1) {
					expandRight(table, antecedent, consequent, sequenceIDsX);
				}

				// if the left side size is less than the maximum size, we will try to expand
				// the rule
	...
	private void saveRule(int[] antecedent, int[] consequent, double utility, double support, double confidence)
		...
		// if we are in debug mode, we will automatically check that the utility,
	...
	/**
	 * This method is used for debugging. It scan a database to check if the
	 * measures (confidence, utility, support) of a given rule have been correctly
	 * calculated.
	 * 
	 ...
	 * @param confidence the rule confidence
	 */
	private void checkMeasuresForARule(int[] antecedent, int[] consequent, double utility, double support,
			double confidence) {
		...
		if (support != supportOfTheRule) {
			throw new RuntimeException(" The support is incorrect for the rule : " + Arrays.toString(antecedent)
					+ " ==>" + Arrays.toString(consequent) + "   support : " + support + " recalculated support: "
					+ supportOfTheRule);
		}

		// We now check if the confidence is the same as the confidence calculated by
		// HUSRM
		double recalculatedConfidence = supportOfTheRule / supportOfAntecedent;

		if (confidence != recalculatedConfidence) {
			throw new RuntimeException(" The confidence is incorrect for the rule :" + Arrays.toString(antecedent)
					+ " ==>" + Arrays.toString(consequent) + "   confidence : " + confidence
					+ " recalculated confidence: " + recalculatedConfidence);
		}

		// We now check if the utility is the same as the utility calculated by HUSRM
		if (utility != utilityOfTheRule) {
			throw new RuntimeException(" The utility is incorrect for the rule :" + Arrays.toString(antecedent) + " ==>"
					+ Arrays.toString(consequent) + "   utility : " + utility + " recalculated utility "
					+ utilityOfTheRule);
		}
	}

	/**
	 * This method is used to create new rule(s) by adding items to the right side
	 * of a rule
	 * 
	 * @param table                 the utility-table of the rule
	 * @param antecedent            the rule antecedent
	 * @param consequent            the rule consequent
	 * @param sequenceIdsAntecedent the list of ids of sequences containing the left
	 *                              side of the rule
	 * @throws IOException if an error occurs while writing to file
	 */
	private void expandRight(UtilityTable table, int[] antecedent, int[] consequent,
			ListSequenceIDs sequenceIdsAntecedent) throws IOException {
		...
		// We create a new map where we will build the utility table for the new rules
		// that
		// will be created by adding an item to the current rule.
		// Key: an item appended to the rule Value: the utility-table of the
		// corresponding new rule
...
//		// for each sequence containing the original rule (according to its utility table)
		for (ElementOfTable element : table.elements) {
			...
			// We will look here for the case where an item J is added to the right side of
			// a rule
			// but it is an item found between the left side and right side of the rule in
			// the sequence.
			// In that case, the position beta will change to a new position that we will
			// call beta prime.
			...
			for (int i = element.positionBetaItemset - 1; i > element.positionAlphaItemset; i--) {
				...
				for (int j = 0; j < itemsetI.size(); j++) {
					...
					// right side, or left and right side of the rule according to the
					...
					if (isLeft) {
						...
						// the utility of items that could be append to left side until the current
						// itemset
						...
						for (int z = i; z < element.positionBetaItemset; z++) {
							...
							for (int w = 0; w < itemsetZ.size(); w++) {
								...
								boolean wIsLeftRight = itemW > largestItemInAntecedent
										&& itemW > largestItemInConsequent;
						...
						newElement.utilityRight = element.utilityRight - profitItemJ + sumUtilityLRUtilItemsSmallerThanX
								- sumUtilityRUtilItemsSmallerThanX;
						...
						newElement.utilityLeftRight = element.utilityLeftRight - profitItemJ
								- sumUtilityLeftRightUntilBetaPrime;
		...
		for (Entry<Integer, UtilityTable> entryItemTable : mapItemsTables.entrySet()) {
			...
			if (deactivateStrategy4) {
				...
				shouldExpandLeftSide = utilityTable.totalUtility + utilityTable.totalUtilityLeft
						+ utilityTable.totalUtilityLeftRight >= minutil && antecedent.length + 1 < maxSizeAntecedent;
				shouldExpandRightSide = utilityTable.totalUtility + utilityTable.totalUtilityRight
						+ utilityTable.totalUtilityLeftRight + utilityTable.totalUtilityLeft >= minutil
						&& consequent.length + 1 < maxSizeConsequent;
			...
			if (isHighUtility && confidence >= minConfidence) {
				...
				saveRule(antecedent, newConsequent, utilityTable.totalUtility, utilityTable.elements.size(),
						confidence);
				...
				if (DEBUG) {
					System.out.println("\n\t  HIGH UTILITY SEQ. RULE: " + Arrays.toString(antecedent) + " --> "
							...
							+ " support : " + utilityTable.elements.size() + " confidence : " + confidence);
				...
				// If we are in debugging mode and the rule is not high utility and high
				// confidence,
	...
	/**
	 * This method will recursively try to append items to the left side of a rule
	 * to generate rules containing one more item on the left side. This method is
	 * only called for rules of size 1*1, thus containing two items (e.g. a -> b)
	 * 
	 * @param utilityTable          the rule utility table
	 * @param antecedent            the rule antecedent
	 * @param consequent            the rule consequent
	 * @param sequenceIDsConsequent the list of sequences ids of sequences
	 *                              containing the rule antecedent
	 * @throws IOException if error while writting to file
	 */
	private void expandFirstLeft(UtilityTable utilityTable, int[] antecedent, int[] consequent,
			ListSequenceIDs sequenceIDsConsequent) throws IOException {
		...
		// for each sequence containing the rule (a line in the utility table of the
		// original rule)
		for (ElementOfTable element : utilityTable.elements) {
			...
			for (int i = 0; i < element.positionBetaItemset; i++) {
				...
				for (int j = 0; j < itemsetI.size(); j++) {
					...
					// we will update the utility vlaue of that rule by adding the utility of the
					// item
					// in that sequence
					double profitItemJ = sequence.getUtilities().get(i).get(j);
					newElement.utility = element.utility + profitItemJ;
					...
					// called lutil
					if (deactivateStrategy4) {
						newElement.utilityLeft = element.utilityLeft + element.utilityLeftRight + element.utilityRight
								- profitItemJ;
					} else {
		...
		// by recursively adding to the left, will staty the same. So we don't store it
		// in the
		...
		for (Entry<Integer, UtilityTableLeft> entryItemTable : mapItemUtilityTable.entrySet()) {
			...
			// We need to calculate the list of sequences ids containing the antecedent of
			// the new
	 ...
	 * This method will recursively try to append items to the left side of a rule
	 * to generate rules containing one more item on the left side. This method is
	 * called for rules containing at least 2 items on their left side already. For
	 * rules having 1 item on their left side another method is used instead.
	 * 
	 * @param utilityTable          the rule utility table
	 * @param antecedent            the rule antecedent
	 * @param consequent            the rule consequent
	 ...
	 * @throws IOException if error while writting to file
	 */
	private void expandSecondLeft(UtilityTableLeft utilityTable, int[] antecedent, int[] consequent,
			ListSequenceIDs sequenceIDsConsequent, Map<Integer, Integer> tableBeta) throws IOException {
	...
	/**
	 * This class represents a utility-table used for left expansions (what we call
	 * a left-utility table)
	 * 
	 * @author Souleymane Zida, Philippe Fournier-Viger
	 */
	...
	/**
	 * This class represents a element(line) of a utility-table used for left
	 * expansions
	 * 
	 * @author Souleymane Zida, Philippe Fournier-Viger
	 */
	...
	// ============================================================================================================================
	// =========================================== CLASS FOR LEFT-RIGHT
	// UTILITY-TABLES===========================================
	// ============================================================================================================================

	/**
	 * This class represents a utility-table used for left or right expansions (what
	 * we call a left-right utility table)
	 * 
	 * @author Souleymane Zida, Philippe Fournier-Viger
	 */
	...
	/**
	 * This class represents a element(line) of a utility-table used for left or
	 * right expansions
	 * 
	 * @author Souleymane Zida, Philippe Fournier-Viger
	 */
	public class ElementOfTable {
		 ...
		 */
		public ElementOfTable(int sequenceID, double utility, double utilityLeft, double utilityLeftRight,
				double utilityRight) {
			this.numeroSequence = sequenceID;
			this.utility = utility;
			this.utilityLeft = utilityLeft;
