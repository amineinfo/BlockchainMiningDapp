...
/**
 * The CMRules algorithm for mining sequential rules common to several
 * sequences. <br/>
 * <br/>
 * 
 * This algorithm is described in: <br/>
 * <br/>
 * 
 * Fournier-Viger, P., Faghihi, U., Nkambou, R. & Mephu Nguifo, E. (2010).
 * CMRules: An Efficient Algorithm for Mining Sequential Rules Common to Several
 * Sequences. Proceedings of the 23th International Florida Artificial
 * Intelligence Research Society Conference (FLAIRS 2010). AAAI press. <br/>
 * <br/>
 * 
 * This implementation use a modified AprioriTID algorithm for generating
 * association rules in phase 1.
 * 
 * @see Rule
 * @see Rules
 * @see Itemset
...
//	 a triangular matrix for efficiently counting the support of pairs of items
	...
	/**
	 * Run the algorithm with the minsup parameter as a number of sequences
	 * (integer).
	 * 
	 * @param input           input file containing a sequence database.
	 * @param output          the file path for writing the result
	 ...
	 * @throws IOException exception if error while writing the output file.
	 */
	public void runAlgorithm(int relativeSupport, double minConfidence, String input, String output)
			throws IOException {
...
//		System.out.println("STEP 0");
		...
		// (1.b) create the triangular matrix for counting the support of itemsets of
		// size 2
...
//		System.out.println("NUMBER OF DIFFERENT ITEMS : " + mapItemCount.size());
	...
	private void calculateSequentialMeasures(Rule rule, Sequence sequence) {
		...
		// part of the rules
		Set<Integer> setAlreadySeen = new HashSet<Integer>(rule.getItemset1().size() * 3); // could be replaced with a
																							// flag on items
...
//		transactionDatabase.printDatabase();
	...
	/**
	 * Calculate the interestingness measures of a rule and then if it is a valid
	 * rule, save it to the output file.
	 * 
	...
	void checkRule(Rule rule) throws IOException {
		...
		if (rule.sequentialTransactionCount >= minCSupRelative && rule.getSequentialConfidence() >= minSeqConfidence) {
...
//				System.out.println(); // DEBUG
				...
				/// ================ I ADDED THIS BECAUSE THE ALGORITHM AS DESCRIBED BY
				/// AGRAWAL94
				...
				for (Itemset hm_P_1 : H1) {
					...
					if (conf >= minSeqConfidence) {
						...
						if (leftsize <= maxLeftSize && leftsize >= minLeftSize && 1 >= minRightSize
								&& 1 <= maxRightSize) {
...
//		System.out.println(" " + lk.toString() + "  " + Hm.toString());
		if (k > m + 1) {
			...
			for (Itemset hm_P_1 : Hm_plus_1) {
				...
				if (conf >= minSeqConfidence) {
					...
					if (leftsize <= maxLeftSize && leftsize >= minLeftSize && m + 1 >= minRightSize
							&& m + 1 <= maxRightSize) {
	...
	/**
	 * This method checks if all the subsets of size "k" of the itemset "candidate"
	 * are frequent. It is similar to what is used in the Apriori algorithm for
