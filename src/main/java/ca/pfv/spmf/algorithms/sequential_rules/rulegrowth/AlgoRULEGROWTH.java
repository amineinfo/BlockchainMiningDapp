...
public class AlgoRULEGROWTH {
	 ...
	 * occurences (value). // The map of occurences associates to sequence ID (key),
	 * an occurence of the item (value).
	 */
...
//		
	...
	private void expandRight(int[] itemsetI, int[] itemsetJ, Set<Integer> tidsI, Collection<Integer> tidsJ,
			Collection<Integer> tidsIJ, Map<Integer, Occurence> occurencesI, Map<Integer, Occurence> occurencesJ)
			throws IOException {
//		if(true)
//    	return;
		...
		for (Entry<Integer, Set<Integer>> entry : frequentItemsC.entrySet()) {
			...
			if (tidsI_JC.size() >= minsuppRelative) {
				...
				if (confI_JC >= minConfidence) {
					...
					if (DEBUG) {
						Rule newRule = new Rule(itemsetI, itemsetJC, tidsI, tidsJC, tidsI_JC, occurencesI,
								occurencesJC);
						allRulesFoundForDEBUG.add(newRule);
					}
				}
...
//		System.out.println("NUMBER OF DIFFERENT ITEMS : " + mapItemCount.size());
	 ...
	 * in the itemset are sorted in lexical order This version also checks that if
	 * the item "item" was added it would be the largest one according to the
	 * lexical order.
	 * 
