...
/**
 * This is a modified implementation of the TRULEGROWTH algorithm for mining
 * sequential rules from sequences containing Strings instead of integers. <br/>
 * <br/>
 * 
 * Fournier-Viger, P., Wu, C.-W., Tseng, V.S., Nkambou, R. (2012). Mining
 * Sequential Rules Common to Several Sequences with the Window Size Constraint.
 * Proceedings of the 25th Canadian Conf. on Artificial Intelligence (AI 2012),
 * Springer, LNAI 7310, pp.299-304. <br/>
 * <br/>
 * 
 * In future a version of SPMF, it is planned to remove this class and to
 * provide a more general mechanism for handling strings that would work for all
 * algorithms that take sequences as input.
 * 
 * @see Itemset
 * @see AlgoTRuleGrowth
 * @see Sequence
...
public class AlgoTRuleGrowth_withStrings {
	...
	// KEY: an item
	// VALUE: a map of key: sequence ID value: occurences of the item in that
	// sequence.
	 ...
	 */
	public void runAlgorithm(double minSupport, double minConfidence, String input, String output, int windowSize)
			throws IOException {
	...
	public void runAlgorithm(String input, String output, int relativeMinSupport, double minConfidence, int windowSize)
		...
		// IMPORTANT : THIS IS A FIX SO THAT THE DEFINITION IS THE SAME AS IN THE
		// ARTICLE!!
...
//							rules.addRule(ruleJI);
	 ...
	 * any item c. This results in rules of the form I U {c} --> J. The method makes
	 * sure that: - c is not already included in I or J - c appear at least minsup
	 * time in tidsIJ before last occurence of J - c is lexically bigger than all
	 * items in I
	 * 
	 ...
	 */
	private void expandLeft(String[] itemsetI, String[] itemsetJ, Collection<Integer> tidsI, Collection<Integer> tidsIJ // ,
		...
		for (Integer tid : tidsIJ) {
			...
			do {
				...
				if (mapMostLeftFromI.size() == itemsetI.length && mapMostLeftFromJ.size() == itemsetJ.length) {
					...
					// Such item c has to appear in the window before the last occurence of J
					// (before "minimum")
...
//    						if lexical order is not respected or c is included in the rule already.			
	...
	// It was necessary to do that because when an element is re-inserted in a
	// linked list,
	// the access order remain the one of the first insertion.
	private void addToLinked(LinkedHashMap<String, LinkedList<Integer>> mapMostLeftFromI, String key,
	...
	/**
	 * This method removes elements out of the current window from a hashmap
	 * containing the position of items at the left of an itemset. key: item value :
	 * a itemset position
	 * 
	 * @param mapMostLeftFromItemset the map
	 * @param lastElementOfWindow    the last itemset of the window in terms of
	 *                               itemset position in the sequence.
	 */
	private void removeElementOutsideWindow(LinkedHashMap<String, Integer> mapMostLeftFromI,
	...
	/**
	 * This method removes elements out of the current window from a hashmap
	 * containing the position of items at the right of an itemset. key: item value
	 * : a itemset position
	 * 
	 * @param mapMostLeftFromItemset the map
	 * @param lastElementOfWindow    the last itemset of the window in terms of
	 *                               itemset position in the sequence.
	 */
	private void removeElementOutsideWindowER(LinkedHashMap<String, Integer> mapMostRightfromI,
	 ...
	 * sure that: - c is not already included in I or J - c appear at least minsup
	 * time in tidsIJ after the first occurence of I - c is lexically bigger than
	 * all items in J
	 * 
	 * @param mapWindowsJI
	 * @throws IOException
	 */
	private void expandRight(String[] itemsetI, String[] itemsetJ, Set<Integer> tidsI, Collection<Integer> tidsJ,
...
//				Rule ruleI_JC = new Rule(ruleIJ.getItemset1(), itemsetJC, confI_JC, tidsI_JC.size());
	...
	/**
	 * This method calculate the frequency of each item in one database pass. Then
	 * it remove all items that are not frequent.
	 * 
...
//		System.out.println("NUMBER OF DIFFERENT ITEMS : " + mapItemCount.size());
	 ...
	 * that if the item "item" was added it would be the largest one according to
	 * the lexical order
	 * 
	 * @param item    an item
	...
	boolean containsLEXPlus(String[] itemset, String item) {
		...
		for (int i = 0; i < itemset.length; i++) {
			...
			if (itemset[i].equals(item)) {
				...
				return true;
				// if the current item is larger than the item that is searched,
				// then return true because if if the item "item" was added it would be the
				// largest one
	...
	/**
	 * This method checks if the item "item" is in the itemset. It asumes that items
	 * in the itemset are sorted in lexical order
	 * 
	 * @param item    an item
	 * @param itemset an itemset
	 * @return true if the item is contained in the itemset
