...
/**
 * This is the original implementation of the TRULEGROWTH algorithm for mining
 * sequential rules with a window size constraint. The TRuleGrowth algorithm is
 * a variation of the RuleGrowth algorithm described in this paper: <br/>
 * <br/>
 * 
...
public class AlgoTRuleGrowth {
	 ...
	 * Run the algorithm.
	 * 
	 * @param relativeMinSupport the minsup parameter as a a relative value
	 *                           (integer)
	 * @param minConfidence      minimum confidence (a value between 0 and 1).
	 * @param input              the input file path
	 * @param output             the output file path
...
//							rules.addRule(ruleJI);
	 ...
	 */
	private void expandLeft(int[] itemsetI, int[] itemsetJ, Collection<Integer> tidsI, Collection<Integer> tidsIJ // ,
//    						Map<Integer, Occurence> mapOccurencesJ
		...
		for (Integer tid : tidsIJ) {
			...
			// We use linkedhashMap to as to keep the order of what is inserted into the
			// maps
...
//    			}
	...
	/**
	 * This method insert a key and a value in a hashmap while making sure that the
	 * insertion order is preserved. It was necessary to do that because when an
	 * element is re-inserted in a linked list, the access order remain the one of
	 * the first insertion.
	 * 
	 ...
	 * @param value            a value
	 */
	private void addToLinked(LinkedHashMap mapMostLeftFromI, Object key, Object value) {
	 ...
	 */
	private void removeElementOutsideWindow(LinkedHashMap<Integer, Integer> mapMostLeftFromItemset,
	 ...
	 *                               itemset position in the sequence.
	 */
	private void removeElementOutsideWindowER(LinkedHashMap<Integer, Integer> mapMostRightfromI,
	...
	/**
	 * This method search for items for expanding left side of a rule I --> J with
	 * any item c. This results in rules of the form I --> J U�{c}. The method
	 * makes sure that: - c is not already included in I or J - c appear at least
	 * minsup time in tidsIJ after the first occurence of I - c is lexically bigger
	 * than all items in J
	 * 
	 * @param mapWindowsJI
	 * @throws IOException
	 */
	private void expandRight(int[] itemsetI, int[] itemsetJ, Set<Integer> tidsI, Collection<Integer> tidsJ,
...
//    						Map<Integer, Occurence> occurencesJ
		...
		for (Integer tid : tidsIJ) {
			...
			// there maps are used when scanning the sequence to determine
			// what is currently inside the window and what fall out of the window.
			// We use linkedhashMap so as to keep the order of what is inserted into the
			// maps
			// There maps are key: item value: position of an itemset
...
//				Rule ruleI_JC = new Rule(ruleIJ.getItemset1(), itemsetJC, confI_JC, tidsI_JC.size());
	 ...
	 * it remove all items that are not frequent in another database pass.
	 * 
