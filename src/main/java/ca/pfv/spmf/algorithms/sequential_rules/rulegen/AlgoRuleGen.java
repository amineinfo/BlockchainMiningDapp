...
/**
 * This is an implementation of the RuleGen algorithm proposed by Zaki et al to
 * generate sequential rules where the antecedent and consequent are sequential
 * patterns. The RuleGen algorithm is described in: <br/>
 * <br/>
 * 
 * M. J. Zaki, “SPADE: An Efficient Algorithm for Mining Frequent
 * Se-quences,”Machine Learning, vol. 42, no.1-2, pp. 31-60, 2001. <br/>
 * <br/>
 * 
 * However, note that instead of using the SPADE algorithm, we use the
 * PrefixSpan algorithm because at the time that this algorithm was implemented
 * there was no implementation of SPADE in SPMF and PrefixSpan is a fast
 * algorithm.
 * 
 * @see AlgoPrefixSpan
...
public class AlgoRuleGen {
	...
	public void runAlgorithm(int minsup, double minconf, String input, String output) throws IOException {
		...
		// STEP 1: Apply the PrefixSpan algorithm to generate frequent sequential
		// patterns
	...
	/**
	 * Try to generate a rule between two sequential patterns. The rule is generated
	 * if the pattern1 is included in pattern2 and if the confidence is high enough.
	 * 
	 * @param pattern1 a sequential pattern
	 * @param pattern2 another sequential pattern
	 * @throws IOException
	 */
	private void tryToGenerateRule(SequentialPattern pattern1, SequentialPattern pattern2, double minconf)
			throws IOException {
...
//
	...
	boolean strictlyContains(SequentialPattern pattern1, SequentialPattern pattern2) {
		// if pattern2 is larger or equal in size, then it cannot be contained in
		// pattern1
