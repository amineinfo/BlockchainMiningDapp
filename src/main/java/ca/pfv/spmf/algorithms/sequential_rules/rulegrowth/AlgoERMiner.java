...
/**
 * This is the original implementation of the ERMiner algorithm for mining
 * sequential rules common to several sequences where antecedent and consequent
 * are unordered itemsets. <br/>
 * <br/>
...
public class AlgoERMiner {
	...
	// *** internal variables ***/
	/**
	 * This map contains for each item (key) a map of occurences (value). // The map
	 * of occurences associates to sequence ID (key), an occurence of the item
	 * (value).
	 */
	 ...
	 * The main method to run the algorithm
	 * 
	 * @param relativeMinsup : the minimum support as an integer value (a relative
	 *                       minimum support)
	 * @param minConfidence  : the minimum confidence threshold
	 * @param input          : an input file path of a sequence database
	 * @param output         : a file path for writing the output file containing
	 *                       the seq. rules.
	 * @exception IOException if error reading/writing files
	 */
...
//		countPruning = 0;
		...
		// The map for left-equivalence classes has a key : item on the right side,
		// value: list of rules
		Map<Integer, LeftEquivalenceClass> mapEclassLeft = new HashMap<Integer, LeftEquivalenceClass>();
		// The map for right-equivalence classes has a key : item on the left side,
		// value: list of rules
		Map<Integer, RightEquivalenceClass> mapEclassRight = new HashMap<Integer, RightEquivalenceClass>();

		// for each entry in the matrix
		// entry = key: item I value: a map where key: item J and value: the support of
		// {i, j}
			...
			for (Entry<Integer, Integer> entryJ : entry.getValue().entrySet()) {
				...
				if (tidsIJ.size() >= minsuppRelative) {
					...
					if (maxAntecedentSize > 1 || maxConsequentSize > 1) {
						...
						registerRule11(intI, intJ, tidsI, tidsJ, tidsIJ, occurencesI, occurencesJ, mapEclassLeft,
								mapEclassRight);
					}
				}
				...
				if (tidsJI.size() >= minsuppRelative) {
					...
					// register the rule in the appropriate equivalence classes
					if (maxAntecedentSize > 1 || maxConsequentSize > 1) {
						registerRule11(intJ, intI, tidsJ, tidsI, tidsJI, occurencesJ, occurencesI, mapEclassLeft,
								mapEclassRight);
					}
				}
			}
		...
		// PROCESS ALL EQUIVALENCE CLASSES WITH MORE THAN ONE ITEM IN LEF PART FOR
				...
				for (LeftEquivalenceClass eclass : eclassList) {
					if (eclass.rules.size() != 1) {
						Collections.sort(eclass.rules, new Comparator<LeftRule>() {
							public int compare(LeftRule arg0, LeftRule arg1) {
								return arg0.itemsetI[arg0.itemsetI.length - 1]
										- arg1.itemsetI[arg1.itemsetI.length - 1];
							}
						});
	...
	private void registerRule11(Integer intI, Integer intJ, Set<Integer> tidsI, Set<Integer> tidsJ, Set<Integer> tidsIJ,
			Map<Integer, Occurence> occurencesI, Map<Integer, Occurence> occurencesJ,
			Map<Integer, LeftEquivalenceClass> mapEclassLeft, Map<Integer, RightEquivalenceClass> mapEclassRight) {
	...
	private void calculateTidsetsIJandJI(Map<Integer, Occurence> occurencesI, Map<Integer, Occurence> occurencesJ,
			Set<Integer> tidsIJ, Set<Integer> tidsJI) {
	...
	private void expandLeft(LeftEquivalenceClass eclass) {
		for (int w = 0; w < eclass.rules.size() - 1; w++) { // IMPORTANT : SIZE -1 BECAUSE THE LAST ONE HAS NOTHING LEFT
															// FOR COMPARISON
			...
			for (int m = w + 1; m < eclass.rules.size(); m++) {
				...
				// depending of the relative size of tids of I and tids of {c} we have two ways
				// to calculate it
				...
				// depending of the relative size of tids of IUJ and tids of {c} we have two
			...
			for (int m = w + 1; m < eclass.rules.size(); m++) {
				...
				if (tidsI_JC.size() >= minsuppRelative) {
					...
					if (eclass.itemsetI.length < maxAntecedentSize) {
						...
						store.register(leftRule, itemsetJC, tidsJC, eclass.occurencesI, occurencesJC); // register for
																										// left
																										// expansion
					}
				}
