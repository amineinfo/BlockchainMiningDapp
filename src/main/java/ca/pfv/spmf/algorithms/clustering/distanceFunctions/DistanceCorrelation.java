...
*/
/**
 * This class implements the correlation distance function. It is a subclass of
 ...
 * double and returns a value in [0,1]. A result of 0 means that there is a
 * positive linear relationship. A result of 0.5 means that there is a negative
 * linear relationship. A result of 1 means that there is no linear relationship
 * (but there might be a non linear relationship). Note that this implementation
 * is different from the typical correlation similarity measure which returns a
 * value in [-1,1] where 1 is positive and -1 is a negative relationship. The
 * reason why we use [0,1] instead is that in clustering a distance measure
 * should in general return a value of 0 when two objects are similar and a high
 * value when they are different. <br/>
 * <br/>
 * 
...
public class DistanceCorrelation extends DistanceFunction {
	 ...
	 * correlation distance function calculates the distance between two vectors of
	 * double and returns a value in [0,1]. A result of 0 means that there is a
	 * positive linear relationship. A result of 1 means that there is a negative
	 * linear relationship. A result of 0.5 means that there is no linear
	...
	/**
	 * This method calculate the standard deviation of a list of double. Note that
	 * it divides by n-1 instead of n, assuming that it is the standard deviation of
	 * a sample rather than a population.
	 * 
	 * @param list the list of doubles
