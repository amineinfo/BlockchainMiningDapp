...
public class AlgoClusterReader {
	...
	public List<Cluster> runAlgorithm(String input) throws IOException {
		...
		while ((thisLine = myInput.readLine()) != null) {
			...
			// ===============================
			// We first process the line to remove all occurrences of the '[' character
			...
			// Then, if it is the first instance of the first cluster, we will count how
			// many dimensions there is in an instance.
			// We will also check if the instance has name. Having this information will
			// make it easier
			...
			if (clusters.size() == 0) {
				...
				for (String token : tokens) {
					...
					// if it is the last dimension of the first instance, we don't need to look at
					// the other instance
					// because we have collected the information that we want, that is
					// the number of dimensions to describe each instance and wheter instances have
					// names or not
					if (lastDimension) {
						break;
					}
