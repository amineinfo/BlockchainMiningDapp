 ...
 * and V. Kumar. Workshop on Text Mining, KDD, 2000.<br/>
 * <br/>
 * 
...
public class AlgoBisectingKMeans extends AlgoKMeans {
	 ...
	 * @param iter             the number times a split should be repeated to choose
	 *                         the best one
	 ...
	 */
	public List<ClusterWithMean> runAlgorithm(String inputFile, int k, DistanceFunction distanceFunction, int iter,
	...
	void applyAlgorithm(int k, DistanceFunction distanceFunction, List<DoubleArray> vectors, double minValue,
		...
		while (true) {
			...
			for (int i = 0; i < iter; i++) {
				List<ClusterWithMean> newClusters = applyKMeans(2, distanceFunction, currentVectors, minValue, maxValue,
						vectorsSize);
