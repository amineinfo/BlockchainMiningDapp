...
/**
 * This is an implementation of generic Hierarchical Clustering Algorithm as
 * described in this webpage:
 ...
 * when no cluster can be merged. <br/>
 * <br/>
 * 
...
public class AlgoHierarchicalClustering {
	 ...
	 * @param separator        the character used to separated values in the input
	 *                         file (by default, it is the space).
	 * @return a list of Clusters
	 * @throws IOException exception if error while reading the file
	 */
	public List<ClusterWithMean> runAlgorithm(String inputFile, double maxDistance, DistanceFunction distanceFunction,
	...
	private boolean mergeTheClosestCluster() {
		...
		for (int i = 0; i < clusters.size(); i++) {
			for (int j = i + 1; j < clusters.size(); j++) {
				...
				double distance = distanceFunction.calculateDistance(clusters.get(i).getmean(),
						clusters.get(j).getmean());
