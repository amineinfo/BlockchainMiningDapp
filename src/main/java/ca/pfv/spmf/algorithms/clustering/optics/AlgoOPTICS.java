 ...
 * used a KD-Tree, which also have a log(n) complexity but may perform less well
 * than a R*-tree. The OPTICS algorithm was originally published in: <br/>
 * <br/>
 * 
...
public class AlgoOPTICS {
	...
	/**
	 * Array-list buffer for storing neighbor points to a given point. There is two
	 * buffers because the algorithm need to use two at the same time.
	 */
	 ...
	 * @param seaparator the string that is used to separate double values on each
	 *                   line of the input file (default: single space)
	 ...
	 */
	public List<DoubleArrayOPTICS> computerClusterOrdering(String inputFile, int minPts, double epsilon,
	 ...
	 */
	private void expandClusterOrder(DoubleArrayOPTICS pointDBS, List<DoubleArrayOPTICS> orderedFile, double epsilon,
			int minPts) {
		...
		if (pointDBS.core_distance != Double.POSITIVE_INFINITY) {
			...
			while (orderSeeds.isEmpty() == false) {
				...
				neighboorsBuffer2.clear();
				kdtree.pointsWithinRadiusOfWithDistance(currentObject, epsilon, neighboorsBuffer2); /// &$&$
																									/// CurrentObject
	 ...
	 * @param minPts       the minPts parameter
	 */
	private void update(List<KNNPoint> neighbors, DoubleArrayOPTICS centerObject,
			PriorityQueue<DoubleArrayOPTICS> orderSeeds, int minPts, double epsilon) {
		...
		for (KNNPoint object : neighbors) {
			...
			if (objectOP.visited == false) {
				double newRDistance = Math.max(cDist, distanceFunction.calculateDistance(objectOP, centerObject));
	...
	/**
	 * This method extract cluster from the cluster-ordering using a DBScan based
	 * approach, as proposed in the OPTICS paper. However, as explained in the
	 * OPTICS paper, it is better to use the extractClusters() method to find the
	 * clusters than this method. But I have implemented it.
	 * 
	 * @return a list of clusters (some of them may be empty)
	 */
	...
	public void printStatistics() {
		...
		System.out.println(" Time ExtractClusterOrdering() ~: " + timeExtractClusterOrdering + " ms");
