...
*/
/**
 * An implementation of the DBSCAN algorithm (Ester et al., 1996). Note that
 * original algorithm suggested using a R*-tree to index points to avoid having
 * a O(n^2) complexity, but we instead used a KD-Tree. The DBScan algorithm was
 ...
 * Data Mining (KDD-96). AAAI Press. pp. 226�231.
 * 
...
public class AlgoDBSCAN {
	 ...
	 */
	public List<Cluster> runAlgorithm(String inputFile, int minPts, double epsilon, String separator)
...
//		System.out.println(kdtree.toString());
		...
		for (DoubleArray point : points) {
			...
			if (pointDBS.visited == false) {
				...
				// if it is not noise
				if (bufferNeighboors1.size() >= minPts - 1) { // - 1 because we don't count the point itself in its
																// neighborood
	...
	private void expandCluster(DoubleArrayDBS currentPoint, List<DoubleArray> neighboors, double epsilon, int minPts) {
		...
		// This is more efficient and avoid stack overflow errors for large datasets
		...
		for (int i = 0; i < neighboors.size(); i++) {
			...
			if (newPointDBS.visited == false) {
				...
				if (bufferNeighboors2.size() >= minPts - 1) { // - 1 because we don't count the point itself in its
																// neighborood
	...
	public void printStatistics() {
		...
		System.out.println(" SSE (Sum of Squared Errors) (lower is better) : "
				+ ClustersEvaluation.getSSE(clusters, distanceFunction));
