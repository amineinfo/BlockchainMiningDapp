...
/**
 * Implementation of a sequence (a list of itemsets), which represents a
 * sequential pattern or a sequence of a sequence database, as used by SeqDim
 * and Fournier-Viger (2008) algorithms.
 * 
 * @see AlgoSeqDim
...
public class Sequence {
	...
	/**
	 * Get the time length in terms of time units of this sequential pattern (is
	 * useful if timestamps are used, otherwise it is 0)
	 * 
	 ...
	 * @return Return 1 if this sequence STRICTLY contains sequence Return 2 if this
	 *         sequence is exactly the same as sequence2 Return 0 if this sequence
	 *         does not contains sequence2.
	 */
	...
	/**
	 * Helper method for checking if this sequence is contained in another given
	 * sequence
	 * 
	 * @param sequence2          the given sequence
	 * @param index              itemset position indicating where the comparison
	 *                           should start for this sequence
	 * @param index2             itemset position indicating where the comparison
	 *                           should start for the given sequence
	 ...
	 *         does not contains sequence2.
	 */
	private int strictlyContainsHelper(Sequence sequence2, int index, int index2, long previousTimeStamp,
			long previousTimeStamp2) {
		if (index == size()) {
		...
		for (int i = index; i < size(); i++) {
			...
			if (get(i).getItems().containsAll(sequence2.get(index2).getItems()) && interval1 == interval2) {
				...
				if (sequence2.size() - 1 == index2) {
					...
					int resultat = strictlyContainsHelper(sequence2, i + 1, index2 + 1, get(i).getTimestamp(),
							sequence2.get(index2).getTimestamp());
	 ...
	 * 
	 * @param mapSequenceID  a map indicating for each item (key) the IDs of
	 *                       sequences containing the item (value).
