...
public class Sequence {
	...
	/**
	 * Number of items that appear in the sequence, i.e. the addition of the itemset
	 * sizes.
	 */
	...
	/**
	 * Adds an itemset in the sequence. The itemset is inserte at the end of the
	 * sequence
	 * 
	...
	/**
	 * Get the time length of the sequence. It is equal to the last itemset
	 * timestamp minus the first itemset timestamp
	 * 
	...
	/**
	 * It returns a pair <itemset index, item index> indicanting the first
	 * appearance of an item in the sequence. The starting point where the method
	 * takes the search up is given as parameter.
	 * 
	...
	public int[] searchForTheFirstAppearance(int itemsetIndex, int itemIndex, Item item) {
		...
		if (itemsetIndex < size()) {
			...
			for (int i = itemsetIndex; i < itemsets.size(); i++) {
				...
				// the beginning index is 0 or itemIndex if we are in the itemset referred by
				// itemsetIndex
				...
				// If the index returned by the search method is positive and equal or greater
				// than our beginning item index
				if (pos >= beginning) {
					// We return the pair <i,pos>, where i is the current itemset index and pos the
					// current item index in that itemset
	 ...
	 */
	/*
	 * public int[] searchForAnItemInLaterItemset(Item item, int itemsetIndex, int
	 * ItemIndex) { if (itemsetIndex < size()) { int firstItemset = itemsetIndex;
	 * int firstItem = ItemIndex; for (int i = firstItemset; i < itemsets.size();
	 * i++) { Itemset currentItemset = itemsets.get(i); int beginning = (i ==
	 * firstItemset) ? (firstItem) : 0;
	 * 
	 * //int pos = currentItemset.binarySearch(item); //uncomment if you'd rather
	 * run a lineal search int pos = currentItemset.linealSearch(item);
	 * 
	 * if (pos >= beginning) { return new int[]{i,pos}; } } } return null; }
	 */

	/**
	 * It returns a pair <itemset index, item index> indicanting the appearance of
	 * an item in the itemset given as parameter. The starting point where the
	 * method takes the search up is given as parameter
	 * 
	 * @param item         Item to find
	 * @param itemsetIndex Index of the first itemset where we will start to search
	 * @param itemIndex    Index of the first item where we will start to search
	...
	public int[] SearchForItemAtTheSameItemset(Item item, int itemsetIndex, int itemIndex) {
		...
		if (itemsetIndex < size()) {
			...
			// than our beginning item index
			if (pos >= itemIndex) {
				// We return the pair <itemsetIndex,pos>, where itemsetIndex is the current
				// itemset index and pos the current item index in that itemset
	...
	/**
	 * It returns a pair <itemset index, item index> indicanting the first
	 * appearance of an item in the sequence that appears in a itemset that has a
	 * temporal distance with respect to the current itemset, as it the given
	 * parameter. The starting point where the method takes the search up is given
	 * as parameter.
	 * 
	 * @param item             Item to find
	 * @param itemsetIndex     Index of the first itemset where we will start to
	 *                         search
	 * @param itemIndex        Index of the first item where we will start to search
	 * @param temporalDistance Temporal distance at which the item to find should be
	 * @return the pair position if the item is found, otherwise null value
	 */
	public int[] searchForItemInAConcreteTemporalDistance(Item item, int itemsetIndex, int itemIndex,
		...
		if (itemsetIndex < size()) {
			...
			// while we still have itemsets in the sequence and they have a timestamp less
			// than that in which we are interested, we skip them
			while (itemset < itemsets.size() && itemsets.get(itemset).getTimestamp() < objectiveTimestamp) {
				itemset++;
			}
			// When the loop is over, if the itemset timestamp is the same as that we are
			// expecting
			if (itemset < itemsets.size() && itemsets.get(itemset).getTimestamp() == objectiveTimestamp) {
				...
				// than our beginning item index
				if (pos >= 0) {
					// We return the pair <itemset,pos>, where itemset is the current itemset index
					// and pos the current item index in that itemset
	...
	/**
	 * It gives the number of items that appears after a concrete position of the
	 * sequence
	 * 
