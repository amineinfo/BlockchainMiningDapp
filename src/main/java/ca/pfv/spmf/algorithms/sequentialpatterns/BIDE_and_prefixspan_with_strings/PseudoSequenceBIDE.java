...
/**
 * This represents a sequence from a projected database (as used by the BIDE+
 * algorithm). Since it is a projected sequence, it makes reference to the
 * original Sequence. <br/>
 * <br/>
 * 
 * This class is a subclass of "PseudoSequence" used by the PrefixSpan
 * algorithm. This class also include several methods for calculating the
 * maximum periods, semi-maximum periods, etc. as required by the BIDE+
 * algorithm. These methods are quite complex so if you want to understand them,
 * it is recommended to read the BIDE+ paper carefully before reading the code.
 * 
...
class PseudoSequenceBIDE extends PseudoSequence {
	...
	int lastItemset; // the position where the cut was done in terms of itemset in the original
						// sequence
	 ...
	 * @param indexItemset the position of the itemset where the pseudo sequence
	 *                     starts (if it is cut at left)
	 * @param indexItem    the position of the item where the pseudo sequence starts
	 *                     (if it is cut at left)
	 */
	 ...
	 * @param sequence     the original pseudo-sequence
	 * @param indexItemset the position of the itemset where the pseudo sequence
	 *                     starts (!=0 if it is cut at left)
	 * @param indexItem    the position of the item where the pseudo sequence starts
	 *                     (!=0 if it is cut at left)
	 * @param lastItemset  the position of the itemset where this pseudo sequence
	 *                     ends
	 * @param lastItem     the position of the item where this pseudo sequence ends
	 */
	protected PseudoSequenceBIDE(PseudoSequenceBIDE sequence, int indexItemset, int indexItem, int lastItemset,
			int lastItem) {
		// record the original sequence
		this.sequence = sequence.sequence;
		// record the position where the sequence starts if it is cut at left
	...
	/******************************************
	 * All the following methods are specific for BIDE+. I provide a brief
	...
	protected List<List<Position>> getAllInstancesOfPrefixHelper(List<Itemset> prefix, int indexItemset,
			List<List<Position>> allInstances, List<Position> listPositionsTotal, int decalageItemset) {
		...
		for (int i = decalageItemset; i < size(); i++) {
			...
			for (int j = 0; j < getSizeOfItemsetAt(i); j++) {
				...
				if (id.equals(iDCourant)) {// l'item match
					...
					if (listPositions.size() + listPositionsTotal.size() == getItemOccurencesTotalCount(prefix)) // si
																													// on
																													// a
																													// trouvé
																													// tout
	...
	protected Position getIthLastInLastApearanceWithRespectToPrefix(List<Itemset> prefix, int i) {
		...
		PseudoSequencePair lastInstancePair = getLastInstanceOfPrefixSequence(prefix,
				getItemOccurencesTotalCount(prefix));
...
//			pseudo.toString();
	...
	/**
	 * Return the sum of the size of all itemsets of this sequence. Note: used by
	 * the BIDE algorithm
	 */
