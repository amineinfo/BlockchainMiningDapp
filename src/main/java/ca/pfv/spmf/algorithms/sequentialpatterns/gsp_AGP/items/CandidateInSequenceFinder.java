...
public class CandidateInSequenceFinder {
	...
	/**
	 * Recursive method to search for a candidate in a sequence. If the candidate
	 * appears in the sequence, set the flag present to true
	 *
	 ...
	 */
	public void isCandidatePresentInTheSequence_qualitative(Pattern candidate, Sequence sequence, int k, int length,
			List<int[]> position) {
		...
		Abstraction_Generic previousAbstraction = length > 0 ? candidate.getIthElement(length - 1).getAbstraction()
				: null;
		// flag to know if we should cancel the search
		boolean cancelled = false;
		// Initialization of the position of the current item, itemPair, to search for
		int[] pos = null;
		// while the itemset index that correspond to the current item, itempair, is
		// withing the sequence
		while (position.get(length)[0] < sequence.size()) {
			/*
			 * If we are dealing with the first element of the candidate, we search for the
			 * item from the beginning of the sequence
			 */
			if (length == 0) {
				pos = sequence.searchForTheFirstAppearance(position.get(length)[0], position.get(length)[1], itemPair);
			} else {
				/*
				 * Otherwise, we find that item depending on the temporal relation between the
				 * current abstraction and the previous one
				 */
				pos = creator.findPositionOfItemInSequence(sequence, itemPair, abstractionPair, previousAbstraction,
						position.get(length)[0], position.get(length)[1], position.get(length - 1)[0],
						position.get(length - 1)[1]);
			}
			...
			if (pos != null) {
				...
				if (length + 1 < k) {
					...
					// And we make a recursive call in order to deal with the next element of the
					// candidate
					...
					/*
					 * If we are in the last element of the candidate, we have checked that the
					 * candidate it appears in the sequence, and we set the corresponding flat to
					 * true
					 */
					present = true;
					return;
				...
				/*
				 * We set the flag of cancel to true, in order to end this execution and come
				 * back to the previous call, that to referred to the previous element of the
				 * candidate
				 */
				cancelled = true;
		...
		/*
		 * if we exceeded the sequence limits we have to try modifying the previous
		 * element position in order to test if we can find any combination that
		 * contains the whole candidate
		 */
	...
	/**
	 * We move to the next item in the same itemset, returning <
	 * currentItemsetIndex, currentItemIndex + 1 >. If the itemset is exceeded, we
	 * change to the next itemset, establishing the item index in 0 and so, in this
	 * case, our returning value is < currentItemsetIndex +1 , 0 >.
	 * 
	...
	/**
	 * It returns a new pair pointing out the beginning of the next itemset to the
