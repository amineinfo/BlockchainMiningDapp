...
public class PseudoSequence {
	...
	/**
	 * List with the corresponding timeshift that each projection in the sequence
	 * has
	 */
	...
	/**
	 * It make a pseudosequence from a previous pseudosequence. Therefore, we are
	 * making a projection in a previous projection and we have to take into account
	 * the previous index of that projection to establish the ours.
	 * 
	 ...
	 *                       previous pseudoseuqence we are making the new
	 *                       pseudosequence
	 */
	public PseudoSequence(long timeShift, PseudoSequence pseudosequence, int itemsetIndex, int itemIndex,
			int firstItemset) {
		// We point to the same original sequence
		this.sequence = pseudosequence.sequence;
		/*
		 * We define the new time shift, that is the timeshift given plus the value that
		 * the given pseudosequence has. Then we add it to the list in its corresponding
		 * position
		 */
		long newTimeShift = timeShift + pseudosequence.timeShift.get(firstItemset);
		this.timeShift.add(newTimeShift);
		/*
		 * We establish the first itemset of the projection in absolute terms. We have
		 * to add the given itemset index to the first itemset index kept in the given
		 * pseudosequence
		 */
	...
	/**
	 * Method to make a projection in a pseudosequence already projected. In this
	 * way, for a same pseudosequence, we can have several projections. For example,
	 * let us think in a sequence A = (a) (a b) (a b c). If we project by item a, we
	 * obtain three different projections: 1) (a b) (a b c) 2) (*b) (a b c) 3) (*b
	 * c) That we do is to put all the three projections in the same pseudoseuqence,
	 * and we refers each projection with an index. We call to every projection,
	 * projection point.
	 * 
	 * @param firstItemset   The projection point of the given pseudosequence where
	 *                       we are making our new projection
	 ...
	 */
	public void addProjectionPoint(int firstItemset, long timeShift, PseudoSequence pseudosequence, int itemsetIndex,
			int itemIndex) {
		/*
		 * We update the timeshift from the given pseudosequence time shift and, them we
		 * keep it
		 */
		long newTimeShift = timeShift + pseudosequence.timeShift.get(firstItemset);
		this.timeShift.add(newTimeShift);
	...
	public int length(int firstItemset) {
		...
		/*
		 * We count all the elements that appear before the first element from which we
		 * start the projected sequence
		 */
		...
		/*
		 * And we return the difference between the original sequence length and this
		 * counted value
		 */
	 ...
	 * @param itemsetIndex the itemset of the pseudosequence in which we are
	 *                     interested
	 * @return the item
	 */
	...
	/**
	 * Get the index where an item appears in the specified itemset of the specified
	 * projection of the current pseudosequence. If the item does not appear, it
	 * returns -1.
	 * 
