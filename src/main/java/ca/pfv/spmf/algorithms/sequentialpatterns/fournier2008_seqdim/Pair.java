...
/**
 * This class is used by the Fournier08 algorithm and represents a pair of an
 * (1) Item and (2) a time interval. It is used for calculating the support of
 * items in a database. A pair also contains some other information too that are
 * needed by BIDE/PrefixSpan such as if the item was in an itemset that is a
 * prefix (cut at right) or if the item was in an itemset that is a postfix (cut
 * a left).
 * 
 * @see AlgoBIDEPlus
...
class Pair {
	...
	public boolean equals(Object object) {
		...
		if ((paire.timestamp == this.timestamp) && (paire.postfix == this.postfix) && (paire.prefix == this.prefix)
