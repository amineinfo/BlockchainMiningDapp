	...
	private int recursionMultipleItems(List<PseudoSequence> database, int k, int lastBufferPosition)
		...
		for (Entry<Pair, Pair> entry : mapsPairs.mapPairsInPostfix.entrySet()) {
			...
			if (pair.getCount() >= minsuppAbsolute) {
				...
				if (lastBufferPosition == 1 && patternBuffer[0] == 1 && patternBuffer[1] == 3) {
					System.out.print("");
				}
...
//							j--;  Don't need it because the for loop will decrement j
				...
				recursiveExtensionChecking(sequence, posItemsetFirst, firstBufferPositioninEIm1Itemset, posIminus1, i,
						endOfLastItemset, eim1WasSeen, sequences.size(), endOfLastBufferItemset);
...
//			}
			...
			for (int k = 0; k < sequences.size(); k++) {
				...
				if (lastBufferPosition == 1 && patternBuffer[0] == 4 && patternBuffer[1] == 5) {
					System.out.println("pos = " + i);
...
//			}
	 ...
	 *         Otherwise, false.
	 */
	private boolean recursiveExtensionChecking(int[] sequence, int posItemsetSequenceToMatch,
		...
		if (posNextItemset <= endOfLastItemset) {
			// Recursive call
			recursiveExtensionChecking(sequence, posNextItemset, posItemsetBufferToMatch, posIminus1, i,
				...
				if (posItemsetBufferToMatch == lastBufferPosition || patternBuffer[nextBufferPosition] == -1) {
					...
					if (posItemsetBufferToMatch == lastBufferPosition) {
						...
						if (nextJ < endOfLastItemset) {
							...
							// recursive call
							boolean fullyMatched = recursiveExtensionChecking(sequence, nextJ, nextBufferPos,
