...
/**
 * Implementation of the SeqDim algorithm for multi-dimensional sequential
 * pattern mining proposed by Pinto et al (2001). <br/>
 * <br/>
 * SeqDIM is a generic algorithm that can be used in theory with any combination
 * of sequential pattern mining algorithm and MDPattern mining algorithm.<br/>
 ...
 * In SPMF, the MD-Pattern mining algorithm is AlgoDim and it offers to choose
 * between Charm and AprioriClose (see the AlgoDim class for details).
 * 
...
public class AlgoSeqDim {
	...
	/**
	 * if true the sequence identifiers of each patterns will be shown in the output
	 * file
	 */
	 ...
	 * @param algoPrefixSpan     a prefixpsan based algorithm (BIDE, PrefixSpan or
	 *                           Fournier08)
	 * @param algoDim            an instance of the DIM algorithm
	 * @param mineClosedPatterns if true, only closed mdsequential pattern will be
	 *                           returned
	 ...
	 */
	public MDSequences runAlgorithm(MDSequenceDatabase database, AbstractAlgoPrefixSpan algoPrefixSpan, AlgoDim algoDim,
		...
		for (int j = 0; j < sequencesFound.getLevelCount(); j++) {
			...
			for (Sequence sequence : sequencesList) {
				...
				trySequence(sequence, database, algoPrefixSpan.getMinSupp(), algoDim);
			}
		}
	 ...
	 */
	private void trySequence(Sequence sequence, MDSequenceDatabase database, double minsupp, AlgoDim algoDim)
			throws IOException {
		// (a) Create a projected database by using only
		// the sequence containing the given sequential pattern.
		MDPatternsDatabase newContexte = createProjectedDatabase(sequence.getSequencesID(),
	...
	/**
	 * Create a projected MD-pattern. database by keeping only the MDPatterns
	 * corresponding to a set of sequence IDs.
	 * 
...
//		sequences.printFrequentSequences(objectsCount);
	...
	/**
	 * Eliminate non-closed multidimensional sequential patterns by simply looping
	 * and eliminating redundant patterns. This is necessary if we want to mine
	 * closed multi-dim. seq. patterns, because: closed sequential patt. mining +
	 * closed itemset mining != closed multi-dim seq. patt. mining. For more details
	 * about this, see the paper published by Panida Songram, Veera Boonjing and
	 * Sarun Intakosum (2006) that explains why we can do that.
	 * 
