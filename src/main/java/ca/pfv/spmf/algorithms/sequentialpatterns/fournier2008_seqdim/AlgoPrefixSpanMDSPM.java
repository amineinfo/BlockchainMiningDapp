 ...
 * using optimizations such as pseudo-projections, designed specifically for
 * being used with the multi-dimensional sequential pattern mining algorithm
 * SeqDim. There is another implementation of PrefixSpan for general use that is
 * more optimized and can be found in the package:
...
public class AlgoPrefixSpanMDSPM extends AbstractAlgoPrefixSpan {
	 ...
	 * 
	 * @param database a sequence database
	 * @return Map associating to each item (key) the set of IDs of sequences
	 *         (value) containing the item.
	 */
	private Map<ItemSimple, Set<Integer>> calculateSupportOfItems(SequenceDatabase database) {
		// Use a set to remember which item were seen already from a sequence when
		// scanning
		// a sequence
		Set<Integer> alreadyCounted = new HashSet<Integer>();
		// the last sequence scanned
		Sequence lastSequence = null;
		// the map for storing association from each item (key) to the set of IDs of
		// sequences (value)
		// containing the item.
	 ...
	 */
	private PseudoSequenceDatabase buildProjectedContext(ItemSimple item, PseudoSequenceDatabase database,
			boolean inSuffix) {
	...
	private void recursion(Sequence prefix, int k, PseudoSequenceDatabase database) {
		...
		for (Pair paire : pairs) {
			...
			if (paire.getCount() >= minsuppRelative) {
				...
				PseudoSequenceDatabase projectedContext = buildProjectedContext(paire.getItem(), database,
						paire.isPostfix());
	...
	/**
	 * Method to count the support of all pairs in a pseudo-sequence database. A
	 * pair is an item ID and a boolean indicating if the item appear in a postfix
	 * or not. This is for k> 1.
	 * 
	 ...
	 * 
	 * @param databaseSize the number of sequences in the original sequence
	 *                     database.
	 */
