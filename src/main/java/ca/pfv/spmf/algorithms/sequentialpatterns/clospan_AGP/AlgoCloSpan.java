...
public class AlgoCloSpan {
	...
	/**
	 * Standard constructor. It takes the relative minimum support threshold (from 1
	 * up to 0) and an abstraction creator
	 *
	 ...
	 */
	public AlgoCloSpan(double minSupRelative, AbstractionCreator creator, boolean findClosedPatterns,
	 ...
	 * CloSpan. In particular, the original database is fully converted to a
	 ...
	 *                                  of them
	 * @param verbose                   Flag for debugging purposes
	 * @param findClosedPatterns        flag to indicate if we are interesting in
	 *                                  only finding the closed sequences
	 * @param outputFilePath            Path pointing out to the file where the
	 *                                  output, composed of frequent patterns, has
	 *                                  to be kept. If, conversely, this parameter
	 *                                  is null, we understand that the user wants
	 ...
	 * @throws IOException
	 */
	protected void cloSpan(SequenceDatabase database, boolean keepPatterns, boolean verbose, boolean findClosedPatterns,
			boolean executePruningMethods, String outputFilePath, boolean outputSequenceIdentifiers)
			throws IOException {
		...
		/*
		 * We get the map which relates the frequent items with their appearances in the
		 * database
		 */
		...
		// We initialize the class that is in charge of managing the main loop of
		// PrefixSpan
		RecursionCloSpan algorithm = new RecursionCloSpan(abstractionCreator, saver, (int) minSupAbsolute,
				pseudoDatabase, mapSequenceID, findClosedPatterns, executePruningMethods);
		...
		if (verbose) {
			System.out.println("CLOSPAN: The algorithm takes " + timeForMainMethod + " seconds and finds "
					+ numberOfFrequentPatterns + " patterns");
		}
		// If the we are interested in closed patterns, we execute the post-processing
		// step
		if (findClosedPatterns) {
			...
			numberOfFrequentPatterns = algorithm.numberOfFrequentPatterns();
			if (verbose) {
				System.out.println("CLOSPAN: The post-processing algorithm to remove the non-Closed patterns takes "
						+ timeForPostProcessingStep + " seconds and finds " + numberOfFrequentPatterns
						+ " Closed patterns");
			}
		} else {
			if (keepPatterns) {
	...
	private PseudoSequenceDatabase projectInitialDatabase(SequenceDatabase database, Map<Item, BitSet> mapSequenceID,
		...
		for (Sequence sequence : database.getSequences()) {
			...
			if (optimizedSequence.size() != 0) {
				/*
				 * If after remove the infrequent items, we remove all the items of an original
				 * sequence, we insert an empty pseudosequence in order not to affect to the
