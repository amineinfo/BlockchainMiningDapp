...
/**
 * This class, used by the BIDE algorithm, represents an (1) item (2) a boolean
 * indicating if the item is contained in an itemset that was cut at left or not
 * (a postfix). (3) a boolean indicating if the item is contained in an itemset
 * that was cut at right or not (a prefix). and (4) the sequence IDs containing
 * this item. <br/>
 * <br/>
 * This class extends the class Pair used by PrefixSpan. It is is used by BIDE+
 * to count the support of items.
 * 
 * @see Pair
...
class PairBIDE extends Pair {
	...
	/**
	 * indicate if this represents the item appearing // in an itemset that is cut
	 * at the right or not (a prefix)
	 */
	 ...
	 * @param prefix  indicate if this is the case of an item appearing in an
	 *                itemset that is cut at the right because of a projection
	 * @param item    the item
	 */
	...
	public boolean equals(Object object) {
		...
		if ((paire.postfix == this.postfix) && (paire.prefix == this.prefix) && (paire.item.equals(this.item))) {
			return true;
		}
		return false;
	}
