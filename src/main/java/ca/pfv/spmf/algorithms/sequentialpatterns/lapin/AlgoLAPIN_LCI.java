...
//				// Initialize the IE lists and SE lists for that sequence
		...
		for (int i = 0; i < frequentItems.size(); i++) {
			...
			if (item1Border.size() >= minsup) {
				// Create an object prefix to represent the sequential pattern containing the
				// item
				...
				genPatterns(prefix, item1Border, frequentItems, frequentItems, item1, true); // true, to disallow
																								// I-extension because
																								// we explore 2-IE
...
//				Integer lastItem = lastItemset.get(lastItemset.size()-1);
	...
	private int estimateSupportIStep(Integer item, List<Position> itemBorder) {
		...
		for (Position pos : itemBorder) {
			...
			// Scan from last position to first position (they are ordered backward in the
			// table)
	...
	/**
	 * A inner class to store a position (a sequence id + an itemset id). This will
	 * be use to represent the border of a prefix.
	 */
	class Position {
		/** a sequence id */
		int sid;
