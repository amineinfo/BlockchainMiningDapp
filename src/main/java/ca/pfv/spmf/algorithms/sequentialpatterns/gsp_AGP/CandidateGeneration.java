...
/**
 * This is an implementation of the candidate generation addressed in GSP
 * algorithm. This class is one of the two method continuously repeated by means
 * of the GSP's main loop. Here, from a set of frequent candidates k-sequences
 * we generate a set of possible (k+1)-supersequences.
 *
...
class CandidateGeneration {
	...
	/**
	 * Main method that creates, from frequent (k-1)-sequence set (aka L(k-1)) the
	 * new set of (k)-sequences candidates. Before returning the candidate set, the
	 * algorithm prunes those canidates that cannot be frequent at all
	 * 
	 ...
	 */
	public List<Pattern> generateCandidates(Set<Pattern> frequentSet, AbstractionCreator abstractionCreator,
			Map<Item, Set<Pattern>> indexationMap, int k, double minSupportAbsolute) {
		...
		if (k > 2) { // If we are not in the base case, i.e. we are generating a level k>2
			...
			for (Pattern frequentPattern1 : frequentList) {
				...
				if (matching != null) {
					...
					for (Pattern frequentPattern2 : matching) {
						...
						Pattern candidate = abstractionCreator.generateCandidates(abstractionCreator, frequentPattern1,
								frequentPattern2, minSupportAbsolute);
			...
			for (int i = 0; i < frequentList.size(); i++) {
				for (int j = i; j < frequentList.size(); j++) {
					// We create candidates with all the possible combinations of frequent
					// 1-sequences
					prunedCandidates.addAll(abstractionCreator.generateSize2Candidates(abstractionCreator,
							frequentList.get(i), frequentList.get(j)));
				}
			}
		}
	 ...
	 * @return
	 */
	private List<Pattern> prunedSubset(List<Pattern> candidateSet, Set<Pattern> frequentSet,
		...
		for (Pattern candidate : candidateSet) {
			...
			for (int i = 0; i < candidate.getElements().size() && !isInfrequent; i++) {
				...
				// and if this subpattern does not appear in the frequent (k-1)-sequence set,
				// L(k-1)
			...
			if (!isInfrequent) {
				// We only add in the output set those patterns that not have any infrequent
				// subpattern
