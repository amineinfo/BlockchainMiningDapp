...
public class AbstractionCreator_Qualitative implements AbstractionCreator {
			...
			for (int i = 0; i < itemsets.size(); i++) {
				...
				for (int j = 0; j < currentItemset.size(); j++) {
					...
					for (int k = j + 1; k < currentItemset.size(); k++) {
						...
						ItemAbstractionPair pair2 = new ItemAbstractionPair(item2,
								Abstraction_Qualitative.create(true));
						updateAppeareanceSet(totalMap, pair1, pair2, s.getId());
					}
					// And with all the items that are in later itemsets we make 2-sequence
					// extensions
					for (int k = i + 1; k < itemsets.size(); k++) {
						...
						for (int n = 0; n < nextItemset.size(); n++) {
							...
							ItemAbstractionPair pair2 = new ItemAbstractionPair(item2,
									Abstraction_Qualitative.create(false));
							updateAppeareanceSet(totalMap, pair1, pair2, s.getId());
						}
					}
				}
	...
	public void updateAppeareanceSet(Map<Pattern, Pattern> totalMap, ItemAbstractionPair pair1,
			ItemAbstractionPair pair2, int seqId) {
	...
	/**
	 * It returns the subpattern obtained by removing an item at a given index in a
	 * pattern.
	 *
	...
	public Pattern getSubpattern(Pattern extension, int index) {
		...
		for (int i = 0; i < extension.size(); i++) {
			...
			if (i != index) {
				...
				if (i == nextIndex) {
					// If we do no have any value in "abstraction" it means that it was not prepared
					// before. In that case we take the abstraction appearing in the ith-element
					if (abstraction == null) {
						abstraction = extension.getIthElement(i).getAbstraction();
					}
					// Otherwise, we use the abstraction that we had previously prepared
					subpatternElements
							.add(pairCreator.getItemAbstractionPair(extension.getIthElement(i).getItem(), abstraction));
				} else {
					subpatternElements.add(extension.getIthElement(i));
				}
				// If we remove it, we check how are the temporal relations when we do it
			} else {
				// If it is the first element in the pattern, we establish the default
				// abstraction for the next element
				if (index == 0) {
					abstraction = CreateDefaultAbstraction();
				} else {
					// If it is not the first element, we check if it appears in a itemset later
					// than its predecessor
					Abstraction_Qualitative abstractionOfRemovedElement = (Abstraction_Qualitative) extension
							.getIthElement(i).getAbstraction();
	...
	@Override
	public List<Pattern> createSize2Sequences(Map<Integer, Map<Item, List<Integer>>> bbdd,
			Map<Item, Pattern> frequentItems) {
		Map<Pattern, Pattern> totalMap = new HashMap<Pattern, Pattern>();
		List<Pattern> output = new LinkedList<Pattern>();
	...
	/**
	 * Method that finds the position that an item has within a concrete sequence.
	 * The position is returned in a duple <itemset index, item index>. The method
	 * used to find the item it depends on how the relations, between the current
	 * item and the previous one, are
	 *
	 ...
	 * @param previousItemsetIndex Itemset index of the previous item in the pattern
	 * @param previousItemIndex    Item index of the previous item in the pattern
	 * @return The position where the item is
	 */
	public int[] findPositionOfItemInSequence(Sequence sequence, Item itemPair, Abstraction_Generic absPair,
			Abstraction_Generic previousAbs, int itemsetIndex, int itemIndex, int previousItemsetIndex,
		...
		if (abs.hasEqualRelation()) {
			...
			// If the current itemset index and the previous one point out to the same
			// position
			if (itemsetIndex == previousItemsetIndex) {
				// We make the current itemset index to point to the next itemset, starting from
				// the first item index
				itemsetIndexToSearchFor++;
				itemIndexToSearchFor = 0;
			}
			// And, finally, we look for the item in all the itemsets that appear later than
			// the original itemset index
			// pos = sequence.searchForAnItemInLaterItemset(itemPair,
			// itemsetIndexToSearchFor, itemIndexToSearchFor);
	 ...
	 * elements of pattern1, except the first one, are the same as all the elements
	 * of pattern2, except the last one. For example, if pattern1= a < (b c) < d,
	 * and pattern2=(b c) < d < e, we are interested in checking if the subpattern
	 * of pattern1, obtaining by suppressing the first item, is equal to the
	 * subpattern of pattern2 that is obtained by removing its last item. I.e., p1 =
	 * a < (b c) < d p2 = (b c) < d < e and we want two know if the central part of
	 * both pattern is the same. If is not, we set different to true.
	 *
	 *
	 * 
	 * @param creator
	 ...
	 */
	public Pattern generateCandidates(AbstractionCreator creator, Pattern pattern1, Pattern pattern2,
			double minSupport) {
		...
		if (different) {// If we cannot compose any candidate we return null
			return null;
				 ...
				 * otherwise, we first check if the junction of their appearing sets still has
				 * more appearances than the minimum relative support is
				 */
	...
	/**
	 * Method to find a candidate in a sequence. It is an intermedium call in order
	 * to separate the implementation and to be able to define different ways of
	 * searching.
	 * 
	 ...
	 */
	public void isCandidateInSequence(CandidateInSequenceFinder finder, Pattern candidate, Sequence sequence, int k,
			int i, List<int[]> position) {
	...
	public List<Pattern> generateSize2Candidates(AbstractionCreator creator, Pattern pat1, Pattern pat2) {
		...
		elementsOfNewPattern1.add(pairCreator.getItemAbstractionPair(elementFromPattern2.getItem(),
				Abstraction_Qualitative.create(false)));
		...
		if (!elementFromPattern1.equals(elementFromPattern2)) {
			...
			elementsOfNewPattern2.add(pairCreator.getItemAbstractionPair(elementFromPattern1.getItem(),
					Abstraction_Qualitative.create(false)));
			...
			elementsOfNewPattern3.add(
					pairCreator.getItemAbstractionPair(greaterPair.getItem(), Abstraction_Qualitative.create(true)));
