...
/**
 * This class represents a sequence from a projected database (as based in
 * PrefixSpan). Since it is a projected sequence, it makes reference to the
...
//  this represents how much time unit have been removed when the sequence is cut at left
	...
	/**
	 * Constructor to build a new pseudo-sequence cut at left based on a
	 * pseudo-sequence.
	 * 
	 * @param timeShift    how much time unit have been removed when the
	 *                     pseudo-sequence is cut at left
	...
	/**
	 * Constructor to build a new pseudo-sequence based on a pseudo-sequence, and
	 * cut at left and right.
	 * 
	 * @param timeShift    how much time unit have been removed when the
	 *                     pseudo-sequence is cut at left
	 * @param sequence     the original pseudo-sequence
	 ...
	 */
	public PseudoSequence(long timeShift, PseudoSequence sequence, int indexItemset, int indexItem, int lastItemset,
			int lastItem) {
		// save the parameters
		this.timeShift = timeShift;
		this.sequence = sequence.sequence;
	 ...
	 * 
	 * @param timeShift    the number of time units removed from the original
	 *                     sequence
	 * @param sequence     the original sequence
	 * @param indexItemset where the pseudo sequence starts in terms of itemset
	 *                     index
	 * @param indexItem    where the pseudo sequence starts in terms of item index
	 *                     in the itemset where the pseudosequence starts.
	 */
	public PseudoSequence(long timeShift, Sequence sequence, int indexItemset, int indexItem) {
		...
		this.firstItem = indexItem;
		// Last itemset and item (by default, this is the last item & itemset of the
		// sequence.
	...
	/**
	 * Check if the itemset at position "index" is the first itemset of the
	 * pseudoseq.
	 * 
	...
	/**
	 * Check if the itemset at position "index" is the last itemset of the
	 * pseudoseq.
	 * 
	 * @param index the position "index"
	 * @return true if yes, otherwise false.
	...
	/**
	 * Get the timestamp associated to a given itemset without considering the time
	 * shift.
	 * 
	 * @param indexItemset the index of the itemset.
	 * @return the timestamp
	...
	/******************************************
	 * All the following methods are specific to BIDE+. I provide a brief
	 * description of them. Please refer to the original BIDE+ paper for more
	 * details about the BIDE+ algorithm and how it works because it is quite
	 * complex.
	 * 
	 * Remember that a sequential pattern is not closed if there exists a
	 * forward-extension or a backward-extension.
	 ******************************************/

	/**
	 * Structure to contains a sequence and list of positions to elements in the
	 * sequence.
	 */
	...
	/**
	 * Internal class representing a position in a pseudo-sequence. (position of an
	 * itemset + position of an item).
	 */
	...
	/**
	 * Method that find all instances of a prefix in a sequence S. The meaning of
	 * instance is the one from the BIDE+ article when they tak about "first
	 * instance", "last instance". Here instead of finding only the "first instance"
	 * or "last instance", we find all instances and they also respect the
	 * timestamps!
	 */
	List<PseudoSequencePair> getAllInstancesOfPrefix(Sequence prefix, int i) {
		List<List<Position>> listInstances = getAllInstancesOfPrefixHelper(prefix, 0, new ArrayList<List<Position>>(),
				new ArrayList<Position>(), 0, 0);
	...
	List<List<Position>> getAllInstancesOfPrefixHelper(Sequence prefix, int indexItemset,
			List<List<Position>> allInstances, List<Position> listPositionsTotal, long itemsetShift,
			int decalageItemset) {

		for (int i = decalageItemset; i < size(); i++) {
			boolean firstTime = indexItemset == 0;
			if (!firstTime && getTimeStamp(i) - itemsetShift != prefix.get(indexItemset).getTimestamp()) { // VÉRIFIER
																											// DÉCALAGE
																											// TEMPS
				continue;
			}
			...
			for (int j = 0; j < getSizeOfItemsetAt(i); j++) {
				...
				if (id == iDCourant) {// l'item match
					...
					if (listPositions.size() + listPositionsTotal.size() == prefix.getItemOccurencesTotalCount()) // si
																													// on
																													// a
																													// trouvé
																													// tout
																													...
																													// préfix
					{
						...
						if (indexItemset + 1 < prefix.size()) {
							getAllInstancesOfPrefixHelper(prefix, indexItemset + 1, allInstances, newList, decalage,
									i + 1);
						}
					} else {
						indexItem++;
	...
	/**
	 * Last instance of a prefix sequence X in a sequence S. For example, the last
	 * instance of AB in ABBCA is ABB. Additionnal difficulty : this sequence must
	 * respect timestamps!
	 */
	PseudoSequencePair getLastInstanceOfPrefixSequence(Sequence prefix, int i) {
		...
		for (PseudoSequencePair sequencePair : list) {
			...
			if ((sequence.lastItemset > sequenceRetour.lastItemset)
					|| (sequenceRetour.lastItemset == sequence.lastItemset
							&& sequence.lastItem > sequenceRetour.lastItem)) {
	...
	/**
	 * First Instance of the prefix X in a sequence S. Method to find the first
	 * Instance of the prefix sequence X= e1, e2... ei+1 in a sequence S. Exemple:
	 * first instance of AB in the sequence CAABC = CAAB. Additionnal difficulty :
	 * this sequence must respect timestamps!
	 */
	...
	/**
	 * Get the ith last-in-last appearance with respect to a prefix sequence Sp. n =
	 * size of S If i == n, it is the last appearance of ei in the last instance of
	 * Sp. If 0 <= i < n, it is the last appearance of ei in the last instance of
	 * Sp, and LLi must appear before LLi+1. Example : If S= CAABC and SP = AB then
	 * LL0 = second A in CAABC If S= CACAC and SP = CAC then LL0 = second C in S,
	 * LL1 = second A in S and LL2 = third C in S
	 * 
	...
	Position getIthLastInLastApearanceWithRespectToPrefix(Sequence prefix, int i, boolean withTimeStamps) {
		...
		PseudoSequencePair lastInstancePair = getLastInstanceOfPrefixSequence(prefix,
				prefix.getItemOccurencesTotalCount());

		// IF WE DON'T USE TIMESTAMP THE "ITH LAST IN LAST" IS A LITTLE BIT COMPLICATED
		// TO GET :
	...
	/**
	 * Get the ith maximum period of a prefix sequence for this sequence S. The ith
	 * maximum period of the prefix Sp in S is : * if 0 < i <= n, it is the piece of
	 * sequence between the end of the first instance of prefix e1... ei-1 in S and
	 * the ith last-in-last appearance with respect to prefix Sp * if i = 0, it is
	 * the piece of sequence in S located before the first last-in-last appearance
	 * with respect to prefix Sp. Example1: if S = ABCB and Sp = AB the 1th
	 * semi-period of Sp in S is empty the 2th semi-period of Sp in S is BC
	 * Example2: if S = ABBB and Sp = BB the 1th semi-period of Sp in S is AB the
	 * 2th semi-period of Sp in S is B
	 */
	PseudoSequence getIthMaximumPeriodOfAPrefix(Sequence prefix, int i, boolean withTimeStamps) {
		if (i == 0) { // it is the piece of sequence in S located before the first last-in-last
						// appearance with respect to prefix Sp.
		...
		// ELSE it is the piece of sequence between the end of the first instance of
		// prefix e1... ei-1 in S
		// and the ith last-in-last appearance with respect to prefix Sp
		// Important : We thus have to cut the prefix at ei-1 (short prefix = e1 ...
		// ei-1).
		// It is because the parameter i is used by
		// getLastInstanceOfPrefixSequence(...). (???)
	...
	/**
	 * Method that is used by the one above. This method cut a sequence by removing
	 * some part at the begining and at the end. IMPORTANT : this method assumes
	 * that the sequence has never been cut. This simplify what this method has to
	 * do to handle time.
	 * 
...
//		lastIteml
		...
		} else if (absolutePositionLastElement < sequence.size() - 1) { // if the last itemset is not the last itemset
	...
	/**
	 * Get the ith first-in-last appearance with respect to a prefix sequence Sp. n
	 * = size of S If i == n, it is the last appearance of ei in the first instance
	 * of Sp. If 0 <= i < n, it is the last appearance of ei in the first instance
	 * of Sp, and LFi must appear before LFi+1.
	 * 
	...
	Position getIthLastInFirstApearanceWithRespectToPrefix(Sequence prefix, int i, boolean withTimestamps) {
		...
		PseudoSequencePair firstInstancePair = getFirstInstanceOfPrefixSequence(prefix,
				prefix.getItemOccurencesTotalCount());
	...
	/**
	 * Get the ith semi-maximum period of a prefix sequence for this sequence S. The
	 * ith semi-maximum period of the prefix Sp in S is : * if 0 < i <= n, it is the
	 * piece of sequence between the end of the first instance of prefix e1... ei-1
	 * in S and the ith last-in-first appearance with respect to prefix Sp * if i =
	 * 0, it is the piece of sequence in S located before the first last-in-first
	 * appearance with respect to prefix Sp.
	 */
	PseudoSequence getIthSemiMaximumPeriodOfAPrefix(Sequence prefix, int i, boolean withTimestamps) {

		if (i == 0) { // it is the piece of sequence in S located before the first last-in-first
						// appearance with respect to prefix Sp.
		...
		// since the parameter i is used by getLastInstanceOfPrefixSequence(...)
		// and.....(???)
		...
		// we return the piece of sequence between the end of the first instance of
		// prefix e1... ei-1 in S
		// and the ith last-in-first appearance with respect to prefix Sp
