...
public class Trie implements Comparable<Trie> {
	 ...
	 * 
	 * @param index Child index in which we are interested
	 * @return true if the node was removed, otherwise, the index was out of range
	...
	/*
	 * public void mergeWithTrie(TrieNode newTrie) { if (levelSize() == 0) { if
	 * (nodes == null) { nodes = new ArrayList<TrieNode>(1); } nodes.add(newTrie); }
	 * else { nodes.add(newTrie); } }
	 */
		...
		if (nodes != null) {
			...
			for (int i = 0; i < nodes.size(); i++) {
				...
				DefaultMutableTreeNode currentNode = new DefaultMutableTreeNode(
						node.getPair().toString() + child.appearingIn);
	 ...
	 * @return 0 if the id is the same, -1 if the id of this trie is smaller,
	 *         otherwise 1.
	 */
	@Override
