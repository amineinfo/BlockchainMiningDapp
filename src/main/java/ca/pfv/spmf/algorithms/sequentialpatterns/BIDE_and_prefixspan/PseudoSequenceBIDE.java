 ...
 * Since it is a projected sequence, it makes reference to the original
 * Sequence.Java class used by BIDE+.
 * 
 * This class also include several methods for calculating the maximum periods,
 * semi-maximum periods, etc. as required by the BIDE+ algorithm. These methods
 * are quite complex so if you want to understand them, it is recommended to
 * read the BIDE+ paper carefully before reading the code.
 *
...
//	}
	...
	protected Position getIthLastInLastApearanceWithRespectToPrefix(List<Itemset> prefix, int i,
			PseudoSequencePair lastInstancePair) {
	...
	protected PseudoSequenceBIDE getIthMaximumPeriodOfAPrefix(List<Itemset> prefix, int i) {
		...
		PseudoSequencePair lastInstancePair = getLastInstanceOfPrefixSequenceNEW(prefix,
		...
		for (int j = 0; j < size(); j++) {
			...
			for (int k = 0; k < getSizeOfItemsetAt(j) && !allMatched; k++) {
				...
				if (currentItem == searchedItem) {
					tempList.add(new Position(j, k));
					itemInPrefixPosition++;
					if (itemInPrefixPosition == prefix.get(prefixItemsetPosition).size()
							|| tempList.size() == remainingToMatchFromPrefix) {
						allMatched = true;
						break;
					}
					searchedItem = prefix.get(prefixItemsetPosition).get(itemInPrefixPosition);
...
//		PseudoSequencePair firstInstancePair = getFirstInstanceOfPrefixSequence(prefix, getItemOccurencesTotalCount(prefix));
		PseudoSequencePair firstInstancePairNEW = getFirstInstanceOfPrefixSequenceNEW(prefix,
				getItemOccurencesTotalCount(prefix));
...
//			pseudo.toString();
	...
	protected Position getIthLastInFirstApearanceWithRespectToPrefix(List<Itemset> prefix, int i,
			PseudoSequencePair firstInstancePair) {
