...
/**
 * This is an implementation of the "USPAN" algorithm for High-Utility
 * Sequential Pattern Mining as described in the conference paper : <br/>
 * <br/>
 * 
 * Yin, Junfu, Zhigang Zheng, and Longbing Cao. "USpan: an efficient algorithm
 * for mining high utility sequential patterns." Proceedings of the 18th ACM
...
//						else System.out.println(item + " is already considered in the same (line) input sequence");								
		...
		// If we are in debug mode, we will show the number of distinct items in the
		// database,
		...
		try {
			...
			// The second buffer will store the utility of items in a sequence and the -1
			// between them)
			int[] utilityBuffer = new int[BUFFERS_SIZE];
			// The following variable will contain the length of the data stored in the two
			// previous buffer
			int itemBufferLength;
			// Finally, we create another buffer for storing the items from a sequence
			// without
			// the -1. This is just used so that we can collect the list of items in that
			// sequence
			// efficiently. We will use this information later to create the number of rows
			// in the
			// QMatrix for that sequence.
			int[] itemsSequenceBuffer = new int[BUFFERS_SIZE];
			// The following variable will contain the length of the data stored in the
			// previous buffer
			int itemsLength;
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				int nbItemsets = 1;
				// This variable will be used to remember if an itemset contains at least a
				// promising item
				...
				for (int itemset = 0; itemset < nbItemsets; itemset++) {
					...
					while (posBuffer < itemBufferLength) {
						...
						// else if it is the item that correspond to the next row in the matrix
							...
							// it means that the item do not appear in that itemset, so we put a utility of
							// 0
	...
	/**
	 * This is the initial call to the USpan procedure to find all High utility
	 * sequential patterns of length 1. It is optimized for finding patterns of
	 * length 1. To find larger patterns the "uspan" method is then used
	 * recursively.
	 * 
	 ...
	 * @param database     This is the original sequence database (as a set of
	 *                     QMatrix)
	...
	private void uspanFirstTime(int[] prefix, int prefixLength, List<QMatrix> database) throws IOException {
		...
		for (Entry<Integer, Integer> entry : mapItemSWU.entrySet()) {
			...
			if (itemSWU >= minUtility) {
				...
				// We initialize two variables for calculating the total utility and remaining
				// utility
				// of that item
				int totalUtility = 0;
				int totalRemainingUtility = 0;

				// We also initialize a variable to remember the projected qmatrixes of
				// sequences
...
//				}
				...
				// if this item passes the depth pruning (remaining utility + totality >=
				// minutil)
	...
	/**
	 * This inner class is used to store the SWU of an item and the last sequence
	 * where it was seen. It is used in the uspan() method.
	 * 
	 * @author Philippe Fournier-Viger, 2015
	 *
	 */
	private class Pair {
		...
		int swu;
		/**
		 * the last sequence where the item was seen (represented by a projected
		 * QMatrix)
		 */
	...
	/**
	 * This is the general USpan procedure to find all High utility sequential
	 * patterns of length greater than 1.
	 * 
	 * @param prefix       This is the buffer for storing the current prefix.
	 * @param prefixLength The current prefix length.
	 * @param database     This is a projected sequence database (a set of projected
	 *                     QMatrixes)
	 * @param itemCount    the number of items in the prefix
	 * @throws IOException If an error occurs while reading/writting to file.
	 */
	private void uspan(int[] prefix, int prefixLength, List<QMatrixProjection> projectedDatabase, int itemCount)
			throws IOException {
		if (DEBUG) {
		...
		for (Entry<Integer, Pair> entry : mapItemSWU.entrySet()) {
			...
			if (itemSWU.swu >= minUtility) {
				...
				// This variable will be used to calculate this item's utility for the whole
				// database
				int totalUtility = 0;
				// This variable will be used to calculate this item's remaining utility for the
				// whole database
				int totalRemainingUtility = 0;

				// Initialize a variable to store the projected QMatrixes for the
				// i-concatenation
...
//				}
				...
				// if his i-concatenation passes the depth pruning (remaining utility +
				// totality)
				if (totalUtility + totalRemainingUtility >= minUtility) {
		...
		mapItemSWU.clear();
		// Now, we will loop over sequences of the projected database to calculate the
		// local SWU
		// of each item.
		// For each sequence in the projected database
		for (QMatrixProjection qmatrix : projectedDatabase) {
			...
			for (MatrixPosition position : qmatrix.positions) {
				...
				for (int row = 0; row < qmatrix.getItemNames().length; row++) {
					...
					// For each column
					for (int column = position.column
							+ 1; column < qmatrix.originalMatrix.matrixItemUtility[row].length; column++) {
						// if the item appears in that column
						if (qmatrix.getItemUtility(row, column) > 0) {
							// get its swu until now
							Pair currentSWU = mapItemSWU.get(item);
							...
							// we don't need to check the other column if we found one column where this
							// item
...
//			System.out.println(entry.getKey() + "  swu: " + entry.getValue().swu);
			...
			if (itemSWU.swu >= minUtility) {
				...
				// This variable is used to store the utility of this s-concatenation in the
				// whole database
				int totalUtility = 0;
				...
				// Initialize a variable to store position of the last item of this
				// s-concatenation
				List<QMatrixProjection> matrixProjections = new ArrayList<QMatrixProjection>();
				...
				for (QMatrixProjection qmatrix : projectedDatabase) {
					...
					if (rowItem >= 0) {
						...
						for (MatrixPosition position : qmatrix.positions) {
							...
							for (int column = position.column
									+ 1; column < qmatrix.originalMatrix.matrixItemUtility[rowItem].length; column++) {
...
//	}
	...
	private void writeOut(int[] prefix, int prefixLength, int utility) throws IOException {
		...
		if (SAVE_RESULT_EASIER_TO_READ_FORMAT == false) {
			...
			// Otherwise, if the user wants to save in a format that is easier to read for
			// debugging.
	...
	/**
	 * This method check if the utility of a pattern has been correctly calculated
	 * for debugging purposes. It is not designed to be efficient since it is just
	 * used for debugging.
	 * 
	...
	/**
	 * This is some code for verifying that the utility of a pattern is correctly
	 * calculated for debugging only. It is not efficient. But it is a mean to
	 ...
	 * @param prefixPos       the position in the current pattern that we will try
	 *                        to match with the sequence
	 * @param seqPos          the position in the sequence that we will try to match
	 *                        with the pattenr
	 * @param utility         the calculated utility until now
	 * @return the utility of the pattern
	 */
	private int tryToMatch(int[] sequence, int[] sequenceUtility, int[] prefix, int prefixLength, int prefixPos,
			int seqPos, int utility) {
		...
		while (posP < prefixLength & posS < sequence.length) {
			if (prefix[posP] == -1 && sequence[posS] == -1) {
				...
				int otherUtility = tryToMatch(sequence, sequenceUtility, prefix, prefixLength, previousPrefixPos, posS,
						utility);
				...
				if (posP == prefixLength) {
					...
					int otherUtility = tryToMatch(sequence, sequenceUtility, prefix, prefixLength, previousPrefixPos,
							posS, utility);
					otherUtilityValues.add(otherUtility);
