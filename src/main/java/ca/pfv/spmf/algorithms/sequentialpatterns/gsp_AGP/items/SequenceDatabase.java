...
/**
 * Inspired in SPMF Implementation of a sequence database, where each sequence
 * is implemented as an array of integers and should have a unique id. See
 * examples in /test/ directory for the format of input files.
 *
...
public class SequenceDatabase {
	...
	/**
	 * Map to associate the frequent items with the 1-patterns composed by
	 * themselves
	 */
	...
	/**
	 * It loads the database contained in the file path given as parameter. Besides,
	 * all the frequent 1-patterns are identified and the original database is
	 * updated by removing the non-frequent items
	 * 
	...
	public void addSequence(String[] integers) {
		...
		for (int i = start; i < integers.length; i++) {
			if (integers[i].codePointAt(0) == '<') { // Timestamp
				...
				Pattern pattern = frequentItems.get(item);
				if (pattern == null) {
					pattern = patternCreator.createPattern(
							creadorPares.getItemAbstractionPair(item, abstractionCreator.CreateDefaultAbstraction()));
	...
	/**
	 * It return a map where are associated each frequent item with the 1-pattern
	 * composed by itself
	 * 
	 * @return the map
	 */
	...
	/**
	 * Reduce the original database, removing the items given in the parameter set
	 * 
	 * @param keySet
	 */
