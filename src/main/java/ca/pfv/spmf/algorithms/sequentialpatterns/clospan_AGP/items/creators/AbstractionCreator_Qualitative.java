...
public class AbstractionCreator_Qualitative extends AbstractionCreator {
	 ...
	 * 
	 * @param equalRelation The boolean indicatin if the item has an equal relation
	 *                      with the previous item in the pattern
	...
	/**
	 * It adds a Pair object to one list when we keep the sequences counted for that
	 * pair. If the pair has not been previously kept, we keep the sequenceID
	 * 
	 * @param pairMap                     a map of Pair object
	 * @param alreadyCountedForSequenceID the set of sequence IDs that have been
	 *                                    already counted
	 * @param id                          an ID
	 * @param item                        the item
	 * @param postfix                     indicates if it is the case of a postfix
	 *                                    (an itemset cut at left)
	 */
	private void addPair(Map<Pair, Pair> pairMap, Set<Pair> alreadyCountedForSequenceID, int id, Item item,
			boolean postfix) {
		/*
		 * We create a new Pair object from the given item and the postfix flag
		 */
		Pair pair = new Pair(postfix,
				ItemAbstractionPairCreator.getInstance().getItemAbstractionPair(item, createAbstraction(postfix)));
	...
	/**
	 * Method to find all frequent items in a context (database). This is for k> 1.
	 * 
	...
	public Set<Pair> findAllFrequentPairs(List<PseudoSequence> sequences) {
		// we will scan the database and store the cumulative support of each pair in a
		// map.
		...
		for (PseudoSequence sequence : sequences) {
			...
			loop1: for (int k = 0; k < sequence.numberOfProjectionsIncluded(); k++) {
				for (int i = 0; i < sequence.size(k); i++) {
					// If we are after the first projection and after the first itemset of the
					// pseudosequence
					if (k > 0 && i > 0) {
					...
					/*
					 * And for each item from the beginning we add a new Pair in order to find the
					 * frequent items in the projection*
					 */
	...
	/**
	 * It creates an abstraction from a prefix and it is the same abstraction that
	...
	/**
	 * Method that check if for the two patterns given as parameters, the shortest
	 * one is a subpattern of the longest one
	 * 
	 ...
	 * @param index     index that indicates which position we have to take into
	 *                  account
	 * @param positions List of positions of the appearances of the elements of the
	 *                  shorter pattern in the longer one
	...
	public boolean isSubpattern(Pattern shorter, Pattern larger, int index, List<Integer> positions) {
		...
		Integer pos = null;
		/*
		 * While the item index pointed out by the position is less than the size of the
		 * largest pattern
		 */
		while (positions.get(index) < larger.size()) {
			/*
			 * We search for the item of the shorter pattern pointed by index in the longer
			 * one
			 */
			if (index == 0) {
				pos = searchForFirstAppearance(larger, positions.get(index), itemPair);
			} else {
				pos = findItemPositionInPattern(larger, itemPair, absPair, previousAbs, positions.get(index),
			...
			if (pos != null) {
				...
				if (index + 1 < shorter.size()) {
					...
					/*
					 * And we make a recursive call to go on checking if shorter is a subpattern of
					 * longer
					 */
					...
					/*
					 * We have already found a matching between shorter and longer and we conclude
					 * that one is a subpattern of the other one
					 */
					return true;
				}
				...
				cancelled = true;
				/*
				 * And break the loop in order to go back and try to find other matching
				 * elements that makes the subsequence possible
				 */
				break;
			}
		}
		/*
		 * If we are finish the loop and not by breaking it, and we are not looking for
		 * the first element of the shorter pattern
		 */
		...
		/*
		 * We return a false value, indicating that we cannot reach a matching with the
		 * current choices of elements in the longer pattern
		 */
		return false;
	}

	/**
	 * Method that search the first appearance of an item (given as parameter) in a
	 * pattern, starting from a beginning index
	 * 
	 ...
	 * @return The item position where we found the item, or null if this does not
	 *         appear
	 */
	...
	/**
	 * It searches for a position in the pattern given as parameter where an item,
	 * also given as a parameter, appears
	 * 
	 ...
	 * @return the position
	 */
	public Integer findItemPositionInPattern(Pattern p, Item itemPair, Abstraction_Generic currentAbs,
			Abstraction_Generic previousAbs, Integer currentPosition, Integer previousPosition) {
	...
	private Integer searchForInTheSameItemset(Pattern pattern, Item itemPair, Integer beginning) {
		...
		for (int i = beginning; i < pattern.size(); i++) {
			...
			if (!qualitativeAbstraction.hasEqualRelation()) {
				...
				/*
				 * If, conversely, there is an equal relation, we check if this item is equal to
