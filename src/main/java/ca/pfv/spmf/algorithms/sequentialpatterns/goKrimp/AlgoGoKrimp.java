...
public class AlgoGoKrimp {
	...
	public void gokrimp() throws IOException {
		...
		System.out.println("Compressed size: " + comp_size + ", uncompressed size: " + uncomp_size
	...
	/**
	 * candidate based algorithm, search for the best encoding of the data given the
	 * set of candidates
	 * 
	 * @throws IOException
	 */
	...
	void addPattern(MyPattern pattern) {
		Nword = Nword - (pattern.freq - 1) * pattern.ids.size() + pattern.ids.size() + pattern.freq; // update the
																										// number of
																										// encoded words
		...
		for (int i = 0; i < patterns.size(); i++) { // update the frequency of
			...
			if (patterns.get(i).ids.size() == 1 && hm.containsKey(patterns.get(i).ids.get(0))) {// singleton
																								// among the
																								// events of
																								// the
																								// pattern
	...
	MyPattern extend(MyPattern pattern) {
		ArrayList<Integer> ve = get_Extending_Events_SignTest(pattern.ids.get(pattern.ids.size() - 1)); // get the set
																										// of extending
																										// events
		candidates.clear();
	...
	/**
	 * get the set of events being considered to extend a pattern, Signed Test is
	 * used to select such events
	 * 
	...
	/**
	 * get the compress size of the data when the given @param pattern is added to
	...
	double get_Compress_Size_When_Adding(MyPattern pattern) {
		...
		com += new_Nword * Math.log(new_Nword) / Math.log(2) - Nword * Math.log(Nword) / Math.log(2)
				- pattern.freq * Math.log(pattern.freq) / Math.log(2);
		...
		for (Integer key : hm.keySet()) {
			...
			com -= new_freq * Math.log(new_freq) / Math.log(2) - patterns.get(characters.get(key)).freq
					* Math.log(patterns.get(characters.get(key)).freq) / Math.log(2);
		}
		com += pattern.g_cost;
		return com;
	}
				...
				if (i == 0) {
					...
					for (int j = 0; j < pos.get(i).size(); j++) {
						...
						while (index < matrix.get(i - 1).size()
								&& matrix.get(i - 1).get(index).id < pos.get(i).get(j)) {
							if (matrix.get(i - 1).get(index).ts == Integer.MAX_VALUE) {
								index++;
								continue;
							}
							int g = matrix.get(i - 1).get(index).ts
									+ bits(pos.get(i).get(j) - matrix.get(i - 1).get(index).id);
							if (g <= min) {
								min = g;
								mini = index;
							}
	...
	/**
	 * get related events of the event @param e, sign-test is used to select these
	 * events
	 * 
	 ...
	 * @return the number of bits in the binary representation of the input integer
	 *         a using the Elias code
	 */
	int bits(Integer a) {
		if (a.intValue() < 0)
			return 0;
	 ...
	 * 
	 * @param index
	 * @return true if the pattern p is found in the sequence with the input index
	 */
