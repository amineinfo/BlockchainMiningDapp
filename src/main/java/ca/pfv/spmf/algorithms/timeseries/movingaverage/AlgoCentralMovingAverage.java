...
/**
 * An algorithm to calculate the central moving average of a time series. It
 * takes as parameter the window size (a number of data points).
 * 
	...
	public TimeSeries runAlgorithm(TimeSeries timeSeries, int windowSize) throws IOException {
		...
		if (windowSize >= timeSeries.data.length || windowSize < 3 || (windowSize % 2) != 1) {
			throw new IllegalArgumentException(
					" The window size must be odd, greater than 1, and no larger than the number of points in the time series");
		}
