...
/**
 * An algorithm to split a time series in several time series. The algorithm
 * takes as input a time series and the number of data points to be used for
 * each resulting time series.
 * 
...
public class AlgoSplitTimeSeries {
	 ...
	 * @return an array of time series (double array), thus a two-dimensional double
	 *         array
	 */
	...
	private TimeSeries[] splitTimeSeries(TimeSeries timeSeries, int sizeOfSegment) {
		...
		for (int i = 0; i < numberOfSeries; i++) {
			...
			splittedTimeSeries[i] = new TimeSeries(dataPoints, timeSeries.getName() + "_PART" + i);
