 ...
 * The resulting time series contains values in the [-1,1] interval. Calculating
 * the auto-correlation plot is very useful in time series analysis to analyze
 * time series where the time interval between each observation is always the
 * same. It can be used to check if a time series is random or if it is
 * stationary, to detect some periodic patterns, etc.
 * 
...
public class AlgoLagAutoCorrelation {
	...
	/**
	 * Generate the autocorrelation time series of a time series for a lag 1 to a
	 * lag "maxlag"
	 * 
	...
	public TimeSeries runAlgorithm(TimeSeries timeSeries, int maxlag) throws IOException {
		...
		if (maxlag < 1 || maxlag > timeSeries.size()) {
			throw new IllegalArgumentException(
					" The maxlag parameter must be set as follows:  1 <= maxlag <= timeSeries.length");
		}
	...
	/**
	 * Generate the autocorrelation time series of a time series from 1 to a
	 * "maxlag"
	 * 
	 * @param timeSeries a time series
	 * @param maxlag     a maximum lag
