...
/**
 * An implementation of the algorithm to generate the Piecewise Aggregate
 ...
 * 
 * This algorithm is typically used as a first step before applying the SAX
 * algorithm
 * 
...
public class AlgoPiecewiseAggregateApproximation {
	...
	public TimeSeries runAlgorithm(TimeSeries timeSeries, int numberOfSegments) throws IOException {
		...
		TimeSeries paaSeries = new TimeSeries(piecewiseTransformedData, timeSeries.getName() + "_PAA");
	...
	/**
	 * This method calculates the Piecewise Agregation Approximation (PAA)
	 * representation of a time-series.
	 * 
	 * @param dataPoints       the data points of the time series (an array of
	 *                         double)
	...
	private double[] transformTimeSeriesToPAARepresentation(double[] dataPoints, int numberOfSegments) {
		...
		if (isExactlyDivisible == false) {
			...
			// then we multiply the data point by that weight and add it to the sum for that
			// segment
