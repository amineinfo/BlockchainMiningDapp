...
/**
 * An algorithm to calculate the second order differencing of a time series. The
 * purpose of using this algorithm is to remove a trend in a time series. For a
 * time series containing "n" points, the output is a time series containing n-1
 * points where the i-th point of the transformed time series is equal to the
 * difference between the i-th point and the(i+1)-th point in the original time
 * series.
 * 
...
public class AlgoSecondOrderDifferencing {
	...
	/**
	 * Generate the second order differencing of a time series containing at least
	 * three data points.
	 * 
	...
	public TimeSeries runAlgorithm(TimeSeries timeSeries) throws IOException {
		...
		if (timeSeries.size() < 3) {
			throw new IllegalArgumentException(
					" The time series should contain at least 3 points to apply differencing.");
		}
