...
/**
 * An algorithm to calculate the median smoothing of a time series. It takes as
 * parameter the window size (a number of data points).
 * 
...
public class AlgoMedianSmoothing {
	...
	public TimeSeries runAlgorithm(TimeSeries timeSeries, int windowSize) throws IOException {
		...
		if (windowSize >= timeSeries.data.length || windowSize < 2) {
			throw new IllegalArgumentException(
					" The window size must be greater than 1, and no larger than the number of points in the time series");
		}
	...
	/**
	 * Generate the median smoothing of a time series for a window size that is an
	 * odd number.
	 * 
