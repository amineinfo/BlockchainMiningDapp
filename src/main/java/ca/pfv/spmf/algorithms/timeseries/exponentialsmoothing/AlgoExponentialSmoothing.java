...
/**
 * An algorithm to calculate the exponential smoothing of a time series. The
 * purpose of using this algorithm is to reduce noise in a time series by
 * smoothing the time series. It takes as parameter called alpha, which is a
 * percentage representing the influence of a data points as a percentage
 * compared to the influence of the preceding point). It expects that a time
 * series contains at least 2 points. The output For a time series containing
 * "n" points, the output is a time series containing n points.
 * 
 * Exponential smoothing was proposed in this paper: <br/>
 * <br/>
 * 
 * Brown, Robert Goodell (1963). Smoothing Forecasting and Prediction of
 * Discrete Time Series. Englewood Cliffs, NJ: Prentice-Hall.
 * 
...
public class AlgoExponentialSmoothing {
	 ...
	 * @param alpha      the smoothing constant (a double representing a percentage
	 *                   between 0 and 1)
	...
	public TimeSeries runAlgorithm(TimeSeries timeSeries, double alpha) throws IOException {
		...
		if (timeSeries.size() <= 1) {
			throw new IllegalArgumentException(
					" The time series should contain at least 2 points to apply exponential smoothing.");
		}
		...
		TimeSeries transformedTimeSeries = new TimeSeries(transformedTimeSeriesArray,
				timeSeries.getName() + "_EXPSTHG");
	 ...
	 * @param alpha      the smoothing constant (a double value in the [0,1]
	 *                   interval, representing the influence of the current point
