...
/**
 * This class reads a time series file and convert it to a sequence of symbols
 * using the SAX algorithm. The format of the output is the SPMF format. The SAX
 * algorithm is described in this paper:<br/>
 * <br/>
 * 
...
public class AlgoConvertTimeSeriesFileToSequencesWithSAX {
	 ...
	 * @param deactivatePAA    set this parameter to true to apply sax without
	 *                         converting the time series to PAA
	 * @throws IOException exception if error while writing the file
	 */
	public void runAlgorithm(List<TimeSeries> multipleTimeSeries, String output, int numberOfSegments,
		...
		if (DEBUG_MODE) {
			...
			for (TimeSeries timeSeries : multipleTimeSeries) {
				// print the time series number
				System.out
						.println(System.lineSeparator() + "--- Reading time series #" + (timeSeriesCount + 1) + " ---");
			}
		}
		...
		SAXSymbol[][] saxSequences = algo.runAlgorithm(multipleTimeSeries, numberOfSegments, numberOfSymbols,
				deactivatePAA);
