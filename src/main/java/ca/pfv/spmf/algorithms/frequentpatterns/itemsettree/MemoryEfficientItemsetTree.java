...
//			System.out.println(".");
	...
	/**
	 * Given the root of a sub-tree, add an itemset at the proper position in that
	 * tree
	 * 
	 * @param r      the root of the sub-tree
	 * @param s      the itemset to be inserted
	 * @param prefix the current item(s) explored in this branch of the tree until
	 *               the current node r.
	 */
...
//		newNode.parent = r;
	...
	/**
	 * Make a copy of an itemset while removing items that appears in two itemsets
	 * named "prefix" and "s".
	 * 
	 * @param r      the itemset
	...
	public void printStatistics() {
		...
		System.out.println(" Tree construction time ~: " + (endTimestamp - startTimestamp) + " ms");
		...
		System.out.println(" Sum of items in all node: " + totalItemCountInNodes + " avg per node :"
	...
	/**
	 * This method calculate the support of an itemset by using a subtree defined by
	 * its root.
	 * 
	 * Note: this is implemented based on the algorithm "count" of Table 2 in the
	 * paper by Kubat et al. // Note that there was a few problem in the algorithm
	 * in the paper. // I had to change > by < in : ci.itemset[ci.itemset.length -1]
	 * < s[s.length -1]){ // also the count was not correct so i had to change the
	 * way it counted the support a little bit // by using += instead of return.
	 * 
	 * @param s         the itemset
	...
	/**
	 * Get the frequent itemsets subsuming a given itemset for a given minimum
	 * support value.
	 * 
	...
	public HashTableIT getFrequentItemsetSubsuming(int[] is, int minsup) {
		...
		// after finding the itemsets we do a loop to remove those with a support lower
		// than minsup,
	...
	/**
	 * This method pass through the itemset tree to get all itemsets that are
	 * subsuming a given itemset "s" and their support. Note that this method may
	 * also return infrequent itemsets that can be filtered by additional processing
	 * after.
	 * 
	 * @param s the itemset
	...
	/**
	 * This method finds itemsets subsuming a given itemset. It is a recursive
	 * method that scan a subtree of the itemset-tree. It stores itemsets found in
	 * an hashtable together with their support.
	 * 
	 * @param s    the itemset s
	 * @param seti the items from the itemset s stored in a HashSet<Integer> for
	 *             more efficiency for inclusion checking
	 * @param t    the root of the subtree
	 * @param hash the hashtable for storing the result
	 * @return the cumulative support of the t's immediate children. This is needed
	 *         to ensure t is correctly incorporated into the hashtable (and with
	 *         the correct support).
	 */
			...
			if (ciprefix[0] <= s[0]) {
				...
				if (ArraysAlgos.includedIn(s, ciprefix)) {
					...
					if (ci.childs.size() == 0) {
						...
						// Note, we subtract the count returned by selectiveMining (which contains the
						// cumulative
						// support of the ci's immediate children) from ci's support
						// remainingSup thus indicates how many times ci's itemset appeared by itself in
						// the database)
						int remainingSup = ci.support - selectiveMining(s, seti, ci, hash, ciprefix);

						// If remainingSup is greater than 0, then it means ci's children do not fully
						// account for all of
						// the occurrences of ci's itemset. In other words, ci's itemset appeared by
						// itself remainingSup times
						// Hence, we need to put s in the hashtable with remainingSup, and then call
						// recursive add on ci
						// with remainingSup.
						if (remainingSup > 0) {
	...
	/**
	 * Perform a recursive add (as based on the procedure presented in the paper by
	 * Kubat et al.)
	 * 
