...
public class estTree {
	...
	/********************************************************************
	 * Recursive method for updating the counters of itemsets that belong to a given
	 * transaction (Phase 2: count updating phase).
	 *
	...
	void updateNodes(estNode currentNode, int[] transaction, int ind) {
		...
		if (child != null) {
			...
			if (child.computeSupport(N) >= minsig)
				...
				/*
				 * PFV: WHY NOT REMOVE? currentNode.children.remove(child); else
				 */
			}
	 ...
	 * new transaction (Phase 3 : Delayed Insertion Phase).
	 *
	...
	void insertItemset(int[] transaction) {
		...
		// add each item from the given transaction that has enough support to the new
		// transaction
		...
		// func2
		/*
		 * int[] itemset = new int[1]; for (int i = 0; i < transaction2.size(); ++i) {
		 * itemset[0] = transaction2.get(i);
		 * insert_n_itemsets2(root.getChildWithID(transaction2.get(i)), transaction2, i
		 * + 1, itemset); }
		 */
	 ...
	 * @param the         depth of the current node with respect to the root of the
	 *                    tree
	 * @param itemset
	 ********************************************************************/
	...
	public void insert_n_itemsets2(estNode currentNode, List<Integer> transaction, int ind, int[] itemset) {
...
//********************************************************************
	public void insert_n_itemsets3(estNode currentNode, List<Integer> transaction, int ind, int[] itemset, int length) {
