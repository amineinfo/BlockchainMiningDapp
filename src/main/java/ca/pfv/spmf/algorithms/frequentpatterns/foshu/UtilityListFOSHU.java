...
public class UtilityListFOSHU {
	...
	// A list where each entry correspond to a period.
	// An entry contains the sum of iutil and rutil valuess corresponding to that
	// period
	 ...
	 * @param x           the utility-list of pU{X}. . * @param y the utility-list
	 *                    of pU{Y}.
	...
	public UtilityListFOSHU(int periodCount, UtilityListFOSHU pUL, UtilityListFOSHU x, UtilityListFOSHU y) {
		...
		periodsElements = new ArrayList[periodCount];
		// Initialize an array to store the sum of iutil and rutil values for each
		// period
		periodsSumIutilRutil = new int[periodCount];
	 ...
	 */
	private void construct(int period, List<ElementFOSHU> pElements, List<ElementFOSHU> pXElements,
			List<ElementFOSHU> pYElements) {
		;
		...
		for (ElementFOSHU ex : pXElements) {
			...
			if (e != null) {
				// Create new element
				ElementFOSHU eXY = new ElementFOSHU(ex.tid, ex.iputils + ey.iputils - e.iputils,
						ex.inutils + ey.inutils - e.inutils, ey.rutils);
