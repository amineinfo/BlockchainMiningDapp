...
/**
 * This is an implementation of the LCMFreq algorithm for mining frequent
 * itemsets from a transaction database. More information on the LCMFreq
 * algorithm can be found in papers by T. Uno, such as: <br/>
 * <br/>
 * 
 * T. Uno, M. Kiyomi, and H. Arimura. Lcm ver. 2: Efficient mining algorithms
 * for frequent/closed/maximal itemsets. In FIMI, 2004
 * 
 * This implementation of LCM was made by Alan Souza and was modified by
 * Philippe Fournier-Viger to add optimizations. <br/>
 * 
...
public class AlgoLCMFreq {
	...
	/**
	 * This buffer is used to store an itemset that will be written to file so that
	 * the algorithm can sort the itemset before it is output to file (when the user
	 * choose to output result to file).
	 */
	private int[] itemsetOutputBuffer = null;

	/**
	 * This buffer is used to store the locally frequent items in a projected
	 * database
	 */
//	private int[] locallyFrequentItemsBuffer = null;

	/**
	 * Buckets for occurence delivery Recall that each bucket correspond to an item
...
//	ItemNameConverter nameConverter;
	 ...
	 * Run the algorithm
	 * 
	 * @param minimumSupport the minimum support threshold as percentage value
	 *                       between 0 and 1
	 * @param dataset        the dataset
	 * @param outputPath     the output file path to save the result or null if to
	 *                       be kept in memory
...
//			}
	...
	private void backtrackingLCMFreq(int[] p, int pLength, List<Transaction> transactionsOfP,
		...
		for (int j = 0; j < frequentItems.size(); j++) {
			...
			if (pLength + 2 <= maxItemsetSize) {
				...
				// (i.e. "e" appears after the position of the tail item in the list of all
				// items)
	 ...
	 * @param e
	 */
	private void anyTimeDatabaseReductionFreq(List<Transaction> transactionsPe, int j, List<Integer> frequentItems,
			int[] itemset, int itemsetLength, Integer e) {
	...
	/**
	 * Write a frequent itemset that is found to the output file or keep into memory
	 * if the user prefer that the result be saved into memory.
	 */
