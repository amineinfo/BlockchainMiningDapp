...
//				+ " " + sameCount);
		...
		while (sibling != null) {
			if ((level == 1 && itemsetCount[(curNode.label - 1) * curNode.label / 2 + sibling.label] >= minSupport)) {
				...
				lastChild = is2_itemSetValid(curNode, sibling, level, lastChild, sameCountTemp);
	...
	NodeListTreeNode is2_itemSetValid(NodeListTreeNode ni, NodeListTreeNode nj, int level, NodeListTreeNode lastChild,
			IntegerByRef sameCount) {
		int i = ni.label;
		int j = nj.label;
	...
	/**
	 * This method write an itemset to file + all itemsets that can be made using
	 * its node list.
	 * 
