...
//		currentNode.counter = support;
	...
	/**
	 * Perform the subset test to see if an itemset is a subset of an already found
	 * CFI with the same support
	 * 
	...
	public boolean passSubsetChecking(int[] headWithP, int headWithPLength, int headWithPSupport) {
		...
		if (lastAddedItemsetNode != null && lastAddedItemsetNode.counter == headWithPSupport) {
			boolean isSubset = issASubsetOfPrefixPath(headWithP, headWithPLength, lastAddedItemsetNode);
			// if the itemset is a subset of the last added itemset, we do not need to check
			// further
			if (isSubset) {
				return false;
			}
		...
		// OTHERWISE, WE NEED TO COMPARE "headwithP" with all the patterns in the
		// CFI-tree.
		CFINode node = mapItemNodes.get(firstITem);
		// if that last item is not yet in the CFI-tree, it means that "itemset" is not
		// a subset
		...
		do {
			// for a node, we will check if "headwithP" is a subset of the path ending at
			// node
	...
	/**
	 * Check if the itemset headwithP is contained in the path ending at "node" in
	 * the CFI-tree and have the same support
	 * 
	 * @param headWithP       the itemset headwithP
	 * @param headWithPLength the last position to be considered in headWithP
	 * @param node            the node
	 * @param the             support of the itemset headwithP
	 * @return true if "headwithP" is contained in the path ending at "node" in the
	 *         CFI-Tree and has the same support. Otherwise, false.
	 */
	private boolean issASubsetOfPrefixPath(int[] headWithP, int headWithPLength, CFINode node) {
		// optimization proposed in the fpmax* paper: if there is less than itemset node
		// in that branch,
	...
	/**
	 * This method calculate the support of any frequent itemset by using the
	 * CFI-tree containing all the frequent closed itemsets. Note that this method
	 * assumes that all frequent closed itemsets are stored in the CFI-tree.
	 * 
	 * **** It is not used by FPClose***.
	 * 
	 * It is instead designed for algorithms that use the CFI-tree generated by
	 * FPClose such as the algorithm for generating closed association rules.
	 * 
...
//					return node.counter;
	...
	/**
	 * Sort an array of items according to the total order of support This has an
	 * average performance of O(n^2)
	 * 
