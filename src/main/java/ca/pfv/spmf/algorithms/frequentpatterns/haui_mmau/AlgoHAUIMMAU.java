...
/**
 * This is an implementation of the "HAUI-MMAU" for High Average-Utility
 * Itemsets Mining. HAUI-MMAU is described in the conference paper : <br/>
 * <br/>
 * 
 * Jerry Chun-Wei Lin, Ting Li, Philippe Fournier-Viger, Tzung-Pei Hong, and
 * Ja-Hwung Su. Efficient Mining of High Average-Utility Itemsets with Multiple
 * Minimum Thresholds Proceedings of Industrial Conference on Data Mining,
 * 2016:14-28.
 * 
 * This implementation uses the Apriori algorithm as it seems to be suggested by
 * the article, even if the Apriori algorithm is not mentionned explicitly in
 * the article.
 *
 * @see ItemsetsTP
 * @see ItemsetTP
...
public class AlgoHAUIMMAU {
	 ...
	 * @param mutipleMinUtilities: the minimum average-utility threshold of each
	 *        item.
	 * @param GLMAU: user-specified and represents the global least average-utility
	 *        value
	 * @return final result
	 */
	public ItemsetsTP runAlgorithm(UtilityTransactionDatabaseTP database, Map<Integer, Integer> mutipleMinUtilities,
			int GLMAU) {
		// save the parameters
		...
		// Scan database one time to get the tidset of each item and its utility for the
		// whole database
		...
		for (int i = 0; i < candidatesSize1.size(); i++) {
			for (int j = candidatesSize1.size() - 1; j > 0; j--) {
				if (mutipleMinUtilities.get(candidatesSize1.get(j - 1).getItems().get(0)) > mutipleMinUtilities
						.get(candidatesSize1.get(j).getItems().get(0))) {
		...
		while (true) {
			...
			currentLevel = generateCandidateSizeK(currentLevel, highAUtilityItemsets, candidateHAUIMap,
					mutipleMinUtilities, GLMAU);
			// if no new candidates are found, then we stop because no more candidates will
			// be found.
		...
		// ======================== PHASE 2: Calculate exact average utility of each
		// candidate =============
		...
		for (List<ItemsetTP> level : highAUtilityItemsets.getLevels()) {
			...
			while (iterItemset.hasNext()) {
				...
				// Calculate exact average utility of that HAUUBI by scanning transactions of
				// its tidset
				// For each transaction
				for (TransactionTP transaction : database.getTransactions()) {
					// variable to store the transaction utility of "candidate" for the current
					// transaction
	...
	/**
	 * X1+X2=X, if X1 and X2 are both not , then X will not be a
	 * 
	...
	protected static boolean judge(List<Integer> items, Map<List<Integer>, Integer> HAUIMap, boolean mark) {
		...
		// which means scanning database is needed for getting actually average utility
		// value for items
		if (items.size() == 1)
			return true;
