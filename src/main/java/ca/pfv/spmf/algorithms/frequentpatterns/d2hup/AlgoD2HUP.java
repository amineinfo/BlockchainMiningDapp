...
/**
 * This is an implementation of the "D2HUP Algorithm" for High-Utility Itemsets
 * Mining as described in the conference paper : <br/>
 * <br/>
 * 
 * Liu, J., Wang, K., Fung, B. (2012). Direct discovery of high utility itemsets
 * without candidate generation. Proceedings of the 2012 IEEE 12th International
 * Conference on Data Mining. IEEE Computer Society, 2012.
 *
...
public class AlgoD2HUP {
	...
	/**
	 * This represent the cells in transactions in the CAUL structure It is an
	 * array, where each transaction appears followed by "null". This is the most
	 * efficient way that I have found to implement CAUL.
	 **/
	Cell cells[];
	...
	/**
	 * temporary structure used to match an item to its row in a CAUL structure. It
	 * is used for building each CAUL. Key= item Value= the corresponding row
	 */
	...
	public void runAlgorithm(String input, String output, int minUtility) throws IOException {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				Arrays.sort(cells, transactionBegin, transactionEnd + 1, new Comparator<Cell>() {
		...
		// it may not look exactly the same as in the paper. But the result of the
		// algorithm
		// is the same.
		if (DEBUG) {
	 ...
	 * @param rowList       the list of row in the current CAUL (containing only
	 *                      promising items)
	 * @param prefixSupport This is the support of the prefix
	 * @param prefixUtility the utility of the prefix
	 * @throws IOException
	 */
	private void d2hup(int[] prefix, int prefixLength, List<Row> rowList, int prefixSupport, int prefixUtility)
			throws IOException {
		...
		for (Row row : rowList) {
			...
			if (row.utility + row.rutil >= minUtility) { /// xxxxxxxx
				...
				for (Pointer pointer : row.pointers) {
					...
					// newPrefixUtility
					int newPrefixRowUtility = pointer.prefixUtility + cells[pointer.pos].utility;
