...
/**
 * This is an implementation of the "PFPM" algorithm for Periodic Itemsets as
 * described in the conference paper : <br/>
 * <br/>
 * 
 * Fournier-Viger, P., Lin, C.-W., Duong, Q.-H., Dam, T.-L., Sevcic, L., Uhrin,
 * D., Voznak, M. (2016). PFPM: Discovering Periodic Frequent Patterns with
 * Novel Periodicity Measures. Proc. 2nd Czech-China Scientific Conference 2016,
 * Elsevier, 10 pages.
 *
 * @see TIDList
 * @see Element
...
public class AlgoPFPM {
	 ...
	 */
	public void runAlgorithm(String input, String output, int minPeriodicity, int maxPeriodicity,
...
//						+ " avg periodicity: " + itemInfo.averagePeriodicity
						...
						+ itemInfo.smallestPeriodicity + "\tmaxPer: " + itemInfo.largestPeriodicity + "\tsup.: "
						+ itemInfo.support);
			}
		}
		if (DEBUG) {
...
//		int compare = (int)( mapItemToItemInfo.get(item1).support - mapItemToItemInfo.get(item2).support);
	...
	private void fpp(int[] prefix, int prefixLength, TIDList pUL, List<TIDList> ULs) throws IOException {
		...
		for (int i = 0; i < ULs.size(); i++) {
			...
			if (averagePeriodicity <= maxAveragePeriodicity && averagePeriodicity >= minAveragePeriodicity
					&& X.smallestPeriodicity >= minPeriodicity && X.largestPeriodicity <= maxPeriodicity) {
...
//		}
	 ...
	 */
	private void writeOut(int[] prefix, int prefixLength, TIDList utilityList, double averagePeriodicity)
			throws IOException {
	...
	public void printStats() throws IOException {
		...
		System.out.println("=============  PFPM ALGORITHM v2.17" + optimizationESCP + "=====");
