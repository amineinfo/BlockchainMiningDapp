...
/**
 * This is an implementation of the EFIM_Closed algorithm for mining
 * high-utility itemsets from a transaction database. More information on the
 * EFIM algorithm can be found in that paper: <br\><br\>
 * 
 * Fournier-Viger, P., Zida, S. Lin, C.W., Wu, C.-W., Tseng, V. S. (2016).
 * EFIM-Closed: Fast and Memory Efficient Discovery of Closed High-Utility
 * Itemsets. Proc. 12th Intern. Conference on Machine Learning and Data Mining
 * (MLDM 2016). Springer, LNAI, 15 pages, to appear
 *
...
public class AlgoEFIMClosed {
	 ...
	 */
	private int backtrackingEFIM(List<Transaction> transactionsOfP, List<Integer> itemsToKeep,
		...
		for (int j = 0; j < itemsToExplore.size(); j++) {
						...
						if (activateTransactionMerging
							...
							if (previousTransaction == null) {
								...
								if (consecutiveMergeCount == 0) {
									...
									int[][] mergeOriginalTransactions = mergeOriginalTransactions(previousTransaction,
			...
			if (hasNoBackwardExtension(temp, prefixLength, transactionsPe, nowEmptyTransactionsPe, e)) {
				...
				int utilityOfRemainingItemsJumpingClosure = useUtilityBinArraysToCalculateUpperBounds(transactionsPe, j,
						itemsToKeep);
				...
				if (shouldJumpToClosure) {
					...
					if (activateSubtreeUtilityPruning) {
						...
						recursiveSupport = backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToExplore,
								prefixLength + 1);
					} else {
						// if sub-tree utility pruning is deactivated, we consider
						// secondary items also as primary items
						recursiveSupport = backtrackingEFIM(transactionsPe, newItemsToKeep, newItemsToKeep,
								prefixLength + 1);
					}
	...
	private int[][] mergeOriginalTransactions(Transaction transaction1, Transaction transaction2) {
		...
		System.arraycopy(transaction1.originalTransactions, 0, mergeOriginalTransactions, 0,
				transaction1.originalTransactions.length);
		System.arraycopy(transaction2.originalTransactions, 0, mergeOriginalTransactions,
				transaction1.originalTransactions.length, transaction2.originalTransactions.length);
	 ...
	 * @param transactionsPe       the projected transactions containing the prefix
	 *                             P U {e} that are not empty
	 * @param nowEmptyTransactions the projected transactions containing the prefix
	 *                             P U {e} that are empty
	 * @param e                    the item "e"
	 * @return true if P U {e} has no backward extension
	 */
	private boolean hasNoBackwardExtension(int[] prefix, int prefixLength, List<Transaction> transactionsPe,
		...
		for (int item : firstTrans) {
			...
			if (!containsByBinarySearch(prefix, prefixLength, item) && isItemInAllTransactions(transactionsPe, item)
	...
	/**
	 * Check if an item appears in all transactions after the first one in a list of
	 * transactions
	 * 
	 ...
	 * @return
	 */
	private int useUtilityBinArraysToCalculateUpperBounds(List<Transaction> transactionsPe, int j,
