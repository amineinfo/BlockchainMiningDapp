...
/**
 * This is an implementation of the "HAUI-Miner Algorithm" for High
 * Average-Utility Itemsets Mining as described in the paper : Jerry Chun-Wei
 * Lin, Ting Li, Philippe Fournier-Viger, Tzung-Pei Hong, Justin Zhan, and
 * Miroslav Voznak. An Efficient Algorithm to Mine High Average-Utility
 * Itemsets[J]. Advanced Engineering Informatics, 2016, 30(2):233-243
 *
 * @see UtilityList
 * @see Element
 * @author Ting Li
			...
			for (int i = 0; i < revisedDatabase.get(tid).size(); i++) {
				...
				if (revisedDatabase.get(tid).get(i).item == item) {
					...
					for (int j = i; j < revisedDatabase.get(tid).size(); j++) {
						...
						// create a new Element to the utility list of this item corresponding to this
						// transaction
