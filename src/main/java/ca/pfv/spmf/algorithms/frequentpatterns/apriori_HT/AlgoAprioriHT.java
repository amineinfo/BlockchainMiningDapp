 ...
 * store candidates, to calculate their support and to generate candidates
 * efficiently. The other version (AlgoApriori) do not use a hash tree. <br/>
 * <br/>
 * 
 ...
 * <br/>
 * 
 * Note that the performance of the Hash-Tree version of Apriori depends on the
 * BRANCH COUNT value in the class ItemsetHashTree. In my test, I have used a
 * value of 30 because it seems to provide the best results. But other values
 * could also be used (see Agrawal & Srikant for details). To change the
 * branch_count variable, see the ItemsetHashTree class (default is 30).
 * 
 * 
 * @see Itemset
...
public class AlgoAprioriHT {
	 ...
	 */
	public void runAlgorithm(double minsup, String input, String output, int hash_tree_branch_count)
			throws IOException {
		// record start time
