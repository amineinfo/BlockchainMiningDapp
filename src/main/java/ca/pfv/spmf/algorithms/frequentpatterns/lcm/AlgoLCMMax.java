...
/**
 * This is an implementation of the LCMMax algorithm for mining frequent maximal
 * itemsets from a transaction database. More information on the LCM algorithm
 * can be found in papers by T. Uno, such as: <br/>
 * <br/>
 * 
 * T. Uno, M. Kiyomi, and H. Arimura. Lcm ver. 2: Efficient mining algorithms
 * for frequent/closed/maximal itemsets. In FIMI, 2004
 * 
 * This implementation of LCMMax was made by Alan Souza and was modified by
 * Philippe Fournier-Viger to add optimizations and support for LCMFreq/LCMMax
 * (note LCMMax has been temporarily removed in the current version of SPMF).
 * <br/>
 * 
 * The implementation is similar to LCM version 2 with some differences. For
 * example, transaction merging is not performed yet and items in transactions
 * are not sorted in descending order of frequency.
 *
...
public class AlgoLCMMax {
	 ...
	 * Run the algorithm
	 * 
	 * @param minimumSupport          the minimum support threshold as percentage
	 *                                value between 0 and 1
	 * @param dataset                 the dataset
	 * @param outputPath              the output file path to save the result or
	 *                                null if to be kept in memory
	...
	public Itemsets runAlgorithm(double minimumSupport, Dataset dataset, String outputPath) throws IOException {
		...
		// Since it is the empty set, we will have all transactions and no frequency
		// count
	 ...
	 */
	private boolean backtrackingLCMMax(List<Integer> p, List<Transaction> transactionsOfP, List<Integer> frequentItems,
			int tailPosInP, Integer itemELastAddedToP) throws IOException {

		throw new RuntimeException("This algorithm is unavailable in the current version of SPMF. \n");
		/*
		 * boolean foundOneMaxItemset = false;
		 * 
		 * // ======== for each frequent item e ============= for (int j = 0; j <
		 * frequentItems.size(); j++) { Integer e = frequentItems.get(j);
		 * System.out.println(" p : " + p + " \te: " + e); // if the item is not already
		 * in p before the current tail position // we will consider it to form a new
		 * itemset if(e <= itemELastAddedToP || (p != null && containsByBinarySearch(p,
		 * e, tailPosInP))) { // #DIFF continue; }
		 * 
		 * // Calculate transactions containing P U e // At the same time truncate the
		 * transactions to keep what appears after "e" List<Transaction> transactionsPe
		 * = intersectTransactions(transactionsOfP, e); //ok
		 * 
		 * //====== Check if PU{e...} is a ppc extension ====== if (isPPCExtension(p,
		 * transactionsPe, e)) { // ======= Create a closed itemset using PU{e...} =====
		 * // First add all items from PU{e} List<Integer> itemset = new
		 * ArrayList<Integer>(); if(p != null) { //add every item i of p such that i < e
		 * to the itemset for (int m = 0; m < p.size() && p.get(m) < e; m++) {
		 * itemset.add(p.get(m)); } } itemset.add(e); int tailPositionInPe =
		 * itemset.size()-1;
		 * 
		 * for (int k = j+1; k < frequentItems.size(); k++) { Integer itemk =
		 * frequentItems.get(k); // for every item i > e add if it is in all
		 * transactions of T(P U e) // ### FIRST DIFFERENCE WITH LCM ###
		 * if(isItemInAllTransactions(transactionsPe, itemk)) { itemset.add(itemk); } }
		 * 
		 * /// #### SECOND DIFFERENCE #### // int itemsAddedAfterECount = itemset.size()
		 * - (tailPositionInPe + 1);
		 * 
		 * // ===== save the frequent closed itemset int supportPe =
		 * transactionsPe.size();
		 * 
		 ...
		 * ============ // Get all frequent items e such that e > tailOfP // (i.e. "e"
		 * appears after the position of the tail item in the list of all items)
		 * List<Integer> newFrequentItems = new ArrayList<Integer>(); for (int k = 0; k
		 * < frequentItems.size(); k++) { /// ##### ANOTHER DIFFERENCE START FROM K = 0
		 * Integer itemK = frequentItems.get(k); int supportK = buckets[itemK].size();
		 * if(supportK >= minsupRelative) { newFrequentItems.add(itemK); } }
		 * 
		 * // ===== ### SECOND DIFFERENCE WITH LCM #### ===== // int
		 * itemsAddedAfterEcount = (itemset.size() - p.size());
		 * System.out.println(itemset); if(newFrequentItems.size() == itemset.size()) {
		 * output(itemset, supportPe); System.out.println("OUTPUT " + itemset);
		 * foundOneMaxItemset = true; }else { // #DIFF : ELSE // === recursive call
		 * boolean found = backtrackingLCMMax(itemset, transactionsPe, newFrequentItems,
		 * tailPositionInPe, e); // ### DIFF : PASS E if(found == false) { // ## DIFF
		 * System.out.println("OUTPUT2 " + itemset); output(itemset, supportPe); } } } }
		 * 
		 * MemoryLogger.getInstance().checkMemory(); return foundOneMaxItemset; // ##
		 * DIFF
		 */
	}

	/**
	 * Perform the initial occurence delivery with the original dataset containing
	 * all items
	 * 
	 * @param dataset
	 */
	...
	private void anyTimeDatabaseReductionMax(List<Transaction> transactionsPe, int j, List<Integer> frequentItems,
			List<Integer> itemset, Integer e) {
	...
	/**
	 * Check if a given itemset PUe is a PPC Max extension according to the set of
	 * transactions containing PUe.
	 * 
	 ...
	 * @param item         an item
	 * @return true if the item appears in > minsup-1 transactions after the first
	 *         one
	 */
	...
	/**
	 * Save a frequent closed itemset to file or memory depending on what the user
	 * chose.
	 * 
