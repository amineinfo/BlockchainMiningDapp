...
/**
 * This is an implementation of the "TSHOUN" algorithm for High on-shelf utility
 * itemsets Mining while considering negative profit value. TSHOUN is described
 * in this paper: <br/>
 * <br/>
 * 
 * Guo-Cheng Lan, Tzung-Pei Hong, Jen-Peng Huang, Vincent S. Tseng: On-shelf
 * utility mining with negative item values. Expert Syst. Appl. 41(7): 3450-3459
 * (2014)<br/>
 * <br/>
 * 
 * 
 * WARNING : THIS CODE PROBABLY ASSUME THAT TRANSACTIONS ARE SORTED IN
...
//	private HashTable highUtilityItemsets = null;
	 ...
	 * @throws IOException
	 */
	public void runAlgorithm(DatabaseWithPeriods database, double minUtilityRatio, String output, int periodCount)
			throws Exception {
		// save the parameters
		...
		for (Entry<Integer, Pair> entry : database.getMapItemExactEstUtility().entrySet()) {
			...
			for (int i = periodsOfItem.nextSetBit(0); i >= 0; i = periodsOfItem.nextSetBit(i + 1)) {
...
//			// IF IT IS NOT A CANDIDATE THEN WE WILL NOT CONSIDER THIS ITEM ANYMORE
		...
		// ===== OPTIMIZATION: Scan the database once more to remove unpromising items
		// AND
		// === delete transactions containing only unpromising items AND
		// ==== adjust the transaction utilities by removing utility of unpromising
		// items ===/
		if (DEBUG) {
...
//				}
				...
				for (int i = 0; i < estimatedUtilityIJ.length; i++) {
					...
					if (estimatedUtilityIJ[i] != null) {
						int twuIJ = 0;
						if (isPromisingInAtLeastOnePeriod == false) {
							if (calculateRelativeUtility(periodUtilities.get(i),
									estimatedUtilityIJ[i]) >= minUtilityRatio) {
								isPromisingInAtLeastOnePeriod = true;
							}
						}
...
//					+ "   periodEnd : " + database.getTransactions().get(endIndex-1).getPeriod());
		...
		// ======= SCAN THE DATABASE TO CALCULATE THE EXACT UTILITY OF CANDIDATES IN
		// HASHTABLE ========
...
//				// for each period containing the itemset
				loop: for (int period = periods.nextSetBit(0); period >= 0; period = periods.nextSetBit(period + 1)) {
					// sum the period utility
					sumPeriodUtility += periodUtilities.get(period);

					// check if the utility of the itemset in that period has been already been
					// calculated
...
//					System.out.println(Arrays.toString(itemset.items) + " " + exactUtility);
	...
	/**
	 * Perform a binary search on the list of transaction to find the first
	 * transaction of a given period
	 * 
	...
	/**
	 * This method search for the last transactions of a given period in a list of
	 * transactions
	 * 
	 * @param keyPeriod the period to be searched
	 * @param list      the list of transactions
	 * @param mid       the current position
	 * @return -1 if the current position is smaller than the position that we
	 *         search, 0 if equal, or 1 otherwise
	 */
	 ...
	 * @param candidates2   the candidates of size 2 that should be used for
	 *                      starting the mining
	 * @param period        the id of the period
	 */
	public void performMiningOnPeriod(List<TransactionWithPeriod> database, int periodUtility,
			List<ItemsetTP> candidates2, short period) {
...
//				}
	...
	/**
	 * Calculate the exact and estimated utility of each candidate in the list of
	 * candidates
	 * 
	 ...
	 */
	private void pruneCandidatesAndCalculateExactUtility(List<TransactionWithPeriod> database, int periodUtility,
			short period, List<int[]> candidates) {
...
//			// else we keep it
	...
	/**
	 * Get the utility of an itemset in a transaction It assumes that itemsets are
