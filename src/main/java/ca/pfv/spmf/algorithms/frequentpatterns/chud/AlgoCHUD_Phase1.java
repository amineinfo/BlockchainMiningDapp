...
/**
 * This class implements the Phase 1 of the CHUD algorithm for closed high
 * utility itemset mining. The CHUD algorithm was published in the following
 * paper: <br/>
 * <br/>
 * 
 ...
 * Proceedings of the 11th IEEE Intern. Conference on Data Mining (ICDM 2011).
 * IEEE CS Press, pp.824-833.
 * 
 * @see AlgoCHUD
 */
class AlgoCHUD_Phase1 {
	...
	/**
	 * if true, the strategy "removing items that has been processed and thier
	 * utilities from golbal utiltiy table" is activated
	 */
	boolean useStrategy2 = true;
	/**
	 * if true, the strategy " removing MIU (Minimum item utility) from the local
	 * table" is activated
	 */
	boolean useStrategy3 = true;
	/**
	 * if true, the strategy " Discard candidates whose MAU is smaller than
	 * min_utility " is activated
	 */
	...
	public void runAlgorithm(int minUtility, String filePathInput1, String filePathInput2, String filePathInput3,
			String output1) throws IOException {
		...
		// ---------------------------------------------------
		// (B) WE SCAN FILE 2 CONTAINING THE TID OF EACH TRANSACTION AND ITS TRANSACTION
		// UTILITY.
		...
		// (C) WE SCAN THE VERTICAL DATABASE (FILE 1) to calculate the exact utility of
		// each item
		...
		// (D) We recalculate TWU with only items of size 1 that have minimum estimated
		// utility
...
//		printStatistics();
	...
	private void recalculateTU(Set<Integer> itemsK1, Set<Integer> itemsK2, String filePathInput1, int[] tableTU)
	...
	private void chud_phase1(boolean firstTime, List<Integer> closedset, Set<Integer> closedsetTIDs,
			List<Integer> postset, List<Integer> preset, int[] tableTU, int[] tableMin, int[] tableMax, int level)
			throws IOException {
...
//					}
					...
					chud_phase1(false, closedsetNew, closedsetNewTIDs, postsetNew, presetNew, tableTUnew, tableMin,
							tableMax, level + 1);
	...
	/**
	 * Check if an item is smaller than another according to the support ascending
	 * order or if the support is the same, use the lexicographical order.
	 */
