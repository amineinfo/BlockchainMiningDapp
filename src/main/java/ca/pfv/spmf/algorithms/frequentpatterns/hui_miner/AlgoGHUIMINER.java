...
/**
 * This is an implementation of the "GHUI-MINER" algorithm for High-Utility
 * Itemsets Mining as described in the conference paper : <br/>
 * <br/>
 * 
 * Fournier-Viger, P., Wu, C.W., Tseng, V.S. (2014). Novel Concise
 * Representations of High Utility Itemsets using Generator Patterns. Proc. 10th
 * Intern. Conference on Advanced Data Mining and Applications (ADMA 2014),
 * Springer LNCS 8933, pp. 30-43.
 * 
 * @see UtilityList
 * @see Element
...
public class AlgoGHUIMINER {
	...
	/**
	 * number of times that the generator check was stoped early due to
	 * optimizations
	 */
	...
	/**
	 * NEW OPTIMIZATION - Structure used by the EUCP strategy Key: for an item x,
		...
		for (int i = prefixLength; i < closedItemsetsBySize.size(); i++) {
			...
			if (list != null) {
				...
				for (Itemset itemsetInList : list) {
					...
					if (support == itemsetInList.support
							&& ArraysAlgos.includedIn(itemsetX, prefixLength, itemsetInList.itemset)) {
	...
	/**
	 * This method checks if a given itemset is a subset or a strict subset of a
	 * CHUI.
	 * 
	 * @param itemset           the itemset
	 * @param support           the itemset support
	 * @param prefixLength      the prefix length
	 * @param strictSubsetCheck if true, this method check for "strict subset of".
	 *                          Otherwise, it checks for "subset of"
	 * @return true if the itemset is a subset (strict subset) of a CHUI. Otherwise
	 *         false.
	 */
...
//					return o2.support - o1.support;  // DECREASING SUPPORT ORDER
	...
	/**
	 * Implementation of Insertion sort for sorting an itemset by TWU order This has
	 * an average performance of O(n log n)
	 * 
	 ...
	 */
	public void runAlgorithm(String input, String output, int minUtility, List<List<Itemset>> closedItemsets,
			Set<Integer> itemsInClosedItemsets) throws IOException {
		...
		// initialize the structured for EUCP strategy introduced in FHM algorithm
	 ...
	 * @param p_UL          This is the Utility List of the prefix. Initially, it is
	 *                      empty.
	 * @param extensionsULs The utility lists corresponding to each extension of the
	 *                      prefix.
		...
		for (int i = 0; i < extensionsULs.size(); i++) {
			...
			if (pX_UL.sumIutils + pX_UL.sumRutils >= minUtility) {
				...
				for (int j = i + 1; j < extensionsULs.size(); j++) {
					...
					UtilityListWithCriticalObjects pXYUL = (p_UL.item == null) ? construct(pX_UL, pY_UL, minUtility)
							: construct(pX_UL, pY_UL, minUtility);
					...
					// if the pattern has the same support as PX or PY, it is not a generator, WE
					// SKIP IT
...
//					int[] append = ArraysAlgos.appendIntegerToArray(newPrefix, pY_UL.item);
					...
					if (isGenerator) {
						...
						if (pXYUL.sumIutils >= minUtility) {
							...
							writeOut(itemsetBuffer, prefixLength + 1, pXYUL.item, pXYUL.sumIutils,
									pXYUL.elements.size());
				...
				// We make a recursive call to discover all itemsets with the prefix pXab by
				// combining
	 ...
	 */
	private boolean checkIfGenerator(UtilityListWithCriticalObjects p_UL, UtilityListWithCriticalObjects pX_UL,
			int prefixSize) {
...
// =================================================================
	...
	/**
	 * This method checks if an itemset X is a generator (optimized for the case of
	 * an itemset containing a single item)
	 * 
	...
	/**
	 * This method constructs the utility list of itemset {x,y} using the utility
	 * lists of itemsets {x} and {y} (done as in HUI-Miner).
	 * 
	 ...
	 */
	private UtilityListWithCriticalObjects construct(UtilityListWithCriticalObjects px,
			UtilityListWithCriticalObjects py, int minUtility) {
	...
	public void printStats() throws IOException {
		...
		System.out.println("   Candidate count : " + candidateCount + "     (avoided by FHM : " + candidateAvoidedbyFHM
				...
				+ generatorChecks + "   (partially avoided : " + partiallyAvoidedOrAvoidedGeneratorChecks + ")");
