 ...
 * <br/>
 * 
 * and the original Apriori article:<br/>
 * <br/>
 * 
 ...
 * <br/>
 * 
 * Note that unlike AlgoAprioriInverse, this implementation is based on
 * APRIORI-TID rather than the regular Apriori. This means that the algorithm
 * counts the support itemsets by using transaction identifiers lists
 * (tid-lists) rather than scanning the database. The AprioriInverse algorithm
 * finds all perfectly rare itemsets. A perfectly rare itemset is an itemset
 * such that all its subsets are rare. It is very similar to the original
 * Apriori algorithm. The main difference is that it uses a threshold "maxsup"
 * and that frequent items of size 1 need to have a support not higher than
 * maxsup.<br/>
 * <br/>
 * 
...
public class AlgoAprioriTIDInverse {
	...
	/**
	 * variable to store the result if the user choose to save to memory instead of
	 * a file
	 */
	...
	/**
	 * the current transaction database, if the user has provided one instead of an
	 * input file.
	 */
	 ...
	 * 
	 * @param input  the file path of an input file. if null, the result is returned
	 *               by the method.
	 * @param output the output file path
	 * @param minsup the minimum support threshold as a percentage (double)
	 * @param maxsup the maximum support value as a percentage (double)
	 * @return if no output file path is provided, the method return frequent
	 *         itemsets, otherwise null
	 * @throws IOException exception if error reading/writing the file
	 */
	public Itemsets runAlgorithm(String input, String output, double minsup, double maxsup)
		...
		while (iterator.hasNext()) {
			...
			if (entry.getValue().size() >= minSuppRelative && entry.getValue().size() <= maxSuppRelative) { // THE ONLY
																											// BIG
																											// DIFFERENCE
																											// WITH
																											// APRIORI
	...
	/**
	 * Save a frequent itemset to the output file or memory, depending on what the
	 * user chose.
	 * 
