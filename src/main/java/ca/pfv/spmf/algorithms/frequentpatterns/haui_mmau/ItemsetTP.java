...
/**
 * This class represents an itemset (a set of items) with average utility
 * information found by the HAUI-MMAU algorithm.
 *
 * @see AlgoHAUIMMAU
 * @author Ting Li, 2016
 */
	...
	public float getItemsetMau(Map<Integer, Integer> mutipleMinUtilities, int GLMAU) {
		...
		for (int i = 0; i < items.size(); i++) {
			if (GLMAU > mutipleMinUtilities.get(items.get(i)))
				...
				sumMau += mutipleMinUtilities.get(items.get(i));
		}
