...
/**
 * This is an implementation of the IHUP algorithm that is designed to be run as
 * a batch algorithm rather than an incremental algorithm. <\br><\br>
 * 
 ...
 * Incremental Databases. IEEE Trans. Knowl. Data Eng. 21(12): 1708-1721
 * (2009)<\br><\br>
 * 
		...
		for (int i = tree.headerList.size() - 1; i >= 0; i--) {
			...
			if (DEBUG) {
				System.out.println("LOCAL TREE for projection by:"
						+ ((prefix == null) ? "" : Arrays.toString(prefix) + ",") + item + "\n" + localTree.toString());
			}
...
//			int itemCPB = pathCPB.itemID;
	...
	/**
	 * Update the exact utility of an itemset given a transaction It assumes that
	 * itemsets are sorted according to the lexical order.
	 * 
	...
	private void checkMemory() {
		...
		// if higher than the maximum until now replace the maximum with the current
		// memory usage
