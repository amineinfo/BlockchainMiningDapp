...
/**
 * This is an implementation of the CORI algorithm to mine correlated itemsets.
 * CORI is actually a quite simple extension of ECLAT. However, it is not
 * explained in the paper describing CORI that it extends ECLAT.
 * 
 * CORI was proposed by Bouasker et Yahia (2015). <br/>
 * <br/>
 * 
 * See this article for details about CORI: <br/>
 * <br/>
 * 
 * Bouasker, S., Yahia, S. B. (2015). Key correlation mining by simultaneous
 * monotone and anti-monotone constraints checking. Proc. of the 2015 ACM
 * Symposium on Applied Computing (SAC 2015), pp. 851-856. <br/>
 * <br/>
 * 
...
public class AlgoCORI {
	 ...
	 */
	public ItemsetsCORI runAlgorithm(String output, TransactionDatabase database, double minsupp, double minBond,
...
//			}
		...
		if (maxItemsetSize >= 2) {
			...
			for (int i = 0; i < singleItems.size(); i++) {
				...
				// The third structure stores the bitset of conjunctive support of each
				// 2-itemset
				...
				if (equivalenceClassIitems.size() > 0) {
					...
					processEquivalenceClass(itemsetBuffer, 1, equivalenceClassIitems, equivalenceClassItidsets,
							equivalenceClassConjunctiveItidsets);
				}
			}
		}
	 ...
	 */
	int calculateSupportSingleItems(TransactionDatabase database, final Map<Integer, BitSetSupport> mapItemTIDS) {
	 ...
	 * @param tidsetI   the first tidset
	 * @param tidsetJ   the second tidset
	 * @param supportIJ the support of the intersection (already known) so it does
	 ...
	 * itemsets,
	 * 
	 * @param prefix                              a common prefix to all itemsets of
	 *                                            the equivalence class
	 * @param equivalenceClassItems               a list of suffixes of itemsets in
	 *                                            the current equivalence class.
	 * @param equivalenceClassTidsets             a list of tidsets of itemsets of
	 *                                            the current equivalence class.
	 * @param equivalenceClassConjunctiveItidsets the bitsets of conjunctive support
	 *                                            for itemsets of the current
	 *                                            equivalence class.
	...
	private void processEquivalenceClass(int[] prefix, int prefixLength, List<Integer> equivalenceClassItems,
			List<BitSetSupport> equivalenceClassTidsets, List<BitSetSupport> equivalenceClassConjunctiveItidsets)
			throws IOException {
...
//		insertionSort(equivalenceClassItems, equivalenceClassTidsets);
			...
			if (prefixLength + 2 <= maxItemsetSize) {
				...
				if (equivalenceClassISuffixItems.size() > 0) {
					...
					processEquivalenceClass(prefix, newPrefixLength, equivalenceClassISuffixItems, equivalenceITidsets,
							equivalenceConjunctiveITidsets);
				}
			}
		}
	 ...
	 */
	private void save(int[] prefix, int prefixLength, int suffixItem, BitSetSupport tidset, double bond)
			throws IOException {
	...
	/**
	 * Anonymous inner class to store a bitset and its cardinality (an itemset's
	 * tidset and its support). Storing the cardinality is useful because the
	 * cardinality() method of a bitset in Java is very expensive, so it should not
