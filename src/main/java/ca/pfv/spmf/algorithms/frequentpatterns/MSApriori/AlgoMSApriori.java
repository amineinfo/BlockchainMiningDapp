...
/**
 * This is an implementation of the MSApriori algorithm as described by :<br/>
 * <br/>
 * 
 * Bing Liu et al. (1999). Mining Association Rules with Multiple Minimum
 * Supports, Proceedings of KDD 1999. <br/>
 * <br/>
 * 
 * This implementation was made by AZADEH SOLTANI based on the Apriori
...
public class AlgoMSApriori {
	...
	/**
	 * the array of MIS value where the position i indicate the MIS of item with ID
	 * i.
	 */
	int MIS[];
	...
	/**
	 * an in-memory representation of the transaction database // where position i
	 * represents the ith transaction as an integer array
	 */
	 ...
	 * @param beta   the parameter Beta for generating MIS values for all items (see
	 *               paper)
	 * @param LS     the parameter LS for generating MIS values for all items (see
	 *               paper)
	...
	public void runAlgorithm(String input, String output, double beta, double LS) throws IOException {
		...
		for (Entry<Integer, Integer> entry : mapItemCount.entrySet()) {
			// add the item to M
			M.add(entry.getKey());
			// calculate the MIS value for that item by using the formula described in the
			// paper
	 ...
	 * @param mapItemCount a map indicating the support of each item (key: item,
	 *                     value: support)
	...
	protected List<Itemset> generateCandidateSizeK(List<Itemset> levelK_1) {
		...
		loop1: for (int i = 0; i < levelK_1.size(); i++) {
			...
			loop2: for (int j = i + 1; j < levelK_1.size(); j++) {
				...
				// itemset1 is smaller than the last item of itemset2, we will combine them to
				// generate a
				// candidate
	 ...
	 * @param postRemoved a position that should be ignored from itemset "candidate"
	 *                    for the comparison
	 * @return 0 if they are the same, <0 if "itemset" is smaller than candidate
	 *         according to the MIS order, otherwise >0
	 */
	...
	public void printStats() {
		...
		System.out.println(" The algorithm stopped at level " + (k - 1) + ", because there is no candidate");
