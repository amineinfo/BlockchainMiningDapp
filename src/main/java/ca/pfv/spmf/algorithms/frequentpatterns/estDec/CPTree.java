...
public class CPTree {
	...
	/********************************************************************
	 * Method for inserting new possible frequent itemsets to the tree based on the
	 * new transaction (Phase 3 : Insertion Phase).
	 * 
	 * @param transaction the new transaction m
	 *********************************/
	...
	double getCountOfItemset(int[] itemset) {
		...
		while (true) {
			if (ind >= itemset.length)
				break;
			oldPInd = parentInd;
			parentInd = currentNode.getInnerIndexWithID(itemset[ind], parentNode, parentInd);
			if (parentInd != -1) {
				ind++;
				l++;
				continue;
	 ...
	 * 
	 * @param currentNode , transaction, index
	 ********************************************************************/
	 ...
	 * @param PI          the inner parent index
	 * 
	 * @param transaction the transaction
	 * @param ind         the depth of the current node with respect to the root of
	 *                    the tree
	 * @param itemset
	 * @param length      the length of the itemset
	 * 
	 ********************************************************************/

	public void insert_n_itemsets(CPTreeNode currentNode, short PI, List<Integer> transaction, int ind, int[] itemset,
			int length) {
		// stop recursion
			...
			if (child != null)
				...
				if (currentNode.counter1 / N >= minsig) {
					...
					if (c / N >= minsig) {
						...
						if (((currentNode.counter1 - child.counter2) / N) < delta && (child.counter2 / N) > minmerg)
...
//		int l = m.itemIDList.size();
	...
	public void traverse(CPTreeNode m, CPTreeNode mp, int q, int[] transaction) {
		...
		if (m.counter1 / N < minsig) {
			...
			if (m.isLeafLevel(0))
				...
				levelParents.add(0);
				while (true) {
					levelParents = FindLevelCommonItems(m, levelParents, leafCommonItemInds, transaction);
			...
			if ((mp.counter1 - m.counter2) / N < delta && m.counter2 / N >= minmerg) {
				if (mp != root)
					merge(mp, m);
			} else if ((m.counter1 - m.counter2) / N > delta && m.counter2 / N >= minmerg && m.itemIDList.size() > 1)
				split(m);
	 ...
	 * 
	 */
	List<Integer> FindLevelCommonItems(CPTreeNode m, List<Integer> levelParents, List<Integer> leafCommonItemInds,
			int[] transaction) {
