 ...
 * (a version of Apriori implemented with a hash tree). <br/>
 * <br/>
 * 
...
public class ItemsetHashTree {
	...
	/**
	 * Inserts an itemset in the hash-tree (this is called recursively to search
	 * where to insert the itemset)
	 * 
	...
	/**
	 * This method increase the support count of all itemsets contained in the
	 * hash-tree that are contained in a transaction.
	 * 
	...
	/**
	 * Recursive method for increasing the support count of all itemsets contained
	 * in the hash-tree that are contained in a transaction.
	 * 
	 * @param transaction          the transaction
	 * @param node                 the current node that is explored
	 * @param firstPositionToCheck the current position in the transaction to be
	 *                             explored
	...
	private void updateSupportCount(int[] transaction, InnerNode node, int firstPositionToCheck, int[] prefix) {
		...
		// the index of the last item that can be the first item in lexical order of an
	 ...
	 * @param posRemoved the position of an item that should be ignored in this
	 *                   itemset
			...
			if (count == itemsetSize) {
				...
				while (first <= last) {
					...
					if (ArraysAlgos.sameAs(list.get(middle).getItems(), itemset, posRemoved) < 0) {
						first = middle + 1; // the itemset compared is larger than the subset according to the lexical
											// order
					} else if (ArraysAlgos.sameAs(list.get(middle).getItems(), itemset, posRemoved) > 0) {
						last = middle - 1; // the itemset compared is smaller than the subset is smaller according to
											// the lexical order
					} else {
	...
	/**
	 * A method that check if an itemset is equal to another itemset called "prefix"
	 * + itemI + itemJ
	 * 
