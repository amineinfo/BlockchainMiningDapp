 ...
 * <br/>
 * 
 * Duong, Q.H., Fournier-Viger, P., Ramampiaro, H., Norvag, K. Dam, T.-L.
 * (2017). Effcient High Utility Itemset Mining using Buffered Utility-Lists.
...
public class AlgoULBMiner {
	...
	public void runAlgorithm(String input, String output, int minUtility) throws IOException {
		...
		for (Integer item : promisingItems) {
			...
			// For the current item, we know that the number of tids will be equal to its
			// support.
...
//		// if the same, use the lexical order otherwise use the TWU
	 ...
	 * @throws IOException
	 */
	private void fhm(int[] prefix, int prefixLength, int pULPosition, int previousStartPosition,
			int previousEndPosition, int minUtility) throws IOException {
		...
		for (int X = previousStartPosition; X < previousEndPosition; X++) {
			...
			if (sumIutils + sumRutils >= minUtility) {
				...
				for (int Y = X + 1; Y < previousEndPosition; Y++) {
					...
					boolean succeeded = construct(pULPosition, X, Y, minUtility, newEndPosition, itemY,
							sumIutils + sumRutils);
					if (succeeded == true) {
						newEndPosition++;
					}
	...
	private boolean construct(int PPosition, int pXPosition, int pYPosition, int minUtility, int endPosition, int itemY,
			int totalUtility) {
		...
		int posP = 0;
		/*
		 * To specify the common of 3 sorted arrays, we use 3 index pointers. Not using
		 * binary search as original methods The Steps are follows. 1. Starting with 3
		 * pointers point to the first index of arrays. 2. 2 pointers of 2 arrays move
		 * step by step until the values in 2 arrays are equal. 3. The 3rd pointer then
		 * move to the index having tid equal to the current tid in 2 arrays above 4.
		 * Loop will be terminated when 2 pointers point to the last position in arrays
		 */
		while (posX < countX && posY < countY) {
			...
			if (ex.tid < ey.tid) {
				...
				utilityListBuffer.addElementToCurrentUtilityList(ex.tid, ex.iutils + ey.iutils - epIutil, ey.rutils);
				posX++;
				posY++;
			}
		}
