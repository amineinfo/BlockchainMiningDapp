 ...
 * - intersecting bit by bit and stop at first different bit for inclusion check
 * (similar to what is described in the TKDE paper, but check bits instead of
 * words) <br/>
 * <br/>
 * 
 * But more optimizations could be done:<br/>
 * - intersecting word by word and stop at first different word for inclusion
 * check (described in the TKDE paper)<br/>
 * - reorder columns of the matrix (described in the TKDE paper)<br/>
 * - reusing results of previous bitwise intersections (described in the TKDE
 * paper)<br/>
 * - changing for a breath-first DCI-like approach for dense datasets (as
 * described in the TKDE paper)<br/>
 * - ...<br/>
 ...
 * instead of the BitSet class of Java, because the BitSet class of Java does
 * not let us iterate over the words inside the BitSet directly.
 *
 * @see BitMatrix
...
public class AlgoDCI_Closed_Optimized {
	...
	/**
	 * Scan database to know the database size and number of items to initialize the
	 * bit matrix.
	 * 
	 ...
	 */
	private void dci_closed(boolean firstTime, List<Integer> closedset, BitSet bitset, List<Integer> postset,
			List<Integer> preset, BitMatrix matrix, BitMatrix originalMatrix) throws IOException {
			...
			if (newgenTIDs.cardinality() >= minSuppRelative) {
				...
				if (isDuplicate(newgenTIDs, preset, matrix) == false) {
					...
					if (firstTime) {
						...
						dci_closed(false, closedsetNew, closedsetNewTIDs, postsetNew, presetNew, matrix,
								originalMatrix);
					}
	...
	/**
	 * Project the bitmatrix with a given bitset. This removes all the columns in
	 * the bitmatrix that do not contain a 1 in the given bitset.
	 * 
	...
	/**
	 * Checks if the TIDs set represented by bs1 is included in the TIDs set
	 * represented by bs2.
	 * 
