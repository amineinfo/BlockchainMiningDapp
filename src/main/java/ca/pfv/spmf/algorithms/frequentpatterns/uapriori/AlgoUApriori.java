...
public class AlgoUApriori {
	...
	/**
	 * variable indicating the current level for the Apriori generation // (itemsets
	 * of size k)
	 */
	protected int k;
	...
	public void runAlgorithm(double minsupp, String output) throws IOException {
		...
		Set<ItemsetUApriori> level = createLevelWithFrequentCandidates(minsupp, candidatesSize1);
		...
		while (!level.isEmpty() && k <= maxItemsetSize) {
			...
			Set<ItemsetUApriori> levelK = createLevelWithFrequentCandidates(minsupp, candidatesK);
	 ...
	 * keep only the itemset meeting that minimum threshold.
	 * 
	 ...
	 */
	protected Set<ItemsetUApriori> createLevelWithFrequentCandidates(double minsupp, Set<ItemsetUApriori> candidatesK)
			throws IOException {
