 ...
 * rare itemset is an itemset such that all its subsets are rare. It is very
 * similar to the original Apriori algorithm. The main difference is that it
 * uses a threshold "maxsup" and that frequent items of size 1 need to have a
 * support not higher than maxsup.<br/>
 * <br/>
 * 
...
public class AlgoAprioriInverse {
	 ...
	 * @throws IOException exception if error while writing or reading the
	 *                     input/output file
	 */
	public Itemsets runAlgorithm(double minsup, double maxsup, String input, String output) throws IOException {
		...
		for (Entry<Integer, Integer> entry : mapItemCount.entrySet()) {
			if (entry.getValue() >= minsupRelative && entry.getValue() <= maxsupRelative) { // THE ONLY BIG DIFFERENCE
																							// WITH APRIORI IS HERE...
																							// WE CHECK MAXSUP FOR
																							// ITEMSETS OF SIZE 1
	...
	/**
	 * Save an itemset of size 1 to the file or to memory if no output file path was
	 * provided
	 * 
