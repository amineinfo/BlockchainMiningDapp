 ...
 * erasable itemset mining.<br/>
 * <br/>
 * 
 * The VME algorithm finds all the ereasable itemsets from a product
 * database.<br/>
 * <br/>
 * 
 * Actually, this algorithms is a only slight modification of the AprioriTID
 * algorithm.<br/>
 * <br/>
 * 
 * I have implemented mostly as described in the paper with some modifications
 * to make it more efficient.<br/>
 * First, the authors suggested to generate all candidates of a level before
 * removing the unereasable ones. This is inefficient. Instead, in my
 * implementation, I check the "gain" (loss of profit) directly after generating
 * a candidate so I can eliminate them right away. <br/>
 * Second, it is unecessary to check the subsets like the authors suggest
 * because they use a vertical representation. <br/>
 * Third, the authors suggest to store the profit of transactions in PID List.
 * This is not memory efficient. For implementation it is better to store the
 * profit of each transaction only once in a hashtable.
 * 
 * @see Itemset
...
public class AlgoVME {
	...
	/**
	 * variables for counting support of items // key: item value: tidset of the
	 * item as a set of integers
	 */
	Map<Integer, Set<Integer>> mapItemTIDs = new HashMap<Integer, Set<Integer>>();

	/**
	 * variables for storing the profit of each transaction // key: transaction id
