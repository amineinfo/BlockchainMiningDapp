 ...
 * <br/>
 * 
 * Fournier-Viger, P., Lin, C.W., Duong, Q.-H., Dam, T.-L. (2016). FHM+: Faster
 * High-Utility Itemset Mining using Length Upper-Bound Reduction. Proc. 29th
 * Intern. Conf. on Industrial, Engineering and Other Applications of Applied
 * Intelligent Systems (IEA AIE 2016), Springer LNAI, to appear
 *
...
public class AlgoFHMPlus {
	...
	/**
	 * this variable indicate to use the alphabetical order instead of the ascending
	 * order of RTWU
	 */
	 ...
	 */
	public void runAlgorithm(String input, String output, int minUtility, int minimumLength, int maximumLength)
			throws IOException {
		// reset maximum
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				for (int i = 0; i < items.length; i++) {
					...
					// if the item is promising, we add it to the buffer for storing elements in the
					// transactions
					// otherwise, we do not.
					if (mapItemToTWU.get(item) >= minUtility) {
						// convert it to int
...
//			}
	 ...
	 * @throws IOException
	 */
	private void fhm(int[] prefix, int prefixLength, UtilityListFHMPlus pUL, List<UtilityListFHMPlus> ULs,
	 ...
	 */
	private UtilityListFHMPlus construct(UtilityListFHMPlus P, UtilityListFHMPlus px, UtilityListFHMPlus py,
			int minUtility, int pxyLength) {
	 ...
	 * @return the utility list of pXY
	 */
	private UtilityListFHMPlus oldConstruct(UtilityListFHMPlus P, UtilityListFHMPlus px, UtilityListFHMPlus py,
			int minUtility, int pxyLength) {
		// create an empy utility list for pXY
		UtilityListFHMPlus pxyUL = new UtilityListFHMPlus(py.item);
	...
	private void writeOut(int[] prefix, int prefixLength, int item, long utility) throws IOException {
		// if this pattern does not respect the minimum length constraint, then we don't
		// output it.
