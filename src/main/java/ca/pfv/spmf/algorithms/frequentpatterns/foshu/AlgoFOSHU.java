 ...
 * Mining with negative profit values as described in the conference paper :
 * <br/>
 * <br/>
 * 
 * Fournier-Viger, P., Zida, S. (2015). FOSHU: Faster On-Shelf High Utility
 * Itemset Mining– with or without negative unit profit. Proc. 30th Symposium
 * on Applied Computing (ACM SAC 2015). ACM Press, 8 pages.
 * 
...
public class AlgoFOSHU {
	...
	// The following line allows to specify how many lines from the input file
	// should
	// be read. By default this parameter is set to Integer.MAX_VALUE (read the
	// whole file).
	...
	public void runAlgorithm(String input, String output, double minUtilityRatio) throws IOException {
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null && transactionCount <= maxSEQUENCECOUNT) {
...
//					}
	...
	/**
	 * Method to compare items according to the total order of increasing TWU such
	 * that negative items are always last.
	 * 
	 * @param item1 an item
	...
	private void foshu(int[] prefix, UtilityListFOSHU pUL, List<UtilityListFOSHU> ULs) throws IOException {
		...
		for (int i = 0; i < ULs.size(); i++) {
			...
			double ru = calculateRelativeUtility(sumPeriodUtility, (X.sumIutilP + X.sumIutilN));
			...
			// if X is not promising, then we will not explore its extensions, so we
			// continue.
