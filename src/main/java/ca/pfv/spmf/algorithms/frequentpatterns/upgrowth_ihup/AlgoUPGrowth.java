 ...
 * 
 * V. S. Tseng, C.-W. Wu, B.E. Shie, P. S. Yu: UP-Growth: an efficient algorithm
 * for high utility itemset mining. KDD 2010: 253-262 <\br><\br>
 * 
			...
			for (int j = 1; j < prefixPath.size(); j++) {
				...
				if (itemPathUtility.get(node.itemID) >= minUtility) {
					localPath.add(node.itemID);
							...
							// utillity
					Integer minItemUtility = mapMinimumItemUtility.get(node.itemID);
