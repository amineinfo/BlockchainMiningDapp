...
/**
 * This is an implementation of the "HUI-LIST-INS" algorithm by Lin et al.
 * (2014) an incremental version of the FHM algorithm.
 * 
 * This algorithm is said to be incremental because it updates the utility-lists
 * and EUCS structure of FHM if some new transactions are added which speed up
 * the first two database scans. However, the search is redone from scratch
 * every time that some new transactions are added.
 * 
 * @see UtilityList
 * @see Element
...
public class AlgoHUI_LIST_INS {
	 ...
	 */
	public void runAlgorithm(String input, String output, Integer minUtil, int firstLine, int lastLine)
			throws IOException {
		// Reset statistics
		candidateCount = 0;
		huiCount = 0;
