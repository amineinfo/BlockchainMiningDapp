...
//		System.out.println(percentage);
	...
	private void update(int minUtility) {
		...
		for (i = 0; i < pop_size; i++) {
			...
			// update velocity
			for (j = 0; j < twuPattern.size(); j++) {
				double temp = V.get(i).get(j) + r1 * (pBest.get(i).X.get(j) - population.get(i).X.get(j))
	...
	private int fitCalculate(List<Integer> tempParticle, int k) {
		...
		for (p = 0; p < database.size(); p++) {// p scan the transactions in
			...
			// particle
			while (j < k && q < database.get(p).size() && i < tempParticle.size()) {
	...
	public void printStats() {
		System.out.println("=============  HUIM-BPSO ALGORITHM v.2.11 - STATS =============");
