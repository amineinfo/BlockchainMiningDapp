...
/**
 * This is an implementation of the dECLAT algorithm. The difference between
 * DECLAT and ECLAT is that dECLAT utilizes diffsets instead of tidsets. In this
 * implementation, diffsets are represented as bitsets. Note that this class is
 * a subclass of the ECLAT algorithm because a lot of code is the same and we
 * wanted to avoid redundancy. Note also that implementing diffsets using
 * bitsets may not provide the optimal performance for dEclat since even if
 * diffsets are smaller than tidsets, as a bitset, they may not be much smaller.
 * It is thus recommended to try also the regular implementation
 * 
 * IMPORTANT: dEClat returns Itemsets annotated with their diffsets rather than
 * tidsets when the user choose to keep the result in memory.
 * 
 * DEclat was proposed by ZAKI (2000). <br/>
 * <br/>
 * 
