...
/**
 * This is an implementation of the "FCHM" algorithm for High-Utility Itemsets
 * Mining as described in the conference paper : <br/>
 * <br/>
 * 
 * Fournier-Viger, P., Zhang, Y., Lin, J. C.-W., Dinh, T., Le, B. (2018) Mining
 * Correlated High-Utility Itemsets Using Various Correlation Measures. Logic
 * Journal of the IGPL, Oxford Academic, to appear.
 * 
...
//						}
	 ...
	 * @throws IOException
	 */
	private void fchm(int[] prefix, int prefixLength, UtilityListFCHM_bond pUL, List<UtilityListFCHM_bond> ULs,
			int minUtility) throws IOException {
					...
					if (mapTWUF != null) {
						...
						if (twuF != null) {
							...
							if (ENABLE_BOND_PAIR_PRUNING) {
								...
								int max_dissup = Y.bitsetDisjunctiveTIDs.support > X.bitsetDisjunctiveTIDs.support
										? Y.bitsetDisjunctiveTIDs.support
										: X.bitsetDisjunctiveTIDs.support;
								...
								/*
								 * boolean conditionY = twuF.support / (double) Y.bitsetDisjunctiveTIDs.support
								 * < minBond; boolean conditionX = twuF.support / (double)
								 * X.bitsetDisjunctiveTIDs.support < minBond;
								 */
	 ...
	 */
	private UtilityListFCHM_bond construct(UtilityListFCHM_bond P, UtilityListFCHM_bond px, UtilityListFCHM_bond py,
			int minUtility, BitSetSupport bitsetPXY) {
		/*
		 * BitSetSupport bitsetPX = px.bitsetDisjunctiveTIDs; BitSetSupport bitsetPY =
		 * py.bitsetDisjunctiveTIDs; BitSetSupport bitsetPXY = performOR(bitsetPX,
		 * bitsetPY);
		 */
		...
		if (DEBUG) {
			for (Entry<Integer, Map<Integer, TwuSupportPair>> entry : mapSMAP.entrySet()) {
				...
				for (Entry<Integer, TwuSupportPair> entry2 : entry.getValue().entrySet()) {
					System.out.println(" (Item: " + entry2.getKey() + " twu: " + entry2.getValue().twu + " sup: "
							+ entry2.getValue().support);
				}
			}
		}
