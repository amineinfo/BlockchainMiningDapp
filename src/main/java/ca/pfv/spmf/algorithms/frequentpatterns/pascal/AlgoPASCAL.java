 ...
 * algorithm that use information about generators to skip some database scans.
 * <br/>
 * <br/>
 * 
 ...
 * algorithme d'extraction des motifs fr�quents, 65-95. In Techniques et
 * Science Informatiques 21 (1). <br/>
 * <br/>
 * 
...
public class AlgoPASCAL {
	...
	public void runAlgorithm(double minsup, String input, String output) throws IOException {
		...
		for (Entry<Integer, Integer> entry : mapItemCount.entrySet()) {
			...
			if (itemsetSupport >= minsupRelative && maxItemsetSize >= 1) {
				...
				ItemsetPascal itemset = new ItemsetPascal(new int[] { entry.getKey() });
	...
	private List<ItemsetPascal> generateCandidate2(List<ItemsetPascal> frequent1) {
		...
		for (int i = 0; i < frequent1.size(); i++) {
			...
			for (int j = i + 1; j < frequent1.size(); j++) {
				...
				itemset.isGenerator = itemset1.isGenerator && itemset2.isGenerator;
	...
	void saveItemsetToFile(ItemsetPascal itemset) throws IOException {
		writer.write(itemset.toString() + " #SUP: " + itemset.getAbsoluteSupport() + " #IS_GENERATOR "
				+ itemset.isGenerator);
		writer.newLine();
		itemsetCount++;
	}
