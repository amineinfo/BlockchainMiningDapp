...
/**
 * This is an implementation of the "PHM" algorithm for Periodic High-Utility
 * Itemsets Mining as described in the conference paper : <br/>
 * <br/>
 * 
 * Fournier-Viger, P., Lin, C.W., Duong, Q.-H., Dam, T.-L. (2016). PHM: Mining
 * Periodic High-Utility Itemsets. Proc. 16th Industrial Conference on Data
 * Mining. Springer LNAI 9728, 15 pages .
 *
 * @see UtilityListPHM
 * @see Element
...
public class AlgoPHM {
	 ...
	 */
	public void runAlgorithm(String input, String output, int minUtility, int minPeriodicity, int maxPeriodicity,
			int minAveragePeriodicity, int maxAveragePeriodicity) throws IOException {
		// reset maximum
...
//			}
			...
			// Important: we do not update the minimum periodicity of the item using its
			// last period
...
//						+ " avg periodicity: " + itemInfo.averagePeriodicity
						+ "\tavgPer: " + (databaseSize / (double) (itemInfo.support + 1)) + "\tminPer: "
						+ itemInfo.smallestPeriodicity + "\tmaxPer: " + itemInfo.largestPeriodicity + "\tTWU: "
						+ mapItemToTWU.get(entry.getKey()) + "\tsup.: " + itemInfo.support);
			}
		}
		if (DEBUG) {
		...
		for (Integer item : mapItemToTWU.keySet()) {
			...
			ItemInfo itemInfo = mapItemToItemInfo.get(item);
			if (itemInfo.support >= supportPruningThreshold && itemInfo.largestPeriodicity <= maxPeriodicity
...
//		int compare = (int)( mapItemToItemInfo.get(item1).support - mapItemToItemInfo.get(item2).support);
	...
	/**
	 * This is the recursive method to find all periodic high utility itemsets. It
	...
	private void phm(int[] prefix, int prefixLength, UtilityListPHM pUL, List<UtilityListPHM> ULs, int minUtility)
		...
		for (int i = 0; i < ULs.size(); i++) {
			...
			if (X.sumIutils + X.sumRutils >= minUtility) {
				...
				if (X.sumIutils >= minUtility && averagePeriodicity <= maxAveragePeriodicity
						&& averagePeriodicity >= minAveragePeriodicity && X.smallestPeriodicity >= minPeriodicity
				...
				// If we still want to find larger patterns... (based on the optional maximum
				// length constraint)
...
//		}
	 ...
	 */
	private void writeOut(int[] prefix, int prefixLength, UtilityListPHM utilityList, double averagePeriodicity)
			throws IOException {
	...
	public void printStats() throws IOException {
		...
		System.out.println("=============  PHM ALGORITHM v2.17" + optimizationEUCP + optimizationESCP + "=====");
	 ...
	 * 
	 * @param enable if this parameter is true, EUCP will be enabled. Otherwise,
	 *               not.
	 */
	 ...
	 * 
	 * @param enable if this parameter is true, ESCP will be enabled. Otherwise,
	 *               not.
	 */
