...
/**
 * This is an implementation of an algorithm for finding the set of Minimum Non
 * Redundant rules (MNR) from a transaction database. <br/>
 * <br/>
 * 
 * This set is defined as the set of association rules of the form P1 ==> P2 /
 * P1, where P1 is a generator of P2, P2 is a closed itemset, and the rule has a
 * support and confidence respectively no less than minsup and minconf. <br/>
 * <br/>
 * 
 * See the following publication for more details: <br/>
 * <br/>
 * 
 ...
 * <br/>
 * 
 * Here, the implementation is based on the description in Szathmary's thesis
 * (2006). The algorithm proceed by exploiting the generators and closed itemset
 * found by the Zart algorithm. <br/>
 * <br/>
 *
 * This algorithm can save the result to a file or keep it into memory if the
 * user gives a null output path to the runAlgorithm() method. <br/>
 * <br/>
 *
...
public class AlgoMNRRules {
	 ...
	 * Run the algorithm.
	 * 
	 * @param closedPatternsAndGenerators Closed itemsets and their associated
	 *                                    generators.
	 * @param databaseSize                the number of transactions in the
	 *                                    transaction database.
	 * @param minconf                     minimum confidence threshold
	 * @param outputFilePath              the output file path, if the results
	 *                                    should be saved to a file. if null, the
	 *                                    result are saved in memory and returned by
	 *                                    this method.
	 * 
	 * @return if the user chose to save to memory, this methods return the set of
	 *         IGB association rules, otherwise, the result is saved to the output
	 *         file chosen by the user.
	 * @throws IOException
	 */
	public Rules runAlgorithm(String outputFilePath, double minconf, TZTableClosed closedPatternsAndGenerators,
			int databaseSize) throws IOException {
		...
		for (Map.Entry<Itemset, List<Itemset>> entryEquivalenceClass : closedPatternsAndGenerators.mapGenerators
				.entrySet()) {
			// get the list of generators
			List<Itemset> listGenerators = entryEquivalenceClass.getValue();
			// if the equivalence class has no generator, then its closed itemset is a
			// generator...
			...
			for (Itemset generatorG : listGenerators) {
				...
				for (Itemset closedItemset : closedPatternsAndGenerators.mapGenerators.keySet()) {
					if (generatorG.size() < closedItemset.size() && closedItemset.containsAll(generatorG)) {
				...
				for (Itemset closedItemset : supersets) {
					...
					double conf = ((double) closedItemset.getAbsoluteSupport())
							/ ((double) generatorG.getAbsoluteSupport());
//										
					if (conf >= minconf) {
	 ...
	 */
	private void saveRule(Itemset itemset1, Itemset itemset2, int absoluteSupport, double confidence)
			throws IOException {
