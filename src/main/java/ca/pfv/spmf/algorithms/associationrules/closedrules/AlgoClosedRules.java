 ...
 * RJ9839, June 1994. Here it is adapted for mining closed rules. The main
 * difference with AlgoAgrawalFaster94.java, is that this implementation takes a
 * different type of Itemset object as input. Moreover, the method
 * CALCULATESUPPORT() is different because if an itemset is not closed, we must
 * find its closure to determine its support. To avoid redundancy as much as
 * possible, this class is a subclass of AlgoAgrawalFaster94.java and share
 * several methods. <br/>
 * <br/>
 * 
...
public class AlgoClosedRules extends AlgoAgrawalFaster94 {
			...
			for (Itemset lk : patterns.getLevels().get(k)) {
				...
				for (int item : lk.getItems()) {
					...
					if (usingLift) {
						...
						supportHm_P_1 = calculateSupport(itemsetHm_P_1); // if we want to calculate the lift, we need to
																			// add this.
						// calculate the lift
					...
					saveRule(itemset_Lk_minus_hm_P_1, support, itemsetHm_P_1, supportHm_P_1, lk.getAbsoluteSupport(),
							conf, lift);
	 ...
	 * patterns. An alternative would be to use a trie to store patterns but it may
...
//		
		...
		// We start from itemset of length |itemset|+1 and increase the size of itemset
		// after each
		// while loop.
		...
		loop: while (true) {
			...
			for (Itemset pattern : patternsList) {
				...
				// If the first item of the pattern is larger than the first item of the
				// itemset,
