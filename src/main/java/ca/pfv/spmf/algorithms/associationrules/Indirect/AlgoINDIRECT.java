...
/**
 * This is an implementation of the INDIRECT algorithm for generating indirect
 * association rules. <br/>
 * <br/>
 * 
 * The implementation is based on the description in the book: Tan, Steinbach &
 * Kumar (2006) "Introduction to data mining", chapter 7, p. 469, Algorithm 7.2.
 * and the KDD 2000 paper by Tan et al. <br/>
 * <br/>
 * 
 * However, note that the algorithm is not exactly the same as what the authors
 * did, because there is not enough details in the original paper and in the
 * book. To implement the algorithm, I therefore had to make some choices based
 * on what I tought what the best or easiest way to do it. <br/>
 * <br/>
 * 
 * Also, note that instead of using the IS measure to compute the dependancy
 * between itemsets, I chose to use the confidence. The confidence is easier to
 * calculate. <br/>
 * <br/>
 * 
 * Also, note that there is some faster algorithm that exists for generating
 * indirect association rules that have been proposed after INDIRECT (but not
 * implemented in SPMF). <br/>
 * <br/>
 * 
 * Lastly, note that in my implementation I use an AprioriTID like procedure for
 * generating frequent itemsets that are needed to generate indirect rules.
 * However I do not save the frequent itemsets to file because we don't need to
 * keep them (we just want to generate the indirect rules). <br/>
 * <br/>
 * 
 * If you find some errors or have some ideas for optimization, please let me
 * know by contacting me on my website. <br/>
 * <br/>
 * 
 * One possible optimization that I could do in the future would be to use
 * BitSet instead of HashSet to represent the tids sets. <br/>
 * <br/>
 *
...
public class AlgoINDIRECT {
	 ...
	 * @throws IOException exception if there is an error while writing the output
	 *                     file.
	 */
	public void runAlgorithm(String input, String output, double minsup, double ts, double minconf)
...
//			}
	...
	/**
	 * Generate candidate of size K by using frequent itemsets of size k-1. (this
	 * process is similar to Apriori).
	 * 
...
// For each itemset I1 and I2 of level k-1
		...
		// TRY ALL COMBINATION TO GENERATE INDIRECT RULES FROM ITEMSET OF SIZE K, IF K >
	...
	/**
	 * Test if an indirect rule satisfies the criterion for an indirect association
	 * rules ts and minconf.
	 * 
	 * @param itemset a potential itemset that could be a mediator if we remove "a"
	 *                and "b"
	 * @param a       the item a
	 * @param b       the item b
	 ...
	 */
	private void saveRule(Integer a, Integer b, Itemset itemset, double confAY, double confBY, int supAY, int supBY)
			throws IOException {
