 ...
 * RJ9839, June 1994. Here it is adapted for mining closed rules using the
 * results obtained from the FPClose algorithm (the frequent closed itemsets).
 * The main difference with AlgoAgrawalFaster94.java, is that this
 * implementation takes a different type of Itemset object as input. Moreover,
 * the method CALCULATESUPPORT() is different because if an itemset is not
 * closed, we must find its closure to determine its support. To implement the
 * calculation of the support efficiently, this class uses the CFI-tree
 * generated by CFPClose. It allows to calculate very quickly the support of non
 * closed frequent itemsets. This is the main difference with
 * "AlgoClosedRules.java" To avoid redundancy as much as possible, this class is
 * a subclass of AlgoAgrawalFaster94.java and share several methods. <br/>
 * <br/>
 * 
...
public class AlgoClosedClassRules_UsingFPClose extends AlgoAgrawalFaster94 {
	...
	/**
	 * the frequent closed itemsets stored in a CFITree. This allows to efficiently
	 * count the support of their subsets.
	 */
	 ...
	 * @param output                an output file path for writing the result or
	 *                              null if the user want this method to return the
	 *                              result
	 * @param databaseSize          the number of transactions in the database
	 * @param minconf               the minconf threshold
	 * @param cfiTree               the cfiTree generated by FPClose to store
	 ...
	 */
	public AssocRules runAlgorithm(Itemsets patterns, String output, int databaseSize, double minconf, CFITree cfiTree,
			int[] itemToBeUsedAsConsequent) throws IOException {
	 ...
	 * @param cfiTree      the cfiTree generated by FPClose to store frequent closed
	 *                     itemsets
	 * @return the set of association rules if the user wished to save them into
	 *         memory
	 * @throws IOException exception if error writing to the output file
...
//				apGenrules(k, 1, lk, H1_for_recursion);
	...
	public int calculateSupport(int[] itemset) {
		// **************************************************************************************
		// THIS IS THE MAIN DIFFERENCE WITH THE OTHER IMPLEMENTATION FOR MINING CLOSED
		// RULES
		// We will calculate the support of an itemset using the CFI-tree structure
		// generated by
		// FPClose.
		// **************************************************************************************
