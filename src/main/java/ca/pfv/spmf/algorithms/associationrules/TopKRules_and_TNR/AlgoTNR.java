...
/**
 * TNR is an algorithm for mining the TOP-K non redundant association rules with
 * a pattern growth approach and several optimizations. This is the original
 * implementation as proposed in the following paper: <br/>
 * <br/>
 * 
 * Fournier-Viger, P., Tseng, V.S. (2012). Mining Top-K Non-Redundant
 * Association Rules. Proc. 20th International Symposium on Methodologies for
 * Intelligent Systems (ISMIS 2012), Springer, LNCS 7661, pp. 31- 40.
 * 
...
public class AlgoTNR {
	...
	/**
	 * a vertical representation of the database [item], IDs of transaction
	 * containing the item
	 */
	BitSet[] tableItemTids; //
	/**
	 * a table indicating the support of each item [item], support
	 */
	int[] tableItemCount; //
...
//			candidates.remove(rule);
	 ...
	 * record them for future expansions.
	 * 
	 ...
	 */
	private void generateRuleSize11(Integer itemI, BitSet tidI, Integer itemJ, BitSet tidJ, BitSet commonTids,
			int cardinality) {
		// Create the rule I ==> J
	 ...
	 * 
	 * @param expandLR if true the rule will be considered for left/right expansions
	 *                 otherwise only right.
	...
	private void expandLR(RuleG ruleG) {
		...
		// Key: item Value: bitset indicating the IDs of the transaction containing the
		// item
		...
		for (int tid = ruleG.common.nextSetBit(0); tid >= 0; tid = ruleG.common.nextSetBit(tid + 1)) {
			...
			while (iter.hasNext()) {
				...
				// CAN DO THIS BECAUSE TRANSACTIONS ARE SORTED BY DESCENDING ITEM IDS (see
				// Database.Java)
		...
		if (ruleG.getItemset2().length < maxConsequentSize) {
			for (Entry<Integer, BitSet> entry : mapCountRight.entrySet()) {
				...
				if (ruleSupport >= minsuppRelative) {
					...
					// create the rule
					RuleG candidate = new RuleG(ruleG.getItemset1(), newRightItemset, ruleSupport, ruleG.tids1,
							tidsRule, ruleG.maxLeft, maxRight);
		...
		if (ruleG.getItemset1().length < maxAntecedentSize) {
			for (Entry<Integer, BitSet> entry : mapCountLeft.entrySet()) {
				...
				if (ruleSupport >= minsuppRelative) {
					...
					// create the rule
					RuleG candidate = new RuleG(newLeftItemset, ruleG.getItemset2(), ruleSupport, tidsLeft, tidsRule,
	...
	private void expandR(RuleG ruleG) {
		...
		for (Entry<Integer, BitSet> entry : mapCountRight.entrySet()) {
			...
			if (ruleSupport >= minsuppRelative) {
				...
				// create the rule
				RuleG candidate = new RuleG(ruleG.getItemset1(), newRightItemset, ruleSupport, ruleG.tids1, tidsRule,
...
//		System.out.println("Candidates count : " + candidates.size());
	...
	/**
	 * Set whether the transaction identifiers should be shown in the output file or
	 * not
	 * 
