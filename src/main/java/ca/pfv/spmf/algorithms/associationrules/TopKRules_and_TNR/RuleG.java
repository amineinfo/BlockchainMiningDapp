...
/**
 * This class represents an association rule found by the TNR or TopKRules
 * algorithm for top-k association rule mining. <br/>
 * <br/>
 * 
 * This implementation is optimized for these algorithms. In particular, it
 * stores the tidset of the rule and the tidset of the rule antecedent as
 * bitsets. Furthermore, the antecedent and consequent are stored as array of
 * integers. Lastly, for optimization, the maximum item id of the antecedent and
 * consequent of the rule are precalculated and kept (as explained in the
 * papers).
 * 
 * @see AlgoTNR
 * @see AlgoTopKRules
...
public class RuleG implements Comparable<RuleG> {
	 ...
	 * 
	 * @return 1 if this rule is larger, 0 if equal, or -1 if smaller. The
	 *         comparison is done based on the support, then on the size of the
	 *         antecedent, then on the size of the consequent, then on the
	 *         confidence, then on the hashCodes.
	 */
