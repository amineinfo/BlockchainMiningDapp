 ...
 * rules in a transaction database. This algorithm is described in the paper:
 * <br/>
 * <br/>
...
public class AlgoFHSAR {
	 ...
	 * @param inputSAR the file path to a set of sensitive association rules to be
	 *                 hidden
	 * @param output   the output file path for writing the modified transaction
	 *                 database
	 ...
	 */
	public void runAlgorithm(String input, String inputSAR, String output, double minsup, double minconf)
			throws IOException {
		...
		// At the same time, we will calculate the wi for each transaction in the
		// database
		String line;
		...
		while (((line = reader.readLine()) != null)) {
			...
			boolean thereIsARuleSupportedByTransaction = false; // flag to know if at least one sensitive rule appear in
																// this transaction
			List<Rule> rulesContained = new ArrayList<Rule>(); // the list of sensitive rules contained in this
																// transaction
			...
			for (Rule rule : sensitiveRules) {
				...
				loop: for (int i = 0; i < lineSplited.length; i++) {
					...
					// if the rule completely matches with this transaction... (both left and right
					// sides)
		...
		// We will delete items until the sensitive association rules fell below the
		// threshold.
		while (sensitiveRules.isEmpty() == false) {
			...
			// At the same time we will update the support count of the sensitive
			// association rules.
			Map<Integer, Integer> mapItemCount = new HashMap<Integer, Integer>();
...
//			System.out.println("remove " + maxItem + " from " + td.items);
			...
			// if at least one sensitive rule is STILL contained in this transaction, we
			// need to update
