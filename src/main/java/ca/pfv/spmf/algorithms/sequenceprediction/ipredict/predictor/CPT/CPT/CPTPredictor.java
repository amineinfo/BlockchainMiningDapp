 ...
 */
/**
 * CPT - Compact Prediction Tree 1st iteration from ADMA 2013, with speed
 * enhancement
 */
public class CPTPredictor extends Predictor {
	 ...
	 * @param weight         Weight to add for each occurrence of an item in the
	 *                       CountTable
	 * @param CountTable     The CountTable to update/fill
	 * @param hashSidVisited Prevent for processing the same branch multiple times
	 */
	private void UpdateCountTable(Item[] targetArray, float weight, Map<Integer, Float> CountTable,
			HashSet<Integer> hashSidVisited) {

		Bitvector indexes = getMatchingSequences(targetArray);

		// if there is not set of sequences ids containing all the target's items
		// together
	...
	/**
	 * Generate the highest rated sequence from a CountTable using the Lift or the
	 * Confidence
	 * 
	...
	public Sequence Predict(Sequence target) {
		...
		// remove items that were never seen before from the Target sequence before LLCT
		// try to make a prediction
		// If set to false, those items will be still ignored later on (in
		// updateCountTable())
		...
		int maxRecursion = (parameters.paramInt("recursiveDividerMax") > targetArray.length) ? targetArray.length
				: parameters.paramInt("recursiveDividerMax");
	...
	/**
	 * Obtain the count table used for the latest prediction made by the model.
	 * 
	 * @return a map where the key is a symbol and the value is a score assigned by
	 *         the model. The item with the highest score is the prediction.
	...
	/**
	 * Divides the target sequence into all possible sub sequence with a minimum
	 * size of minSize
	 * 
...
//			}
	...
	public Boolean Train(List<Sequence> trainingSequences) {
		...
		/**
		 * OPTIMIZATION: Removes all the unique items with a really low support from the
		 * inverted index. Should be tested some more, appears to boost the coverage
		 * with no significant effect on the precision
		 */
	...
	public float memoryUsage() {
		...
		float sizePredictionTree = nodeNumber * 3 * 4; // each node uses 3 integers, one for value, one for parent link,
														// and one on average for child

		float sizeInvertedIndex = (float) (II.size() * (Math.ceil(LT.size() / 8) + 4));

		float sizeLookupTable = LT.size() * 2 * 4; // the key and the value of this hashmap are integer and pointer
													// respectively (4 bytes)
