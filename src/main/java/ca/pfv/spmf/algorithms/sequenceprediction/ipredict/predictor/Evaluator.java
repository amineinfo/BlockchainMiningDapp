...
public class Evaluator {
	 ...
	 */
	public StatsLogger Start(int samplingType, float param, boolean showResults, boolean showDatasetStats,
			boolean showExecutionStats) throws IOException {
	...
	/**
	 * Holdout method Data are randomly partitioned into two sets (a training set
	 * and a test set) using a ratio. The classifier is trained using the training
	...
	public void Holdout(double ratio, int classifierId) {
		...
		// DEBUG
		// System.out.println("Dataset size: "+ (trainingSequences.size() +
		// testSequences.size()));
		// System.out.println("Training: " + trainingSequences.size() + " and Test set:
		// "+ testSequences.size());
	...
	/**
	 * Random subsampling Holdout method repeated 10 times
	 * 
	...
	/**
	 * k-fold cross-validation Data are partitioned in k exclusive subsets (folds)
	 * of same size. Training and testing is done k times. For each time; a fold is
	 * used for testing and the k-1 other folds for training
	 */
	...
	private void StartClassifier(List<Sequence> testSequences, int classifierId) {
		...
		for (Sequence target : testSequences) {
			...
			if (target.size() > (Profile.paramInt("consequentSize"))) {
				...
				Sequence consequent = target.getLastItems(Profile.paramInt("consequentSize"), 0); // the lasts actual
																									// items in target
				Sequence finalTarget = target.getLastItems(Profile.paramInt("windowSize"),
						Profile.paramInt("consequentSize"));
