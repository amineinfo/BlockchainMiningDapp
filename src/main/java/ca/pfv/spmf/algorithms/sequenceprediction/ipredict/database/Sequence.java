...
public class Sequence {
	 ...
	 * 
	 * @return [length] or less (if not enough items) items as a sequence or NULL on
	 *         error
	 */
	public Sequence getLastItems(int length, int offset) {
		...
		if (items.isEmpty()) {
			...
			// WE MODIFY THE LIST WE MAY GET A CONCURRENT ACCESS EXCEPTION (I was getting
			// one!)
//		    //  new ArrayList(...)
		...
		/*
		 * //TODO: this is way too too strict, let it loose int size = size() - offset;
		 * if(length <= size) { //splitting list List<Item> truncatedList =
		 * items.subList( (size - length), (size) );
		 * 
		 * //creating new sequence with truncated list Sequence truncatedSequence = new
		 * Sequence(0); truncatedSequence.setItems(truncatedList);
		 * 
		 * return truncatedSequence; } else { return null; //should never happen! it
		 * would cause the algo to not work properly //TODO: throw exception or maybe
	...
	/*
	 * public void keepOnlyLastItems(int length) { if(length < size()) { //Sequence
