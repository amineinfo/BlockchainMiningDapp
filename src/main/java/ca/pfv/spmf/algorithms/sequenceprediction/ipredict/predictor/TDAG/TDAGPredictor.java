 ...
 */
/**
 * TDAG predictor is based on Markov Trees Its fast but it has drawbacks in
 * terms It takes a lot of space! This implementation has a few of the
 * optimization presented in the original paper below.
 * 
 * Optimization#1: max tree height as described in original papel -> reduce
 * spacial size without noticeable effect on accuracy and coverage (to be double
 * checked)
 * 
 * Source: P. Laird and R. Saul, "Discrete sequence prediction and its
 * applications" Mach. Learning, vol. 15, pp. 43-68, 1994.
 */
public class TDAGPredictor extends Predictor {
	...
	/**
	 * Max tree height, forbid creating branch with a length higher than this
	 * parameter
	 */
	private final Integer maxTreeHeight = 6;

	/**
	 * Map a list of symbol to a specific node in the tree It is used to lookup
	 * specific nodes in the prediction method.
	 */
			...
			for (Item item : seq.getItems()) {
				...
				for (TDAGNode node : state) {
					...
					if (node.pathFromRoot.size() <= maxTreeHeight) {
						...
						/**
						 * START OF BUGFIX 2018-01-24 Removed redundant size increments.
						 *
						 * size was incremented although the child node might have already existed. Now
						 * the absence of the child node is checked before the size is incremented.
						 */
...
//						size++;
	...
	public Sequence Predict(Sequence target) {
		...
		/***
		 * START BUG FIX 2018-01-25 Fixed TDAG Prediction Bug 1: Because symbols(0) is
		 * the root node of the sequence, the symbol with the least relevance is always
		 * at position 1. This bug is one reason why the TDAG algorithm has high 'No
		 * Match' values.
		 */
...
//			symbols.remove(0);
			...
			/**
			 * START BUG FIX 2018-01-25 Fixed TDAG Prediction Bug 2: When searching for a
			 * suitable context in the tree, ignore all contexts that do not have any
			 * children, as they do not provide any information. Else the prediction will
			 * return an empty Sequence, although there might be a context of a lower order
		...
		if (context != null) {
			...
			// For each child of this context, we calculate the score (probability of
			// appearance given the context)
			...
			Double treshold = 0.0;
			/**
			 * START OF BUG FIX 2018-01-24 /** Fixed TDAG Prediction Bug 3: If a context has
			 * two children with the same score, the predictor returned an empty sequence
			 * instead of one of the two symbols.
			 *
			 * Instead of changing > to >=, the whole determination of candidate 2 could be
			 * skipped too.
			 */
			if (candidate1 != null &&
	...
	/**
	 * Each node has a list of children, the sum of the lists for all children is
	 * equals to the number of nodes. + * Each nodes has also three integers one
	 * child ref (4 bytes) + 3 integers (12 bytes) = 16 bytes
	 */
