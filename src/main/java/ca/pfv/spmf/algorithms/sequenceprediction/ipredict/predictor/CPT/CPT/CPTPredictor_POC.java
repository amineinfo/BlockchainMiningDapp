 ...
 */
/**
 * CPT - Compact Prediction Tree 1st iteration from ADMA 2013, without speed
 * enhancement
 * 
 * Source: T. Gueniche, P. Fournier-Viger and V. S. Tseng, "Compact prediction
 * tree: A lossless model for accurate sequence prediction" in Advanced Data
 * Mining and Applications, Springer, 2013, pp. 177-188.
 */
public class CPTPredictor_POC extends Predictor {
	 ...
	 */
	private void UpdateCountTable(Sequence target, float weight, Map<Integer, Float> CountTable,
	 ...
	 * @param useLift    Whether to use the Lift or the Confidence to calculate the
	 *                   score
	...
	public Sequence Predict(Sequence target) {
		...
		int maxRecursion = (parameters.paramInt("recursiveDividerMax") > target.size()) ? target.size()
	...
	public Boolean Train(List<Sequence> trainingSequences) {
		 ...
		 * Iterator it = II.entrySet().iterator(); while (it.hasNext()) {
		 * Map.Entry<Integer, BitSet> pairs = (Map.Entry)it.next();
		 * 
		 * if(pairs.getValue().cardinality() < minSup) { it.remove(); } }
		 */
		/***************** END OF OPTIMIZATION ***********************/
