...
public class SequenceDatabase {
	...
	public void loadFileCustomFormat(String filepath, int maxCount, int minSize, int maxSize) throws IOException {
		...
		try {
			...
			// For each line in the files -- up to the end of the file or the max number of
			// sequences
			int count = 0;
		...
		try {
			...
			while ((thisLine = myInput.readLine()) != null && count < maxCount) { // until end of file
				...
				if (lastId != id) { // if new sequence
					if (lastId != 0 && sequence.size() >= minSize && sequence.size() <= maxSize) { // adding last
																									// sequence to
																									// sequences list
						sequences.add(sequence);
						count++;
	...
	/**
	 * Load the sequences for a FIFA (World cup) log file see:
	...
	public void loadFileLargeTextFormatAsCharacter(String filepath, int maxCount, int minSize, int maxSize)
			throws IOException {
		String thisLine;
		BufferedReader myInput = null;
		try {
	...
	public void loadFileLargeTextFormatAsWords(String filepath, int maxCount, int minSize, int maxSize,
			boolean doNotAllowSentenceToContinueOnNextLine) throws IOException {
		String thisLine;
		BufferedReader myInput = null;
		try {
			FileInputStream fin = new FileInputStream(new File(filepath));
