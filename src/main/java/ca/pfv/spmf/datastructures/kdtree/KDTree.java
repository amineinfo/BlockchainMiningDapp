 ...
 * book: "Algorithms in a Nutshell" by Heineman et al. (2008). <br/>
 * <br/>
 * 
 ...
 * <br/>
 * 
 * Elements that are inserted into the tree have to be arrays of double. <br/>
 * <br/>
 * 
 * The class provide methods for - building the tree by inserting points, -
 * using the tree to find the nearest neighbor to a given point - using the tree
 * to find the k nearest neighbors to a given point <br/>
 * <br/>
 * 
 * To find the k-nearest neighboors, the closest points are stored in a red
 * black tree.
 *
...
//	List<DoubleArray> pointsDEBUG = null;
	...
	/**
	 * This method build the KDtree from a set of points. This method should be
	 * called only once.
	 * 
...
//		pointsDEBUG = points;
	 ...
	 * It is based on the pseudo-code of the Randomized-Select algorithm in the book
	 * "Introduction to algorithms" by Cormen et al. (2001), with some modifications
	 * such as using a while loop instead of recursive calls.
	 * 
	...
	public DoubleArray nearest(DoubleArray targetPoint) {
		...
		// distance with the parent. If a point with a distance smaller than the parent
		// is found,
		// then return it.
	...
	public RedBlackTree<KNNPoint> knearest(DoubleArray targetPoint, int k) {
		...
		// Now start back at the root, and check all rectangles that have a
	...
	/**
	 * Start back at the root, and check all rectangles that have a perpendicular
	 * distance smaller than the k best points found until now.
	 * 
	 * @param node        the current node
...
//			}
	...
	// =========================== METHOD TO FIND POINTS WITHIN A RADIUS - used by
	// DBSCAN =============================
	/**
	 * Method to get all the points within the radius of a given target point,
	 * EXCEPT the target point!
	 * 
	 * @param the              target point
	 * @param bufferNeighboors a list where the points should be added should be
	 *                         provided to this method return the list of points
	 */
...
//		// return the points within the radius
	 ...
	 * @param result      a list where the points should be added should be provided
	 *                    to this method
	...
	public List<DoubleArray> pointsWithinRadiusOf(DoubleArray targetPoint, double radius, List<DoubleArray> result) {
		...
		// start at the root, and check all rectangles that have a perpendicular
		// distance
...
//		// return the points within the radius
	...
	/**
	 * Start back at the root, and check all rectangles that have a perpendicular
	 * distance smaller than the radius.
	 * 
	 * @param node        the current node
	 * @param targetPoint the vector
	 * @param result      the set of points within the radius (to be filled by this
	 *                    method)
	 * @param the         radius
	 */
	...
	// =========================== METHOD TO FIND POINTS WITHIN A RADIUS AND KEEP
	// THE DISTANCE - used by OPTICS =============================

	/**
	 * Method to get all the points within the radius of a given target point,
	 * EXCEPT the target point, and also get their distance to the target point!
	 */
	 ...
	 * EXCEPT the target point, and also get their distance to the target point!
	 * 
	 * @param result an array list that will be used for storing the points that are
	 *               found
	 */
	public List<KNNPoint> pointsWithinRadiusOfWithDistance(DoubleArray targetPoint, double radius,
			List<KNNPoint> result) {
		if (root == null) {
			return null;
		}
	 ...
	 * @param targetPoint the vector
	 * @param result      the set of points within the radius (to be filled by this
	 *                    method), and their distance to the target point!
	 * @param the         radius
	 */
	private void findPointsWithinRadiusWithDistance(KDNode node, DoubleArray targetPoint, List<KNNPoint> result,
			double radius) {
		// if it is the target point, we skip it because we don't want to return it
		if (node.values != targetPoint) {
