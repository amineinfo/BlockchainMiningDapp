...
/**
 * This class is for converting transaction databases from various formats to
 * the SPMF format.
 * 
 * @see Formats
...
public class TransactionDatabaseConverter {
	...
	/**
	 * This method converts a transaction database from a given format to the SPMF
	 * format.
	 * 
	 ...
	 *                     reading/writing files Otherwise, null.
	 */
	 ...
	 */
	public Map<Integer, String> convertARFFandReturnMap(String inputFile, String outputFile, int lineCount)
			throws IOException {
		// we save the parameter in the class fields
	 ...
	 */
	private Map<Integer, String> convertARFF(boolean ignoreMissingValues, boolean returnMapItemIDValue)
			throws IOException {
		// This map will be used to store mapping from item id (key) to attribute value
		// (value).
			...
			while ((thisLine = myInput.readLine()) != null) {
				...
				if (thisLine.startsWith("@ATTRIBUTE") || thisLine.startsWith("@attribute")) {
					...
					if (useQuotes) {
						...
						// get the position of the character just before the space after the attribute
						// name
...
//					System.out.println("DATA");
					...
					mapAttributeValuesItemsID = new ArrayList<Map<String, Integer>>(attributeCount);
...
//						System.out.println(entry.substring(indexOfFirstSpace+1));
						...
						if (itemID == null) {
							...
							if (mapItemsIDsToAttributeValues != null) {
								mapItemsIDsToAttributeValues.put(itemID, listAttributeNames.get(i) + "=" + val);
							}
							// add the unseen attribute value to the string for
							// unseen attribute values.
							unseenAttributeValues
									.append("@ITEM=" + itemID + "=" + listAttributeNames.get(i) + "=" + val + "\n");
						}
...
//				}
	...
	/**
	 * This method convert a file from the SPMF sequence database format to the SPMF
	 * transaction database format. Note that this code could be further optimized
