...
/**
 * This class is for converting sequence databases from various formats to the
 * SPMF format.
 * 
 * @see Formats
...
public class SequenceDatabaseConverter {
	...
	/**
	 * This method converts a sequence database from a given format to the SPMF
	 * format.
	 * 
	 * @param input           the path of the input file
	 * @param charset         the charset (encoding) of the input file if it is a
	 *                        text file (e.g "UTF-8") or null if you want to use the
	 *                        default text encoding or if it is not a text file.
	 * @param output          the path of the file to be written to disk in SPMF
	 *                        format
	 * @param inputFileformat the format of the input file
	 * @param lineCount       the number of lines from the input file that should be
	 *                        converted
	 * @throws IOException an exception is thrown if there is an error
	 *                     reading/writing files
	 */
	public void convert(String input, String output, Formats inputFileformat, int lineCount, Charset charset)
			throws IOException {
	...
	/**
	 * This method convert a transaction database in SPMF format to a sequence
	 * database in SPMF format
	 * 
	 * @param charset the charset (encoding) of the input file (e.g "UTF-8") or null
	 *                if you want to use the default text encoding.
	 */
	 ...
	 * 
	 * @param charset the charset (encoding) of the input file if it is a text file
	 *                (e.g "UTF-8") or null if you want to use the default text
	 *                encoding or if it is not a text file.
	 */
	...
	private void convertIBMGenerator() {
		...
		try {
			...
			// (because in the binary format, at the end of a line there is no -1 before the
			// -2
	 ...
	 * between item IDs and attribute value in memory to avoid an extra database
	 * scan.
	 * 
	 ...
	 * @param charset    the charset (encoding) of the input file if it is a text
	 *                   file (e.g "UTF-8") or null if you want to use the default
	 *                   text encoding or if it is not a text file.
	 * @return a map of entry (key : itemID, value: attribute-value) if the input
	 *         format is TEXT
	 * @throws IOException if an error while reading/writing files
	 */
	public Map<Integer, String> convertTEXTandReturnMap(String inputFile, String outputFile, int lineCount,
	 ...
	 * @param charset             the charset (encoding) of the input file(e.g
	 *                            "UTF-8") or null if you want to use the default
	 *                            text encoding.
	 * @throws IOException exception if error while reading/writing files.
	 * @return a map where an entry indicates for an item (key), the corresponding
	...
	private Map<Integer, String> convertTEXT(boolean returnMapItemIDWord, Charset charset) throws IOException {
		...
		// It is used only if returnMapItemIDValue is set to true. This is used by the
		// GUI of SPMF
		// which need to keep this information in memory to avoid an extra database scan
		// after an algorithm
		// is applied.
...
//				// for each word
				for (String word : words) {
					...
					boolean isEndOfSentence = word.endsWith("?") || word.endsWith(".") || word.endsWith("!")
							|| word.endsWith("\u002e") || word.endsWith("\u3002") // <-- This is the Chinese end of
																					// sentence (to support chinese
																					// text)
							|| word.endsWith("\uFF01") // chinese "!"
							|| word.endsWith("\u003F"); // chinese "?"

					// If this word contains punctuation, we remove it (this could include ,()"' and
			...
			// If some sentence has not ended because some punctuation is missing for the
