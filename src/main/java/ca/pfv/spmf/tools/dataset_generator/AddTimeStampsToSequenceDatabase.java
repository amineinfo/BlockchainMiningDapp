...
/**
 * This class is to read a sequence database in SPMF format, add consecutive
 * timestamps to itemsets in each sequence, and then write the sequence database
 * to a file.
 * 
...
public class AddTimeStampsToSequenceDatabase {
	...
	/**
	 * Read a sequence database in SPMF format, add timestamps and then write the
	 * sequence database to a file. Note that this code could be further optimized
	 * if performance is an issue (instead of loading the file into memory, it could
	 * be loaded line by line).
	 * 
