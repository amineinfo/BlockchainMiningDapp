 ...
 * integer with a tid list represented by a list of integers. Itemsets are
 * ordered by size. For example, level 1 means itemsets of size 1 (that contains
 * 1 item).
 * 
	...
	public void printItemsets(int nbObject) {
		...
		for (List<Itemset> level : levels) {
			...
			for (Itemset itemset : level) {
				...
				System.out.print("support :  " + itemset.getRelativeSupportAsString(nbObject));
				patternCount++;
				System.out.println("");
			}
			levelCount++;
