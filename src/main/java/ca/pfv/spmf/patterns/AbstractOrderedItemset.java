...
/**
 * This is an abstract class indicating general methods that an ordered itemset
 * should have, and is designed for ordered itemsets where items are sorted by
 * lexical order and no item can appear twice.
 * 
 * @see AbstractItemset
...
public abstract class AbstractOrderedItemset extends AbstractItemset {
	 ...
	 * 
	 * @param nbObject the number of transactions in the database where this itemset
	 *                 was found
	 * @return the relative support of the itemset as a double
	 */
	public double getRelativeSupport(int nbObject) {
		// Divide the absolute support by the number of transactions to get the relative
		// support
	...
	/**
	 * This methods checks if another itemset is contained in this one. The method
	 * assumed that items are lexically ordered in itemsets.
	 * 
	...
	/**
	 * Check if the items from this itemset are all the same as those of another
	 * itemset except the last item and that itemset2 is lexically smaller than this
	 * itemset. If all these conditions are satisfied, this method return the last
	 * item of itemset2. Otherwise it returns null.
	 * 
